...
class BranchLog {
 public:
  ...
  // Deletes the underlying WAL file without finalizing it. Called on the cap-exceeded path in
  // CaptureCommitIfBranch to remove the partially-written file that would otherwise be orphaned:
  // WalFile::~WalFile() only unlinks when count_ == 0, but by the time the cap check fires,
  // CaptureBranchCommit has already written deltas (count_ > 0). Idempotent — safe to call even if
  // Abandon() was already invoked (the abandoned_ guard suppresses the second DeleteWal()).
  // Must NOT be called after Finalize() — they are on mutually exclusive control-flow paths.
  void Abandon();
 ...
 private:
  ...
  // Set to true by Abandon() so a second call is a no-op (idempotency guard).
