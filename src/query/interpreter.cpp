    ...
    case SystemInfoQuery::InfoType::STORAGE: {
      ...
      handler = [storage = current_db.db_acc_->get()->storage(), interpreter_isolation_level,
        ...
        std::vector<std::vector<TypedValue>> results{
            ...
            {TypedValue("memory_usage"), TypedValue(utils::GetReadableSize(info.memory_usage))},
            {TypedValue("disk_usage"), TypedValue(utils::GetReadableSize(info.disk_usage))},
            {TypedValue("memory_allocated"), TypedValue(utils::GetReadableSize(utils::total_memory_tracker.Amount()))},
            {TypedValue("allocation_limit"),
             TypedValue(utils::GetReadableSize(utils::total_memory_tracker.HardLimit()))},
