                  ...
                  interpreter_context->interpreters.WithLock(
                      [db_name, interpreter_context, interpreter, &evicted](auto &interpreters) {
                        ...
                        interpreter_context->TerminateTransactions(
                            ...
                            privilege_checker);
                        // Also evict idle-pinned sessions from the dropped tenant so it drains without
                        // waiting for the idle reaper's timeout. TerminateTransactions above only aborts
