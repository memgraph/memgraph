...
ScanAllByLabel::ScanAllByLabel(const std::shared_ptr<LogicalOperator> &input, Symbol output_symbol,
    ...
    : ScanAll(input, std::move(output_symbol), view), label_(label) {}
...
ScanAllById::ScanAllById(const std::shared_ptr<LogicalOperator> &input, Symbol output_symbol, Expression *expression,
                         storage::View view)
    : ScanAll(input, std::move(output_symbol), view), expression_(expression) {
...
Expand::Expand(const std::shared_ptr<LogicalOperator> &input, Symbol input_symbol, Symbol node_symbol,
    ...
    : input_(input ? input : std::make_shared<Once>()),
      input_symbol_(std::move(input_symbol)),
      common_{std::move(node_symbol), std::move(edge_symbol), direction, edge_types, existing_node},
      view_(view) {}
...
Union::Union(std::shared_ptr<LogicalOperator> left_op, std::shared_ptr<LogicalOperator> right_op,
    ...
    : left_op_(std::move(left_op)),
      ...
      union_symbols_(std::move(union_symbols)),
      left_symbols_(std::move(left_symbols)),
      right_symbols_(std::move(right_symbols)) {}
