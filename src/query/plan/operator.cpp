 ...
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    while (result_row_it_ == result_.rows.end()) {
      ...
      result_.rows
          .clear();  // =
                     // utils::Allocator<mgp_result>(context.evaluation_context.memory).new_object<mgp_result>(nullptr,
  ...
  void Reset() override {
    //    monotonic_memory_.Release();
    result_.rows.clear();  // = utils::Allocator<mgp_result>(memory_resource_).new_object<mgp_result>(nullptr,
                           // memory_resource_);
    if (cleanup_) {
      cleanup_.value()();
    }
