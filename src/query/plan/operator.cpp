...
class ExpandAllShortestPathsCursorTemp : public query::plan::Cursor {
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    auto expand_pair = [this, &evaluator, &frame, &create_state, &context](
                           const EdgeAccessor &edge, const VertexAccessor &vertex,
                           const utils::pmr::vector<TypedValue> &edge_list, const TypedValue &total_weight,
                           int64_t depth) {
      auto *memory = evaluator.GetMemoryResource();
      if (self_.filter_lambda_.expression) {
        frame[self_.filter_lambda_.inner_edge_symbol] = edge;
      ...
      pq_.push({next_weight, depth + 1, vertex, new_edge_list});
    };
    ...
    // the "where" condition.
    auto expand_from_vertex = [this, &expand_pair](const VertexAccessor &vertex, const TypedValue &weight,
                                                   const utils::pmr::vector<TypedValue> &edge_list, int64_t depth) {
      if (self_.common_.direction != EdgeAtom::Direction::IN) {
        auto out_edges = UnwrapEdgesResult(vertex.OutEdges(storage::View::OLD, self_.common_.edge_types));
        for (const auto &edge : out_edges) {
          expand_pair(edge, edge.To(), edge_list, weight, depth);
        }
      }
      if (self_.common_.direction != EdgeAtom::Direction::OUT) {
        auto in_edges = UnwrapEdgesResult(vertex.InEdges(storage::View::OLD, self_.common_.edge_types));
        for (const auto &edge : in_edges) {
          expand_pair(edge, edge.From(), edge_list, weight, depth);
        }
      }
    };
    ...
    while (true) {
      ...
      if (pq_.empty()) {
        ...
        utils::pmr::vector<TypedValue> empty(mem);
        pq_.push({TypedValue(), 0, vertex, empty});
      }

      while (!pq_.empty()) {
        if (MustAbort(context)) throw HintedAbortError();
        // spdlog::info("Popping from queue. Queue size: {}", pq_.size());

        auto [current_weight, current_depth, current_vertex, current_edge_list] = pq_.top();
        pq_.pop();

        // spdlog::info("Popping from queue. Current path length: {}, Current depth: {}",  current_edge_list.size(),
        // current_depth); spdlog::info("Visited size: {}",  visited_cost_.size());

        // Check if the vertex has already been processed.
        if (visited_cost_.find(current_vertex) != visited_cost_.end()) {
          auto weight = visited_cost_.find(current_vertex)->second;
          // spdlog::info("Node visited, checking length...");
          if (weight.IsNull() || (current_weight <= weight).ValueBool()) {
            visited_cost_.emplace(current_vertex, current_weight);
            // spdlog::info("Length not found and updated.");
          } else {
            // spdlog::info("Skipped expansion");
            continue;
          }
        } else {
          visited_cost_.emplace(current_vertex, current_weight);
 ...
 private:
  ...
  class PriorityQueueComparator {
   public:
    bool operator()(const std::tuple<TypedValue, int64_t, VertexAccessor, utils::pmr::vector<TypedValue>> &lhs,
                    const std::tuple<TypedValue, int64_t, VertexAccessor, utils::pmr::vector<TypedValue>> &rhs) {
      const auto &lhs_weight = std::get<0>(lhs);
      const auto &rhs_weight = std::get<0>(rhs);
      // Null defines minimum value for all types
      if (lhs_weight.IsNull()) {
