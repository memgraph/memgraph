...
struct StoppingContext {
  ...
  auto MustAbort() const noexcept -> AbortReason {
    ...
    if (is_shutting_down && is_shutting_down->load(std::memory_order_acquire)) {
  ...
  auto MaybeMustAbort() const noexcept -> AbortReason {
    // Not thread safe
    return maybe_check_abort_() ? MustAbort() : AbortReason::NO_ABORT;
  }
};
