...
class AuthChecker {
 public:
  ...
  [[nodiscard]] virtual bool IsUserAdmin(const std::optional<std::string> &username) const = 0;
...
class AllowEverythingAuthChecker final : public query::AuthChecker {
 public:
  ...
  bool IsUserAdmin(const std::optional<std::string> & /*username*/) const override { return true; }
