...
Callback HandleAuthQuery(AuthQuery *auth_query, AuthQueryHandler *auth, const Parameters &parameters,
                         RequestRuntimeInterface *manager) {
...
Callback HandleReplicationQuery(ReplicationQuery *repl_query, const Parameters &parameters,
                                InterpreterContext *interpreter_context, RequestRuntimeInterface *manager,
...
struct PullPlan {
  explicit PullPlan(std::shared_ptr<CachedPlan> plan, const Parameters &parameters, bool is_profile_query,
                    ...
                    RequestRuntimeInterface *request_runtime = nullptr,
...
PullPlan::PullPlan(const std::shared_ptr<CachedPlan> plan, const Parameters &parameters, const bool is_profile_query,
                   DbAccessor *dba, InterpreterContext *interpreter_context, utils::MemoryResource *execution_memory,
                   RequestRuntimeInterface *request_runtime, const std::optional<size_t> memory_limit)
    : plan_(plan),
  ...
  ctx_.evaluation_context.properties = NamesToProperties(plan->ast_storage().properties_, request_runtime);
  ctx_.evaluation_context.labels = NamesToLabels(plan->ast_storage().labels_, request_runtime);
  if (interpreter_context->config.execution_timeout_sec > 0) {
    ctx_.timer = utils::AsyncTimer{interpreter_context->config.execution_timeout_sec};
  }
  ctx_.is_shutting_down = &interpreter_context->is_shutting_down;
  ctx_.is_profile_query = is_profile_query;
  ctx_.request_runtime = request_runtime;
...
Interpreter::Interpreter(InterpreterContext *interpreter_context) : interpreter_context_(interpreter_context) {
  ...
  request_runtime_ = std::make_unique<RequestRuntime<io::local_transport::LocalTransport>>(
...
PreparedQuery PrepareCypherQuery(ParsedQuery parsed_query, std::map<std::string, TypedValue> *summary,
                                 ...
                                 RequestRuntimeInterface *request_runtime) {
  //                                 TriggerContextCollector *trigger_context_collector = nullptr) {
  ...
  ExpressionEvaluator evaluator(&frame, symbol_table, evaluation_context, request_runtime, storage::v3::View::OLD);
  ...
  auto plan = CypherQueryToPlan(
      ...
      parsed_query.is_cacheable ? &interpreter_context->plan_cache : nullptr, request_runtime);
  ...
  auto pull_plan = std::make_shared<PullPlan>(plan, parsed_query.parameters, false, dba, interpreter_context,
                                              execution_memory, request_runtime, memory_limit);
...
PreparedQuery PrepareExplainQuery(ParsedQuery parsed_query, std::map<std::string, TypedValue> *summary,
                                  InterpreterContext *interpreter_context, RequestRuntimeInterface *request_runtime,
  ...
  auto cypher_query_plan =
      CypherQueryToPlan(parsed_inner_query.stripped_query.hash(), std::move(parsed_inner_query.ast_storage),
                        ...
                        parsed_inner_query.is_cacheable ? &interpreter_context->plan_cache : nullptr, request_runtime);

  std::stringstream printed_plan;
  plan::PrettyPrint(*request_runtime, &cypher_query_plan->plan(), &printed_plan);
  ...
  summary->insert_or_assign("explain", plan::PlanToJson(*request_runtime, &cypher_query_plan->plan()).dump());
...
PreparedQuery PrepareProfileQuery(ParsedQuery parsed_query, bool in_explicit_transaction,
  ...
  return PreparedQuery{{"OPERATOR", "ACTUAL HITS", "RELATIVE TIME", "ABSOLUTE TIME", "CUSTOM DATA"},
                       ...
                       [plan = std::move(cypher_query_plan), parameters = std::move(parsed_inner_query.parameters),
                        summary, dba, interpreter_context, execution_memory, memory_limit, request_runtime,
...
Interpreter::PrepareResult Interpreter::Prepare(const std::string &query_string,
  ...
  try {
    ...
    if (!in_explicit_transaction_ &&
      ...
      request_runtime_->StartTransaction();
    }
    ...
    if (utils::Downcast<CypherQuery>(parsed_query.query)) {
      prepared_query = PrepareCypherQuery(std::move(parsed_query), &query_execution->summary, interpreter_context_,
                                          ...
                                          &query_execution->notifications, request_runtime_.get());
    } else if (utils::Downcast<ExplainQuery>(parsed_query.query)) {
      prepared_query = PrepareExplainQuery(std::move(parsed_query), &query_execution->summary, interpreter_context_,
                                           &*request_runtime_, &query_execution->execution_memory_with_exception);
    } else if (utils::Downcast<ProfileQuery>(parsed_query.query)) {
      prepared_query = PrepareProfileQuery(std::move(parsed_query), in_explicit_transaction_, &query_execution->summary,
                                           interpreter_context_, &*execution_db_accessor_,
                                           &query_execution->execution_memory_with_exception, request_runtime_.get());
      ...
      prepared_query =
          PrepareReplicationQuery(std::move(parsed_query), in_explicit_transaction_, &query_execution->notifications,
                                  interpreter_context_, request_runtime_.get());
      ...
      prepared_query = PrepareSettingQuery(std::move(parsed_query), in_explicit_transaction_, request_runtime_.get());
