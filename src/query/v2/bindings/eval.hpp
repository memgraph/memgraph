...
class Callable {
 public:
  ...
  auto operator()(const msgs::Value &val, RequestRuntimeInterface *manager) const {
    return ValueToTypedValue(val, manager);
  };
};

}  // namespace detail
using ExpressionEvaluator = expr::ExpressionEvaluator<TypedValue, query::v2::EvaluationContext, RequestRuntimeInterface,
