...
PlanPrinter::PlanPrinter(const RequestRuntimeInterface *request_manager, std::ostream *out)
...
json ToJson(storage::v3::EdgeTypeId edge_type, const RequestRuntimeInterface &request_manager) {
  return request_manager.EdgeTypeToName(edge_type);
}

json ToJson(storage::v3::LabelId label, const RequestRuntimeInterface &request_manager) {
  return request_manager.LabelToName(label);
}

json ToJson(storage::v3::PropertyId property, const RequestRuntimeInterface &request_manager) {
...
json ToJson(const NodeCreationInfo &node_info, const RequestRuntimeInterface &request_manager) {
  json self;
...
json ToJson(const EdgeCreationInfo &edge_info, const RequestRuntimeInterface &request_manager) {
  json self;
