...
//  VertexAccessor(requests::Vertex{}, {});
...
class ExpandVariableCursor : public Cursor {
 ...
 private:
  ...
  bool Expand(Frame &frame, ExecutionContext &context) {
    ...
    //      if (self_.filter_lambda_.expression && !EvaluateFilter(evaluator, self_.filter_lambda_.expression))
    //      continue;
    //
...
class STShortestPathCursor : public query::v2::plan::Cursor {
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    //
    //      int64_t lower_bound =
    //          self_.lower_bound_ ? expr::EvaluateInt(&evaluator, self_.lower_bound_, "Min depth in breadth-first
    //          expansion")
    //                             : 1;
    //      int64_t upper_bound =
    //          self_.upper_bound_ ? expr::EvaluateInt(&evaluator, self_.upper_bound_, "Max depth in breadth-first
    //          expansion")
    //                             : std::numeric_limits<int64_t>::max();
    //
    //      if (upper_bound < 1 || lower_bound > upper_bound) continue;
    //
    //      //      if (FindPath(*context.db_accessor, source, sink, lower_bound, upper_bound, &frame, &evaluator,
    //      context)) {
    //      //        return true;
    //      //      }
    //    }
 ...
 private:
  ...
  //  void ReconstructPath(const VertexAccessor &midpoint, const VertexEdgeMapT &in_edge, const VertexEdgeMapT
  //  &out_edge,
  ...
  //  bool FindPath(const DbAccessor &dba, const VertexAccessor &source, const VertexAccessor &sink, int64_t
  //  lower_bound,
  //                int64_t upper_bound, Frame *frame, ExpressionEvaluator *evaluator, const ExecutionContext
  //                &context) {
...
class SingleSourceShortestPathCursor : public query::v2::plan::Cursor {
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    //                           ? expr::EvaluateInt(&evaluator, self_.lower_bound_, "Min depth in breadth-first
    //                           expansion") : 1;
    //        upper_bound_ = self_.upper_bound_
    //                           ? expr::EvaluateInt(&evaluator, self_.upper_bound_, "Max depth in breadth-first
    //                           expansion") : std::numeric_limits<int64_t>::max();
    //
...
class ExpandWeightedShortestPathCursor : public query::v2::plan::Cursor {
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    //    // queue.
    //    auto expand_pair = [this, &evaluator, &frame, &create_state](const EdgeAccessor &edge, const VertexAccessor
    //    &vertex,
    ...
    //        throw QueryRuntimeException("Calculated weight must be numeric or a Duration, got {}.",
    //        current_weight.type());
    //      }
    //
    ...
    //        return current_weight.IsDuration() && (current_weight >= TypedValue(utils::Duration(0),
    //        memory)).ValueBool();
    //      };
    //
    ...
    //      if (found_it != total_cost_.end() && (found_it->second.IsNull() || (found_it->second <=
    //      next_weight).ValueBool()))
    //        return;
    //
    ...
    //          upper_bound_ =
    //              expr::EvaluateInt(&evaluator, self_.upper_bound_, "Max depth in weighted shortest path
    //              expansion");
...
class CallProcedureCursor : public Cursor {
 ...
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    //        throw QueryRuntimeException("The procedure named '{}' was a {} procedure, but changed to be a {}
    //        procedure.",
    ...
    //      CallCustomProcedure(self_->procedure_name_, *proc, self_->arguments_, graph, &evaluator, memory,
    //      memory_limit,
    //                          &result_);
    //
    ...
    //        throw QueryRuntimeException("Procedure '{}' did not yield a record with '{}' field.",
    //        self_->procedure_name_,
    //                                    field_name);
    //      }
