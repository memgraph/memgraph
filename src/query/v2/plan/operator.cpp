 ...
 public:
  ...
  bool Pull(Frame &frame, ExecutionContext &context) override {
    ...
    if (input_cursor_->Pull(frame, context)) {
      auto &shard_manager = context.request_runtime;
      {
  ...
  void PlaceNodeOnTheFrame(Frame &frame, ExecutionContext &context) {
    ...
    frame[nodes_info_.front()->symbol] =
        TypedValue(query::v2::accessors::VertexAccessor(std::move(v), src_vertex_props_[0], context.request_runtime));
  }
    ...
    for (const auto &node_info : nodes_info_) {
      ...
      if (const auto *node_info_properties = std::get_if<PropertiesMapList>(&node_info->properties)) {
        for (const auto &[key, value_expression] : *node_info_properties) {
          ...
          if (context.request_runtime->IsPrimaryKey(primary_label, key)) {
        ...
        for (const auto &[key, value] : property_map) {
          ...
          auto property_id = context.request_runtime->NameToProperty(key_str);
          if (context.request_runtime->IsPrimaryKey(primary_label, property_id)) {
...
class DistributedScanAllAndFilterCursor : public Cursor {
 public:
  ...
  bool MakeRequest(RequestRuntimeInterface &shard_manager, ExecutionContext &context) {
    {
      SCOPED_REQUEST_WAIT_PROFILE;
...
bool Filter::FilterCursor::Pull(Frame &frame, ExecutionContext &context) {
  ...
  // nodes and edges.
  ExpressionEvaluator evaluator(&frame, context.symbol_table, context.evaluation_context, context.request_runtime,
                                storage::v3::View::OLD);
...
class LoadCsvCursor : public Cursor {
 ...
 private:
  csv::Reader MakeReader(ExecutionContext &context) {
    ...
    auto evaluator =
        ExpressionEvaluator(&frame, symbol_table, eval_context, context.request_runtime, storage::v3::View::OLD);
    ...
    for (const auto &edge_info : std::vector{self_.edge_info_}) {
      ...
      if (const auto *edge_info_properties = std::get_if<PropertiesMapList>(&edge_info.properties)) {
        ...
        for (const auto &[property, value] : property_map) {
          const auto property_id = context.request_runtime->NameToProperty(std::string(property));
      ...
      const auto set_vertex = [&context](const auto &vertex, auto &vertex_id) {
        ...
        for (const auto &[key, val] : vertex.Properties()) {
          if (context.request_runtime->IsPrimaryKey(vertex_id.first.id, key)) {
...
class DistributedExpandCursor : public Cursor {
 public:
  ...
  void PullDstVertex(Frame &frame, ExecutionContext &context, const EdgeAtom::Direction direction) {
    ...
    auto result_rows = context.request_runtime->Request(request_state, std::move(request));
    MG_ASSERT(result_rows.size() == 1);
    auto &result_row = result_rows.front();
    frame[self_.common_.node_symbol] = accessors::VertexAccessor(
        msgs::Vertex{result_row.src_vertex}, result_row.src_vertex_properties, context.request_runtime);
  }
  ...
  bool InitEdges(Frame &frame, ExecutionContext &context) {
    ...
    while (true) {
      ...
      auto result_rows = std::invoke([&context, &request_state, &request]() mutable {
        SCOPED_REQUEST_WAIT_PROFILE;
        return context.request_runtime->Request(request_state, std::move(request));
      });
      MG_ASSERT(result_rows.size() == 1);
      auto &result_row = result_rows.front();
