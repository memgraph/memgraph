...
class DistributedScanAllAndFilterCursor : public Cursor {
 public:
  ...
  /*
      ...
      In which case do we stop:
      -no more vertices returned from the MakeRequest
      -no more valid frame to modify in the multiframes

      When iterating over all valid frames,


  Another idea: maybe we need to provide a copy of multiframe to the input cursor?

  different approach: we take all valid frame from the input cursor, and copy it (how?) to the invalid frame within th
      then for each valid frame


    We take one vertex from the request, and put it on all valid frame from the input cursor then return.
    */
    ...
    while (true) {
      ...
      while (invalid_frames_populator.end() != invalid_frame_it && valid_frames_consumer_->end() != valid_frames_it_) {
        *invalid_frame_it = *valid_frames_it_;
        MG_ASSERT(invalid_frame_it->IsValid());
        invalid_frame_it->at(output_symbol_) = TypedValue(*current_vertex_it);  // #NoCommit use [] instead?
