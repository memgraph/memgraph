...
auto egraph::MakeLiteral(storage::ExternalPropertyValue const &value) -> eclass {
  return from_core(pimpl_->graph.Make<Literal>(value));
}

auto egraph::MakeParameterLookup(int32_t position) -> eclass {
  return from_core(pimpl_->graph.Make<ParamLookup>(position));
}

auto egraph::MakeBind(eclass input, eclass sym, eclass expr) -> eclass {
  return from_core(pimpl_->graph.Make<Bind>(to_core(input), to_core(sym), to_core(expr)));
}

auto egraph::MakeIdentifier(eclass sym) -> eclass { return from_core(pimpl_->graph.Make<Identifier>(to_core(sym))); }
...
auto egraph::MakeOutput(eclass input, std::span<eclass const> named_outputs) -> eclass {
  ...
  return from_core(pimpl_->graph.Make<Output>(std::move(children)));
}
...
auto egraph::MakeFunction(std::string_view name, std::span<eclass const> args, bool is_pure) -> eclass {
  ...
  return from_core(pimpl_->graph.Make<Function>(name, std::move(core_args), std::move(seed), is_pure));
}

auto egraph::MakeUnwind(eclass input, eclass sym, eclass list_expr) -> eclass {
  return from_core(pimpl_->graph.Make<Unwind>(to_core(input), to_core(sym), to_core(list_expr)));
}
...
auto egraph::MakeSubquery(eclass outer_input, eclass inner_root, std::span<eclass const> exposed_syms) -> eclass {
  ...
  return from_core(pimpl_->graph.Make<Subquery>(std::move(children)));
}
...
#define MG_DEFN_MAKE_UNARY(Name, ...) \
  auto egraph::Make##Name(eclass operand)->eclass { return from_core(pimpl_->graph.Make<Name>(to_core(operand))); }
