...
void Streams::Create(const std::string &stream_name, typename TStream::StreamInfo info,
                     std::optional<std::string> owner, TDbAccess db_acc, IStreamConsumerFactory &factory) {
...
template void Streams::Create<KafkaStream, dbms::DatabaseAccess>(const std::string &stream_name,
                                                                 ...
                                                                 IStreamConsumerFactory &factory);
...
Streams::StreamsMap::iterator Streams::CreateConsumer(StreamsMap &map, const std::string &stream_name,
  ...
  auto consumer_function = factory.make_consumer(memgraph::utils::tag<TStream>, std::move(db_acc), stream_name,
...
void Streams::RestoreStreams(TDbAccess db, IStreamConsumerFactory &factory) {
...
template void Streams::RestoreStreams<dbms::DatabaseAccess>(dbms::DatabaseAccess db, IStreamConsumerFactory &factory);
...
template <typename TDbAccess>
TransformationResult Streams::Check(const std::string &stream_name, TDbAccess db_acc, IStreamConsumerFactory &factory,
  ...
  return std::visit(
      [&]<typename TStreamData>(const TStreamData &stream_data) {
        ...
        auto consumer_function =
            factory.make_check_consumer(memgraph::utils::tag<typename TStreamData::stream_t>, std::move(db_acc),
                                        stream_name, transformation_name, test_result);
