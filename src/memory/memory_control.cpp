...
static void *my_alloc(extent_hooks_t *extent_hooks, void *new_addr, size_t size, size_t alignment, bool *zero,
  ...
  MG_ASSERT(old_hooks && old_hooks->alloc);
  return old_hooks->alloc(old_hooks, new_addr, size, alignment, zero, commit, arena_ind);
}

static bool my_dalloc(extent_hooks_t *extent_hooks, void *addr, size_t size, bool committed, unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper my_dalloc: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static void my_destroy(extent_hooks_t *extent_hooks, void *addr, size_t size, bool committed, unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper my_destroy: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static bool my_commit(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length,
                      unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper my_commit: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static bool my_decommit(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length,
                        unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper my_decommit: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static bool my_purge_forced(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length,
                            unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper purge_forced: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static bool my_purge_lazy(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t offset, size_t length,
                          unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper purge_lazy: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static bool my_split(extent_hooks_t *extent_hooks, void *addr, size_t size, size_t size_a, size_t size_b,
  ...
  // std::cout << fmt::format("In wrapper my_split: new_addr: {}, size: {}, arena_ind: {}", addr, size, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
static bool my_merge(extent_hooks_t *extent_hooks, void *addr_a, size_t size_a, void *addr_b, size_t size_b,
                     bool committed, unsigned arena_ind) {
  // std::cout << fmt::format("In wrapper my_merge: addr_a: {}, size_a: {}, arena_ind: {}", addr_a, size_a, arena_ind);
  // extent_hooks_t *old_hooks = original_hooks_vec[arena_ind];
...
void PrintStats() {
  ...
  sz = sizeof(unsigned);
  unsigned narenas{0};
  int success = mallctl("opt.narenas", (void *)&narenas, &sz, nullptr, 0);
  ...
  for (int i = 0; i < narenas; i++) {
    ...
    mallctlRead<extent_hooks_t *, true>(func_name.c_str(), &old_hooks);

    custom_hooks = {.alloc = old_hooks->alloc,
                    .dalloc = old_hooks->dalloc,
                    .commit = old_hooks->commit,
                    .decommit = old_hooks->decommit,
                    .destroy = old_hooks->destroy,
                    .merge = old_hooks->merge,
                    .purge_forced = old_hooks->purge_forced,
