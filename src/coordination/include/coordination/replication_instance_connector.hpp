...
class TimedFailureDetector {
 public:
  explicit TimedFailureDetector(uint32_t instance_down_timeout_sec);
  ...
  void UpdateInstanceDownTimeoutSec(uint32_t new_config);

 private:
  std::atomic<uint32_t> instance_down_timeout_sec_{5};
...
class ReplicationInstanceConnector {
 public:
  ...
  void UpdateInstanceDownTimeoutSec(uint32_t new_config) const;
