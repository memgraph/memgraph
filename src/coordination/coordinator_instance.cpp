...
// TODO: (andi) Check also replication endpoint, not just coordinator endpoint.
auto CoordinatorInstance::RegisterReplicationInstance(CoordinatorClientConfig config)
  ...
  // Not executed on commit but will change
  ...
  try {
    ...
    // TODO: (andi) Restore state in the RAFT log.
...
auto CoordinatorInstance::UnregisterReplicationInstance(std::string instance_name)
  ...
  spdlog::info("Instance {} erased", instance_name);
