...
struct static_vector {
  ...
  constexpr static_vector(static_vector &&other) noexcept : size_{other.size_} {
  ...
  constexpr explicit static_vector(std::initializer_list<T> other) : static_vector(other.begin(), other.end()) {}

  // TODO: change to range + add test
  constexpr explicit static_vector(std::span<T const> other) : static_vector(other.begin(), other.end()) {}
  // TODO: generalise to not just vector
  constexpr explicit static_vector(std::vector<T> &&other) : size_(other.size()) {
    std::ranges::uninitialized_move(other.begin(), other.end(), begin(), end());
  }

  template <typename It>
  constexpr explicit static_vector(It first, It last) : size_(std::distance(first, last)) {
    std::ranges::uninitialized_copy(first, last, begin(), end());
  }

  constexpr bool is_full() const { return N == size_; }
  ...
  constexpr auto operator[](size_type index) -> reference { return begin()[index]; }

  constexpr auto operator[](size_type index) const -> const_reference { return begin()[index]; }
  ...
  constexpr auto begin() -> iterator { return iterator{buffer_->as()}; }

  constexpr auto end() -> iterator { return begin() + size_; }

  [[nodiscard]] constexpr auto begin() const -> const_iterator { return const_iterator{buffer_->as()}; }
  ...
  constexpr friend bool operator==(static_vector const &lhs, static_vector const &rhs) {
    return std::ranges::equal(lhs, rhs);
  }
  constexpr friend bool operator==(std::span<T const> lhs, static_vector const &rhs) {
    return std::ranges::equal(lhs, rhs);
  }
  constexpr friend bool operator==(static_vector const &lhs, std::span<T const> rhs) {
    return std::ranges::equal(lhs, rhs);
  }
