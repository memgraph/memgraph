...
void PriorityThreadPool::ParkAdmission(TaskSignature task, TaskID id, std::chrono::steady_clock::time_point deadline) {
  ...
  parked_admissions_.push_back({id, std::move(task), deadline});
}
...
void PriorityThreadPool::WakeOneParked() {
  TaskSignature task;
  TaskID id;
  {
    std::unique_lock lk{parked_mtx_};  // no unlocked pre-check: racing a non-atomic deque read is UB
    if (parked_admissions_.empty()) return;
    id = parked_admissions_.front().id;
    task = std::move(parked_admissions_.front().task);
    parked_admissions_.pop_front();
  }  // release parked_mtx_ BEFORE ScheduledReAddTask
  ScheduledReAddTask(std::move(task), id, utils::Priority::LOW, /*productive=*/false);
}
