...
// B1.1 HAND-MERGED ctor: v2's (mixed, hp, init_cb) signature + added trailing
// WorkerYieldRegistry* yield_registry = nullptr. All v2 HP thread machinery kept.
// p3's worker_id_ / yield_registry_ publish-before-barrier pattern added for LP workers.
PriorityThreadPool::PriorityThreadPool(uint16_t mixed_work_threads_count, uint16_t high_priority_threads_count,
  ...
  for (size_t i = 0; i < high_priority_threads_count; ++i) {
    pool_.emplace_back([this, i, &barrier, thread_init_callback]() {
      ...
      // HP workers do not need yield_registry_ (they are never preempted by higher priority
      // tasks — they ARE the highest priority). worker_id_ is not published for HP workers
      // because push()'s yield-signal guard only fires for LP workers.
      barrier.arrive_and_wait();
      // Call user-defined thread initialization callback (e.g., to register with Python interpreter)
      if (thread_init_callback) {
        thread_init_callback();
  ...
  // sched_mon only touches work_ (stealable queue) — work_pinned_ is invisible to it,
  // which is what lets pinned continuations never be migrated by the monitor.
                   ...
                   last_task = std::array<TaskID, kMaxWorkers>{}]() mutable {
                    ...
                    for (auto &worker : workers_) {
                      ...
                      if (worker_last_task == worker->last_task_ && worker->working_ && worker->has_pending_work_) {
                        ...
                        // Recheck under lock — only move from work_ (never work_pinned_)
...
void PriorityThreadPool::ScheduledAddTask(TaskSignature new_task, const Priority priority) {
  if (pool_stop_source_.stop_requested()) [[unlikely]] {
    new_task(priority);  // Execute synchronously instead of dropping - prevents deadlock (P9)
    return;
  }
...
bool CurrentResumableTask::RegisterWaiter(WorkerResumeEvent &event, uint64_t observed_epoch) {
  ...
  TaskSignature resume_task = [resume = state.resume_task](Priority) mutable { resume(); };
...
// KEY SIGNATURE DECISION: ResumableWrapper::Run pushes via ScheduledAddTask with a
// [w](Priority) mutable closure (Priority ignored inside Run). This bridges the internal
...
void PriorityThreadPool::ScheduleResumableTask(ResumableTaskSignature task, Priority priority) {
  ...
  struct ResumableWrapper {
    ...
    void Run() {
      ...
      if (auto wid = WorkerYieldRegistry::GetCurrentWorkerId()) {
        pool->RescheduleTaskOnWorker(*wid, [w = *this](Priority) mutable { w.Run(); });
      } else {
        // No worker ID - use ScheduledAddTask as fallback
        pool->ScheduledAddTask([w = *this](Priority) mutable { w.Run(); }, priority);
      }
    }
  };
  ...
  ScheduledAddTask([w](Priority) mutable { w.Run(); }, priority);
}
...
void PriorityThreadPool::RescheduleTaskOnWorker(uint16_t worker_id, TaskSignature task) {
  if (pool_stop_source_.stop_requested()) [[unlikely]] {
    task(Priority::LOW);  // Execute synchronously instead of dropping - prevents deadlock (P4)
    return;
  }
  DMG_ASSERT(
...
// B1.1 HAND-MERGED push: adds work_pinned_ routing + yield-signal guard verbatim from p3.
// Pinned tasks never trigger preemption (they are internal continuations).
void PriorityThreadPool::Worker::push(TaskSignature new_task, TaskID id, bool pinned) {
  {
    ...
    // TODO think about atomic ordering and if this can be missed or requested when not needed
...
// B1.1 HAND-MERGED worker loop: v2's HP `if constexpr` steal branches kept verbatim.
// Added from p3: dual-queue pop_task, Phase-1A yield-registry re-arm, Phase-1B/4 both-queue checks,
// SetCurrentWorker/ClearCurrentWorker at loop start/end. Steal phase and sched_mon untouched.
...
void PriorityThreadPool::Worker::operator()(const uint16_t worker_id,
  ...
  // a concurrent push()). Use the by-value params below for TLS registration only.
  if (yield_registry) {
  ...
  // B1.1: dual-queue pop_task — prefers higher-priority task (larger id) from either queue.
  // Steal phase + sched_mon only ever see work_ (stealable queue); work_pinned_ is invisible to them.
  auto pop_task = [&] {
  ...
  while (run_.load(std::memory_order_acquire)) {
    ...
    if (task) {
      ...
      // B1.1: yield-registry re-arm (LP workers only; HP workers pass nullptr yield_registry).
      if (yield_registry) {
...
void TaskCollection::NotifyProgress() {
  ...
  if (waiter_pool && waiter_worker_id) {
    ...
    waiter_task(Priority::LOW);  // unreachable today (EC-2 blocks stolen tasks from registering); safe via CAS anyway
  }
}
...
bool CollectionScheduler::RegisterProgressWaiter(uint64_t observed_epoch) const {
  ...
  TaskSignature resume_task = [fn = *resume_fn](Priority) mutable { fn(); };
  ...
  for (auto &waiter : waiters) {
    if (waiter.handle) {
      ...
      if (waiter.pool && waiter.worker_id) {
        waiter.pool->RescheduleTaskOnWorker(*waiter.worker_id, [handle = waiter.handle](Priority) mutable {
    ...
    waiter.task(Priority::LOW);
  }
}

}  // namespace memgraph::utils

// B1.1: explicit instantiations now take WorkerYieldRegistry* param (added to both LOW and HIGH).
