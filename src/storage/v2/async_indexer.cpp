...
void AsyncIndexer::Start(std::stop_token stop_token, Storage *storage) {
  index_creator_thread_ = memory::DbAwareThread{[this, stop_token, storage, acquire_fn = acquire_arena_fn_](
                                                    std::stop_token thread_stop_token) mutable {
    const memory::DbArenaScope db_arena_scope{acquire_fn ? acquire_fn() : storage->BaseArenaIdx()};
