...
// `schema_progress` is called once per vertex during index-population and constraint-validation scans;
// returning true cancels the in-progress build (peer gone).  Defaults to neverCancel so the recovery
// caller (wal_window_replay.cpp) needs no change -- it has no live heartbeat to cancel against.
//
// `report_progress` is passed to text/point/vector builders that record heartbeat ticks but have no
// cancellation hook of their own.  Defaults to an empty (no-op) ProgressCallback.
void ApplyWalSchemaDelta(ReplicationAccessor *accessor, InMemoryStorage *storage, durability::WalDeltaData const &delta,
                         uint64_t current_delta_idx, CheckCancelFunction const &schema_progress = neverCancel,
