...
RecoveredSnapshot LoadCurrentVersionSnapshot(
  ...
  // Recover indices.
  {
...

    {
      ...
      auto size = snapshot.ReadUint();
      if (!size) throw RecoveryFailure("Couldn't read the number of global vertex property indices");
      spdlog::info("Recovering metadata of {} global vertex property indices.", *size);
      for (uint64_t i = 0; i < *size; ++i) {
        auto property = snapshot.ReadUint();
...
void EnsureNecessaryWalFilesExist(const std::filesystem::path &wal_directory, const std::string &uuid,
  ...
  for (const auto &item : std::filesystem::directory_iterator(wal_directory, error_code)) {
    ...
    try {
      // Only the uuid decides whether the file is deleted here, so it is read on its own before ReadWalContents,
      // which for a file the writer never finalized has to parse the deltas.
      auto const header = ReadWalHeader(item.path());
      if (header.uuid != uuid) {
        file_retainer->DeleteFile(item.path());
        continue;
      }
      auto const info = ReadWalContents(item.path());
