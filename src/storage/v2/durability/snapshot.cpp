...
RecoveredSnapshot LoadSnapshotVersion22(const std::filesystem::path &path, utils::SkipList<Vertex> *vertices,
                                        utils::SkipList<Edge> *edges, utils::SkipList<EdgeMetadata> *edges_metadata,
                                        std::deque<std::pair<std::string, uint64_t>> *epoch_history,
                                        NameIdMapper *name_id_mapper, std::atomic<uint64_t> *edge_count,
                                        SharedSchemaTracking *schema_info, const Config &config,
  ...
  // Recover indices.
  {
    ...
    // Recover label indices statistics.
    {
      ...
      spdlog::info("Metadata of label indices statistics are recovered.");
    }
    ...
    // Recover label+property indices statistics.
    {
      ...
      spdlog::info("Metadata of label+property indices statistics are recovered.");
    }

    spdlog::info("Recovering metadata of edge indices.");
    if (!snapshot.SetPosition(info.offset_edge_indices)) throw RecoveryFailure("Couldn't read data from snapshot!");
...
RecoveredSnapshot LoadSnapshot(const std::filesystem::path &path, utils::SkipList<Vertex> *vertices,
  ...
  if (*version == 22U) {
    return LoadSnapshotVersion22(path, vertices, edges, edges_metadata, epoch_history, name_id_mapper, edge_count,
                                 schema_info, config, enum_store);
  }
  ...
  // Recover indices.
  {
    ...
    {
      auto size = snapshot.ReadUint();
      if (!size) throw RecoveryFailure("Couldn't recover the number of label property composite indices!");
      spdlog::info("Recovering metadata of {} label+property composite indices.", *size);
      for (uint64_t i = 0; i < *size; ++i) {
        auto label = snapshot.ReadUint();
        if (!label) throw RecoveryFailure("Couldn't read label for label-property composite index!");
        auto properties_count = snapshot.ReadUint();
        if (!properties_count)
          throw RecoveryFailure("Couldn't read the number of properties in label-property composite index!");
        std::vector<PropertyId> properties;
        for (uint64_t j = 0; j < *properties_count; ++j) {
          auto property = snapshot.ReadUint();
          if (!property) throw RecoveryFailure("Couldn't read property of label-property composite index!");
          properties.emplace_back(get_property_from_id(*property));
        }
        AddRecoveredIndexConstraint(&indices_constraints.indices.label_property_composite,
                                    {get_label_from_id(*label), properties},
                                    "The label+property composite index already exists!");
        SPDLOG_TRACE("Recovered metadata of label+property composite index for :{}",
                     name_id_mapper->IdToName(snapshot_id_map.at(*label)));
      }
      spdlog::info("Metadata of label+property composite indices are recovered.");
    }
    ...
    // Recover label+property indices statistics.
    {
      auto size = snapshot.ReadUint();
      if (!size)
        throw RecoveryFailure("Couldn't recover the number of entries for label-property composite statistics!");
      spdlog::info("Recovering metadata of {} label+property composite indices statistics.", *size);
      for (uint64_t i = 0; i < *size; ++i) {
        const auto label = snapshot.ReadUint();
        if (!label) throw RecoveryFailure("Couldn't read label for label-property composite index statistics!");
        auto properties_count = snapshot.ReadUint();
        if (!properties_count)
          throw RecoveryFailure("Couldn't read the number of properties in label-property composite index statistics!");
        std::vector<PropertyId> properties;
        properties.reserve(*properties_count);
        for (uint64_t j = 0; j < *properties_count; ++j) {
          auto property = snapshot.ReadUint();
          if (!property) throw RecoveryFailure("Couldn't read property of label-property composite index statistics!");
          properties.emplace_back(get_property_from_id(*property));
        }
        const auto count = snapshot.ReadUint();
        if (!count) throw RecoveryFailure("Couldn't read count for label-property composite index statistics!!");
        const auto distinct_values_count = snapshot.ReadUint();
        if (!distinct_values_count)
          throw RecoveryFailure("Couldn't read distinct values count for label-property composite index statistics!");
        const auto statistic = snapshot.ReadDouble();
        if (!statistic)
          throw RecoveryFailure("Couldn't read statistics value for label-property composite index statistics!");
        const auto avg_group_size = snapshot.ReadDouble();
        if (!avg_group_size)
          throw RecoveryFailure("Couldn't read average group size for label-property composite index statistics!");
        const auto avg_degree = snapshot.ReadDouble();
        if (!avg_degree)
      ...
      spdlog::info("Metadata of label+property composite indices statistics are recovered.");
    }
