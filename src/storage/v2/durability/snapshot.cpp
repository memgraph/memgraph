...
SnapshotInfo ReadSnapshotInfo(const std::filesystem::path &path) {
  ...
  // Read metadata.
  {
    ...
    auto marker = snapshot.ReadMarker();
    if (!marker || *marker != Marker::SECTION_METADATA)
      throw IncompleteSnapshotFailure("Couldn't read marker for section metadata!");
  ...
  // Recover indices.
  {
    ...
    // Recover vector indices.
    {
      ...
      for (uint64_t i = 0; i < *size; ++i) {
        ...
        indices_constraints.indices.vector_indices.emplace_back(std::move(index_name.value()),
