...
void ApplyLabelIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
                           ...
                           CheckCancelFunction const &schema_progress) {
  spdlog::trace("   Delta {}. Create label index on :{}", current_delta_idx, data.label);
  if (!accessor->CreateIndex(storage->NameToLabel(data.label), schema_progress))
...
void ApplyLabelIndexDrop(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  if (!accessor->DropIndex(storage->NameToLabel(data.label), AbsentIndex::kIsRecorded))
...
void ApplyLabelPropertyIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage, NameIdMapper *mapper,
  ...
  if (!accessor->CreateIndex(
          storage->NameToLabel(data.label), std::move(property_paths), IndexOrder::ASC, schema_progress))
...
void ApplyLabelPropertyIndexDrop(ReplicationAccessor *accessor, InMemoryStorage *storage, NameIdMapper *mapper,
  ...
  if (!accessor->DropIndex(storage->NameToLabel(data.label),
                           ...
                           data.order.value_or(IndexOrder::ASC),
                           AbsentIndex::kIsRecorded)) {
    throw utils::BasicException(
...
void ApplyEdgeTypeIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  if (!accessor->CreateIndex(storage->NameToEdgeType(data.edge_type), schema_progress)) {
...
void ApplyEdgeTypeIndexDrop(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  if (!accessor->DropIndex(storage->NameToEdgeType(data.edge_type), AbsentIndex::kIsRecorded)) {
...
void ApplyEdgeTypePropertyIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
                                      ...
                                      uint64_t current_delta_idx, CheckCancelFunction const &schema_progress) {
...
void ApplyEdgeTypePropertyIndexDrop(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  if (!accessor
           ->DropIndex(storage->NameToEdgeType(data.edge_type),
...
void ApplyEdgePropertyIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  if (!accessor->CreateGlobalEdgeIndex(storage->NameToProperty(data.property), schema_progress)) {
...
void ApplyEdgePropertyIndexDrop(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  if (!accessor->DropGlobalEdgeIndex(storage->NameToProperty(data.property), AbsentIndex::kIsRecorded)) {
    throw utils::BasicException("Failed to drop global edge property index on ({}).", data.property);
  }
}

void ApplyVertexPropertyIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
                                    durability::WalVertexPropertyIndexCreate const &data,
                                    uint64_t /*current_delta_idx*/, CheckCancelFunction const &schema_progress) {
  spdlog::trace("       Create global vertex property index on ({})", data.property);
  if (!accessor->CreateGlobalVertexIndex(storage->NameToProperty(data.property), schema_progress)) {
    throw utils::BasicException("Failed to create global vertex property index on ({}).", data.property);
  }
}

void ApplyVertexPropertyIndexDrop(ReplicationAccessor *accessor, InMemoryStorage *storage,
                                  durability::WalVertexPropertyIndexDrop const &data, uint64_t /*current_delta_idx*/) {
  spdlog::trace("       Drop global vertex property index on ({})", data.property);
  if (!accessor->DropGlobalVertexIndex(storage->NameToProperty(data.property), AbsentIndex::kIsRecorded)) {
...
void ApplyTextIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
                          ...
                          ProgressCallback const &report_progress) {
  ...
  auto ret = accessor->CreateTextIndex(storage::TextIndexSpec{data.index_name, label_id, prop_ids}, report_progress);
  if (!ret) {
...
void ApplyTextEdgeIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  auto prop_ids = data.properties |
                  ...
                  r::to_vector;
  const auto ret =
      accessor->CreateTextEdgeIndex(storage::TextEdgeIndexSpec{data.index_name, edge_type, prop_ids}, report_progress);
  if (!ret) {
...
void ApplyExistenceConstraintCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  auto ret = accessor->CreateExistenceConstraint(
...
void ApplyUniqueConstraintCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  auto ret = accessor->CreateUniqueConstraint(storage->NameToLabel(data.label), properties, schema_progress);
...
void ApplyPointIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  auto res = accessor->CreatePointIndex(labelId, propId, report_progress);
  if (!res) {
...
void ApplyVectorIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  auto res = accessor->CreateVectorIndex(
...
void ApplyVectorEdgeIndexCreate(ReplicationAccessor *accessor, InMemoryStorage *storage,
  ...
  auto res = accessor->CreateVectorEdgeIndex(
...
void ApplyVectorIndexDrop(ReplicationAccessor *accessor, durability::WalVectorIndexDrop const &data,
                          uint64_t current_delta_idx, ProgressCallback const &report_progress) {
  spdlog::trace("   Delta {}. Drop vector index {} ", current_delta_idx, data.index_name);
  auto res = accessor->DropVectorIndex(data.index_name, report_progress);
  if (!res) {
...
void ApplyDescriptionSet(ReplicationAccessor *accessor, durability::WalDescriptionSet const &data,
  ...
  switch (data.kind) {
    ...
    case DescriptionTargetKind::PROPERTY_VALUE:
      accessor->SetPropertyValueDescription(data.property, data.value, data.description);
      break;
    default:
      break;
  }
...
void ApplyDescriptionDelete(ReplicationAccessor *accessor, durability::WalDescriptionDelete const &data,
  ...
  switch (data.kind) {
    ...
    case DescriptionTargetKind::EDGE_TYPE_PATTERN_PROPERTY:
      ...
      break;
    case DescriptionTargetKind::PROPERTY_VALUE:
      accessor->DeletePropertyValueDescription(data.property, data.value);
      break;
    default:
      break;
  }
...
void ApplyWalSchemaDelta(ReplicationAccessor *accessor, InMemoryStorage *storage, durability::WalDeltaData const &delta,
  ...
  if (auto const *data = std::get_if<WalLabelIndexCreate>(&delta.data_)) {
    ApplyLabelIndexCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalLabelPropertyIndexCreate>(&delta.data_)) {
    ApplyLabelPropertyIndexCreate(accessor, storage, mapper, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalEdgeTypeIndexCreate>(&delta.data_)) {
    ApplyEdgeTypeIndexCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalEdgeTypePropertyIndexCreate>(&delta.data_)) {
    ApplyEdgeTypePropertyIndexCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalEdgePropertyIndexCreate>(&delta.data_)) {
    ApplyEdgePropertyIndexCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalVertexPropertyIndexCreate>(&delta.data_)) {
    ApplyVertexPropertyIndexCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  if (auto const *data = std::get_if<WalVertexPropertyIndexDrop>(&delta.data_)) {
    ApplyVertexPropertyIndexDrop(accessor, storage, *data, current_delta_idx);
    return;
  }
  if (auto const *data = std::get_if<WalTextIndexCreate>(&delta.data_)) {
    ApplyTextIndexCreate(accessor, storage, *data, current_delta_idx, report_progress);
    return;
  }
  if (auto const *data = std::get_if<WalTextEdgeIndexCreate>(&delta.data_)) {
    ApplyTextEdgeIndexCreate(accessor, storage, *data, current_delta_idx, report_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalExistenceConstraintCreate>(&delta.data_)) {
    ApplyExistenceConstraintCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalUniqueConstraintCreate>(&delta.data_)) {
    ApplyUniqueConstraintCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalTypeConstraintCreate>(&delta.data_)) {
    ApplyTypeConstraintCreate(accessor, storage, *data, current_delta_idx, schema_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalPointIndexCreate>(&delta.data_)) {
    ApplyPointIndexCreate(accessor, storage, *data, current_delta_idx, report_progress);
    return;
  }
  ...
  if (auto const *data = std::get_if<WalVectorIndexCreate>(&delta.data_)) {
    ApplyVectorIndexCreate(accessor, storage, *data, current_delta_idx, report_progress);
    return;
  }
  if (auto const *data = std::get_if<WalVectorEdgeIndexCreate>(&delta.data_)) {
    ApplyVectorEdgeIndexCreate(accessor, storage, *data, current_delta_idx, report_progress);
    return;
  }
  if (auto const *data = std::get_if<WalVectorIndexDrop>(&delta.data_)) {
    ApplyVectorIndexDrop(accessor, *data, current_delta_idx, report_progress);
    return;
  }
