...
WindowTxnOutcome ReplayOneWindowTransaction(InMemoryStorage *storage, BaseDecoder *decoder, uint64_t version,
  ...
  for (bool transaction_complete = false; !transaction_complete; ++delta_idx) {
    ...
    // N3 fix: old-format WAL (version < kMetaDataDeltasHaveExplicitTransactionEnd, i.e. v14/v15)
    // writes schema/index/constraint/enum/TTL/description deltas as STANDALONE records with no
    // enclosing WalTransactionStart/WalTransactionEnd. IsWalDeltaDataTransactionEnd already returns
    // true for them (setting transaction_complete above), but the WalTransactionEnd commit block at
    // the top of this loop body is never entered because the delta does not hold WalTransactionEnd,
    // and that block's `continue` is never hit. Without this path the accessor exits scope
    // uncommitted, Abort() fires the schema op's abort callback, and the replayed op is silently
    // undone on restart. Mirror the explicit-end commit sequence verbatim.
    //
    // Cannot double-commit: WalTransactionEnd deltas are caught by `holds_alternative<WalTransactionEnd>`
    // above and `continue` past this point; the guard below is a belt-and-suspenders defence.
    // For new-format WAL (version >= kMetaDataDeltasHaveExplicitTransactionEnd), transaction_complete
    // is only true for WalTransactionEnd, which never reaches this point -- so this block is a no-op
    // for all modern WAL files.
    if (transaction_complete && !std::holds_alternative<WalTransactionEnd>(delta.data_)) {
      if (should_commit && accessor) {
        storage->timestamp_ = commit_ts;
        auto const commit_result = accessor->PrepareForCommitPhase(CommitArgs::make_recovery_replay(commit_ts));
        if (!commit_result) {
