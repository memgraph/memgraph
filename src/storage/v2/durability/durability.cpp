...
std::optional<RecoveryInfo> RecoverData(const std::filesystem::path &snapshot_directory,
  ...
  if (!snapshot_files.empty()) {
    ...
    *uuid = snapshot_files.back().uuid;
    ...
    repl_state.SetEpoch(std::move(recovered_snapshot->snapshot_info.epoch_id));
    ...
    *uuid = std::move(wal_files.back().uuid);
    repl_state.SetEpoch(std::move(wal_files.back().epoch_id));
  }
  ...
  if (!wal_files.empty()) {
    ...
    for (auto &wal_file : wal_files) {
      ...
      if (wal_file.epoch_id != repl_state.GetEpoch().id()) {
        ...
        repl_state.SetEpoch(std::move(wal_file.epoch_id));
      }
      try {
