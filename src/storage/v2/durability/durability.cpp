...
GetWalFiles(const std::filesystem::path &wal_directory,
    ...
    try {
      ...
      if ((uuid.empty() || info.uuid == uuid) &&
        ...
        wal_files.emplace_back(info.seq_num, info.from_timestamp,
                               info.to_timestamp, info.uuid, item.path());
  ...
  std::sort(wal_files.begin(), wal_files.end(), by_seq_num);
...
std::optional<RecoveryInfo> RecoverData(
  ...
  if (!snapshot_files.empty()) {
    ...
    auto by_path = [](auto& snap_a, auto& snap_b) {
                      return snap_a.path < snap_b.path;
                   };
  ...
  if (wal_files.empty()) {
    ...
    std::sort(wal_files.begin(), wal_files.end(), by_seq_num);
    {
      const auto &[seq_num, from_timestamp, to_timestamp, _, path] = wal_files[0];
      if (seq_num != 0) {
