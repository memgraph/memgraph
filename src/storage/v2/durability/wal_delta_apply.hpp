...
// Translates a WAL-layer TransactionAccessType (captured from a WalTransactionStart delta) into the
// storage-layer StorageAccessType required when opening a recovery or replication accessor. This is
// a pure 1:1 enum mapping shared by wal_window_replay.cpp (branch replay) and
// replication_handlers.cpp (replica apply); keeping it here avoids the mapping being duplicated
// between the two callers.
inline StorageAccessType TranslateAccessType(durability::TransactionAccessType access_type) {
  switch (access_type) {
    case durability::TransactionAccessType::UNIQUE:
      return StorageAccessType::UNIQUE;
    case durability::TransactionAccessType::WRITE:
      return StorageAccessType::WRITE;
    case durability::TransactionAccessType::READ:
      return StorageAccessType::READ;
    case durability::TransactionAccessType::READ_ONLY:
      return StorageAccessType::READ_ONLY;
    default:
      throw std::runtime_error("Unrecognized WAL transaction access type");
  }
}
