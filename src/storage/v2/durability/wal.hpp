...
struct WalEdgeSetProperty {
  ...
  using ctr_types =
      std::tuple<Gid, std::string, ExternalPropertyValue, VersionDependant<kEdgeSetDeltaWithVertexInfo, Gid>,
                 VersionDependant<kEdgeSetDeltaWithInVertexInfo, std::optional<Gid>>,
                 VersionDependant<kEdgeSetDeltaWithEdgeType, std::string>>;
  Gid gid;
  std::string property;
  ExternalPropertyValue value;
  std::optional<Gid> from_gid;  //!< Used to simplify the edge search (from kEdgeSetDeltaWithVertexInfo)
  std::optional<Gid> to_gid;    //!< In-vertex GID for faster replica resolution (from kEdgeSetDeltaWithInVertexInfo)
  std::optional<std::string> edge_type;  //!< Edge type name for FindEdge (from kEdgeSetDeltaWithEdgeType)
};
