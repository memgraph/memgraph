...
class TransactionReplication {
 public:
  ...
  // Updates commit_ts_info on all non-ASYNC replicas that did not appear in the supplied `failures` (start-txn +
  // ship-delta failures collected by the caller) or in `finalize_failures_` (2nd-phase 2PC failures recorded by
  // FinalizeTransaction).
  void UpdateCommitTsInfo(uint64_t durability_commit_timestamp, std::vector<ReplicaFailure> const &failures);
 ...
 private:
  ...
  // Replicas that failed the 2nd phase of 2PC (SendFinalizeCommitRpc failed). Populated by FinalizeTransaction
  // and consulted by UpdateCommitTsInfo so we don't bump commit_ts_info_ for replicas that didn't actually commit.
