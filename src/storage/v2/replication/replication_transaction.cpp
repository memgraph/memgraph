...
void TransactionReplication::UpdateCommitTsInfo(uint64_t const durability_commit_timestamp,
                                                std::vector<ReplicaFailure> const &failures) {
  if (locked_clients->empty()) return;
  ...
  bool const has_failures = !failures.empty() || !finalize_failures_.empty();
  for (auto const &client : *locked_clients) {
    ...
    if (has_failures) {
      auto const &name = client->Name();
      auto const matches = [&](ReplicaFailure const &f) { return f.name == name; };
      if (std::ranges::any_of(failures, matches) || std::ranges::any_of(finalize_failures_, matches)) {
        continue;
      }
    }
