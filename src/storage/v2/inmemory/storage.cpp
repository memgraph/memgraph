...
InMemoryStorage::InMemoryStorage(Config config, std::optional<free_mem_fn> free_mem_fn_override,
  ...
  if (config_.durability.recover_on_startup) {
    ...
    // because WalEdgeDelete calls DestroyLightEdge directly without queuing).
...
InMemoryStorage::~InMemoryStorage() {
  ...
  // PR7b-ii leak fix: a deleted light edge whose delta chain was never GC-unlinked
...
InMemoryStorage::InMemoryAccessor::DetachDelete(std::vector<VertexAccessor *> nodes, std::vector<EdgeAccessor *> edges,
  ...
  auto const inform_gc_edge_deletion = utils::OnScopeExit{[this, &deleted_edges = deleted_edges]() {
    if (!deleted_edges.empty() && transaction_.storage_mode == StorageMode::IN_MEMORY_ANALYTICAL) {
      ...
      // PR7b-ii leak fix: in analytical mode the edge is pop_back'd from vertex
      // adjacency at delete time (storage.cpp DeleteEdge ~528), so the GC full-scan
      // light arm (which iterates LightEdgeIterable over LIVE adjacency) can never
      // find these light Edge* -> they would be unreachable and leak (heavy works
      // because its node persists in the edges_ skip-list scanned by the heavy
      // full-scan arm). Route the deleted LIGHT Edge* into deleted_edges_, exactly
      // like the transactional FastDiscard path (~1400). CollectGarbage swaps
      // deleted_edges_ (~3013) into current_deleted_edges and the light arm
      // (~3299-3312) pushes them to the graveyard with guard_epoch snapped at
...
void InMemoryStorage::InMemoryAccessor::FastDiscardOfDeltas(std::unique_lock<std::mutex> /*gc_guard*/) {
  ...
  if (!current_deleted_edges.empty()) {
    // PR7b-ii: both heavy and light edges defer to deleted_edges_ here. Light
    // edges must NOT be pushed to the light_edge_graveyard_ at commit time: doing
    // so snaps guard_epoch = CurrentEpoch() before any post-commit index iterable
    // can open (so guard_epoch == 0), which lets DrainLightEdgeGraveyard free the
    // Edge* out from under a live iterable (latent UAF). Instead light edges ride
    // deleted_edges_ into CollectGarbage, which snaps the graveyard watermark at
    // GC-collection time (storage.cpp light arm after RemoveObsoleteEdgeEntries),
    // mirroring how heavy edges only acquire their reclamation watermark when the
    // skiplist node is actually collected. Heavy behaviour is unchanged.
  ...
  if (!transaction_.deltas.empty()) {
    ...
    if (!my_deleted_edges.empty()) {
      if (mem_storage->config_.salient.items.storage_light_edge) {
        // PR7b-ii: do NOT push to light_edge_graveyard_ here. Snapping
        // guard_epoch = CurrentEpoch() at abort time happens before any
        // post-commit index iterable opens (guard_epoch == 0), which would let
        // DrainLightEdgeGraveyard free the Edge* under a live iterable (UAF).
        // Heavy edges defer their reclamation watermark to skiplist-collection
        // time (edges_acc.remove below just marks the node; the skiplist GC frees
        // it only once its accessor watermark clears). Light edges have no
        // skiplist, so we mirror that deferral by routing them through
        // deleted_edges_ into CollectGarbage, which snaps the graveyard watermark
        // at GC-collection time after RemoveObsoleteEdgeEntries.
        mem_storage->deleted_edges_.WithLock([&](auto &deleted_edges) {
  ...
  if (!current_deleted_edges.empty()) {
    if (config_.salient.items.storage_light_edge) {
      ...
      light_edge_graveyard_.WithLock([&](auto &graveyard) {
        LightEdgeGraveyardEntry entry{.guard_epoch = guard_epoch, .edges = std::move(current_deleted_edges)};
...
void InMemoryStorage::DrainLightEdgeGraveyard() {
  ...
  // PR7b-ii: light edges may be pinned by edge-index iterables (MakeEdgePin now
  // returns a LightEdgeIterableGuard that Acquires an epoch). A graveyard entry
  // recorded guard_epoch = CurrentEpoch() at delete time; we may only free its
  // edges once IsSafeToFree(guard_epoch) confirms every reader that existed before
  // the delete has Released. Swap the graveyard out under the lock, free the safe
  // entries lock-free, and splice the not-yet-safe survivors back for a later drain.
  //
  // Pushes happen ONLY at GC-collection time (CollectGarbage transactional arm
  // and the analytical arm) — commit (FastDiscard) and abort route deleted
  // Edge* through deleted_edges_ first, so guard_epoch is snapped AFTER index
  // cleanup and after all pre-existing readers are ordered.
...
Edge *InMemoryStorage::LightEdgePool::Create(Gid gid, Delta *delta) noexcept {
  // Edge(Gid, Delta*) is effectively nothrow: PropertyStore() is defaulted
  // (trivial), RWSpinLock() is trivially default, PointerPack is trivial, and
  // MG_ASSERT is [[noreturn]] on failure. This static_assert makes the guarantee
  // explicit so the construct_at outside the try block is safe to leave as-is.
  // Wrapping construct_at inside the try would require catching unknown
  // exceptions and leaking allocated memory or adding a second deallocate path —
  // neither is worthwhile when the ctor cannot throw.
  static_assert(std::is_nothrow_constructible_v<Edge, Gid, Delta *>,
                "Edge(Gid, Delta*) must be nothrow; if this fails, extend the "
                "try/catch in LightEdgePool::Create to cover construct_at and "
...
void InMemoryStorage::Clear() {
  ...
  schema_info_.Clear();
  // PR7b-ii leak fix (mirrors dtor ordering): a deleted light edge whose delta
  // chain was never GC-unlinked is referenced ONLY by a RECREATE_OBJECT delta
  // in committed_transactions_/waiting_gc_deltas_. The clear() below frees
  // those deltas but NOT the pool-allocated Edge*, and ClearLightEdges only
  // drains {adjacency, graveyard, deleted_edges_} -> the Edge* would leak.
  // Harvest BEFORE ClearLightEdges (and before committed_transactions_->clear()
  // at line ~4855) so the delta chains are still reachable. Gated: heavy Edge*
  // live in edges_ and are freed by the skip-list, not here.
  if (config_.salient.items.storage_light_edge) {
    HarvestDeltaChainOnlyLightEdges();
  }
