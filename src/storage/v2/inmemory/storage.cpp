...
void InMemoryStorage::Clear() {
  ...
  // When creating an accessor, we first lock the main_lock and then create a transaction
  // When creating a tx or committing, we lock the engine lock
  // When gc runs, we hold on to main lock, lock the gc lock and then the engine lock
  // Note commit tries to lock gc lock (but can fail)
  // GC could be running without the main lock
  // Engine lock is needed because of PrepareForNewEpoch
  auto snapshot_lock = std::unique_lock{snapshot_lock_};
  auto main_lock = std::unique_lock{main_lock_};
