...
utils::BasicResult<StorageManipulationError, void> InMemoryStorage::InMemoryAccessor::PeriodicCommit(
  ...
  if (transaction_.deltas.empty() && transaction_.md_deltas.empty()) {
    ...
    // existence and unique constraints need to be checked because deltas will vanish on periodic commit
    // we can't get an error in the end when we don't have any delta
...

    {
      ...
      // we can create this at the end since it's not critical to this transaction
      // LabelIndexAutoCreation();
  ...
  // transaction is still active in this case
  // is_transaction_active_ = false;
...
void InMemoryStorage::InMemoryAccessor::LabelIndexAutoCreation() {
