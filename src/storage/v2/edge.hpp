...
struct Edge {
  ...
  // concurrency contract: `branched()` is a lock-free relaxed-atomic read via
  // `PointerPack::GetRelaxed`. ALL writes to the same packed word -- including `SetDelta`,
  // `SetDeleted`, and `SetBranched` -- go through `std::atomic_ref` (relaxed), so every access to
  // `delta_.storage_` is via an atomic operation and there is no plain-store-vs-atomic-load UB.
  // `lock` remains what serializes concurrent writers against each other; `atomic_ref` is what
  // makes each individual write well-defined from the C++ memory model's perspective.
  // For an Edge specifically, this bit is BEST-EFFORT/
