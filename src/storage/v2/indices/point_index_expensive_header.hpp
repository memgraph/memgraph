...
struct IndexPointWGS2d {
  constexpr explicit IndexPointWGS2d(Point2d point) : rep{point.x(), point.y()} { DMG_ASSERT(IsWGS(point.crs())); }
  using point_type = bg::model::point<double, 2, bg::cs::geographic<bg::degree>>;
  point_type rep;
};
struct IndexPointWGS3d {
  constexpr explicit IndexPointWGS3d(Point3d point) : rep{point.x(), point.y(), point.z()} {
    DMG_ASSERT(IsWGS(point.crs()));
  }
  using point_type = bg::model::point<double, 3, bg::cs::geographic<bg::degree>>;
  point_type rep;
};
struct IndexPointCartesian2d {
  constexpr explicit IndexPointCartesian2d(Point2d point) : rep{point.x(), point.y()} {
...
struct IndexPointCartesian3d {
  constexpr explicit IndexPointCartesian3d(Point3d point) : rep{point.x(), point.y(), point.z()} {
    DMG_ASSERT(IsCartesian(point.crs()));
  }
...
struct Entry {
  ...
  constexpr Entry(Point p, Vertex const *vertex) : p_(p), vertex_(vertex) {}

  constexpr friend bool operator==(Entry const &lhs, Entry const &rhs) {
    if (lhs.vertex_ != rhs.vertex_) return false;
    if (!boost::geometry::equals(lhs.p_, rhs.p_)) return false;
    return true;
  };

  constexpr auto point() const -> point_type const & { return p_.rep; }
  constexpr auto vertex() const -> storage::Vertex const * { return vertex_; }
...
struct bg::index::indexable<memgraph::storage::Entry<IndexPoint>> {
  ...
  constexpr auto operator()(memgraph::storage::Entry<IndexPoint> const &val) const -> result_type const & {
    return val.point();
  }
};
