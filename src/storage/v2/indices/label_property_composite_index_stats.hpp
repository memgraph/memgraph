...
struct LabelPropertyCompositeIndexStats {
  uint64_t count, distinct_values_count;
  double statistic, avg_group_size, avg_degree;
};

static inline std::string ToJson(const LabelPropertyCompositeIndexStats &in) {
  return fmt::format(
...
static inline bool FromJson(const std::string &json, LabelPropertyCompositeIndexStats &out) {
  bool res = true;
