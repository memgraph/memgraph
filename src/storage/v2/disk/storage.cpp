...
//   ro.timestamp = &ts;
//   auto it =
//       std::unique_ptr<rocksdb::Iterator>(storage_->kvstore_->db_->NewIterator(ro,
//       storage_->kvstore_->vertex_chandle));
...
//     return DiskVertexAccessor::Create(&*vertex_it, &transaction_, &storage_->indices_, &storage_->constraints_,
//     config_,
//                                       view);
//   }
...
//   MG_ASSERT(disk_edge_acc, "EdgeAccessor must be from the same storage as the storage accessor when deleting an
//   edge!"); MG_ASSERT(disk_edge_acc->transaction_ == &transaction_,
...
//   auto [src_dest_del_key, dest_src_del_key] = SerializeEdge(from_vertex->gid, to_vertex->gid, edge_type,
//   edge_ref.ptr); edges_to_delete_.emplace_back(src_dest_del_key); edges_to_delete_.emplace_back(dest_src_del_key);
...
//           could_replicate_all_sync_replicas = storage_->AppendToWalDataManipulation(transaction_,
//           *commit_timestamp_);
//         }
