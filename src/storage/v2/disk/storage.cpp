...
void DiskStorage::SetEdgeImportMode(EdgeImportMode edge_import_status) {
  // NOTE (IP-1 coro-prepare-accessor-yield, R4.6): nothing can be parked against a DiskStorage's
  // main_lock_ -- DiskStorage does not override TryAccess, so SupportsParkAcquire() is false and the
  // park path bails before registering. The admit observer still fires on release; it is simply a
  // relaxed load with no waiters to find. Park is InMemory-only for v1.
