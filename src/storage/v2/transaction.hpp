...
struct Transaction {
  ...
  // Commit-lock-narrowing (SI only): iff ts <= snapshot_ts (inclusive; snapshot_ts is the highest
  ...
  // off a bare timestamp). Under the commit-lock-narrowing experiment this is snapshot_ts + 1 (so `ts < bound`
  ...
  // commit-lock-narrowing experiment this is snapshot_ts (which equals last_durable_ts_ — both are read under one
  ...
  // EXPERIMENTAL (commit-lock-narrowing): frozen last-committed-MVCC-ts captured at BEGIN.
  ...
  // EXPERIMENTAL (commit-lock-narrowing): true only for SNAPSHOT_ISOLATION txns when the experiment
