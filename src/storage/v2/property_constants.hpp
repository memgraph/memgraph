...
inline const auto kSmallestNull = PropertyValue();
inline const auto kSmallestBool = PropertyValue(false);
// NOLINTNEXTLINE(bugprone-narrowing-conversions,cppcoreguidelines-narrowing-conversions)
static_assert(-std::numeric_limits<double>::infinity() < std::numeric_limits<int64_t>::min());
inline const auto kSmallestNumber = PropertyValue(-std::numeric_limits<double>::infinity());
...
// not reach one.
inline const auto kSmallestNaN = PropertyValue(std::numeric_limits<double>::quiet_NaN());
inline const auto kSmallestString = PropertyValue("");
inline const auto kSmallestList = PropertyValue(std::vector<PropertyValue>());
inline const auto kSmallestMap = PropertyValue(PropertyValue::map_t{});
inline const auto kSmallestTemporalData =
    PropertyValue(TemporalData{static_cast<TemporalType>(0), std::numeric_limits<int64_t>::min()});
inline const auto kSmallestZonedTemporalData =
...
inline const auto kSmallestEnum = PropertyValue(Enum{EnumTypeId{0}, EnumValueId{0}});
inline const auto kSmallestPoint2d = PropertyValue(Point2d{CoordinateReferenceSystem::WGS84_2d, -180, -90});
inline const auto kSmallestPoint3d =
    PropertyValue(Point3d{CoordinateReferenceSystem::WGS84_3d, -180, -90, -std::numeric_limits<double>::infinity()});
inline const auto kSmallestVectorIndexId = PropertyValue(
...
inline const auto kSmallestProperty = kSmallestMap;
...
/// everything.
inline const auto kLargestProperty = kSmallestNull;
