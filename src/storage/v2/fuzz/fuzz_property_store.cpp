...
bool Validate(PropertyStore const &store, std::map<PropertyId, PropertyValue> const &properties) {
  ...
  for (const auto &[id, value] : properties) {
    if (value.IsNull()) {
      ...
      if (store.HasProperty(id)) return false;
      if (store.PropertySize(id) != 0) return false;
    } else {
    ...
    auto shouldBeEqual = !retrievedValue.IsDouble() || !isnanl(retrievedValue.ValueDouble());
    if (shouldBeEqual) {
      if (retrievedValue != value) return false;
      if (!store.IsPropertyEqual(id, value)) return false;
    } else {
      if (retrievedValue == value) return false;
      if (store.IsPropertyEqual(id, value)) return false;
    }
  ...
  // For all stored ids
  if (!store.HasAllProperties(std::set(property_ids.begin(), property_ids.end()))) return false;
  if (store.ExtractPropertyIds() != property_ids) return false;
...
extern "C" int LLVMFuzzerTestOneInput(std::uint8_t const *data, std::size_t size) {
  ...
  while (true) {
    ...
    if (!Validate(store, properties)) return 1;
  }
}
