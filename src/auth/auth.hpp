...
class Auth final {
 public:
  ...
  /**
   * Authenticates a user using a bearer token, i.e. the identity provider response. Requires an external auth module.
   *
   * @param response
   *
   ...
   */
  std::optional<UserOrRole> Authenticate(const std::string &identity_provider_response);
