...
PermissionLevel FineGrainedAccessPermissions::Has(const std::string &permission,
                                                  const EntityPermission label_permission) const {
...
void FineGrainedAccessPermissions::Grant(const std::string &permission, const EntityPermission label_permission) {
...
void FineGrainedAccessPermissions::Deny(const std::string &permission, const EntityPermission label_permission) {
  if (permission == kAsterisk) {
...
uint64_t FineGrainedAccessPermissions::CalculateGrant(EntityPermission label_permission) {
  uint64_t shift{1};
  uint64_t result{0};
...
uint64_t FineGrainedAccessPermissions::CalculateDeny(EntityPermission label_permission) {
  uint64_t shift{1};
  uint64_t result{0};
  auto uint_label_permission = static_cast<uint64_t>(label_permission);
