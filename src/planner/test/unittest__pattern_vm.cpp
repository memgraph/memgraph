...
// ============================================================================
// MatcherIndex vs VM Executor Consistency
// ============================================================================
//
// These tests verify that MatcherIndex and VM executor produce identical results.
...
TEST_F(MatcherIndexTest, LeafSymbolChildWithMergedENodesConsistency) {
  ...
  //
  // This tests the bug found by the fuzzer where MatcherIndex was producing
  ...
  //
  //   Expected: MatcherIndex and VM produce same number of matches
...
TEST_F(MatcherIndexTest, MultiPatternVMFiltersBySymbolInVerifyMode) {
  ...
  //
  //   Old bug: VM found 1 match (wrong), MatcherIndex found 0 (correct)
...
TEST_F(MatcherIndexTest, DuplicateBindingsFromDifferentENodes) {
  ...
  //
  // Both MatcherIndex and VM find 2 matches because the full tuples differ (different roots).
  ...
  for (auto const &m : matches) {
    ...
    for (size_t i = 0; i < pattern().num_vars(); ++i) {
      tuple.push_back(egraph.find(bindings[i]));
    }
...
TEST_F(MatcherIndexTest, RepeatedVarInNestedPatternWithSelfReferentialEClass) {
  ...
  // Egglog ground truth: 3 matches. Bug: MatcherIndex/VM find 4.
  //
