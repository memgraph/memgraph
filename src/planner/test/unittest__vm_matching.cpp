...
{Multi | Nesting | Merge | SelfLoop, "MatchesSelfRefNoRoot", [](auto &t) -> MatchingExpected {
  // Same graph as MatchesSelfRef: EC1 = {F(A), F(EC1)}
  auto a = t.leaf(Op::A);
  auto f_a = t.node(Op::F, a);
  auto f_f_a = t.node(Op::F, f_a);
  t.merge(f_a, f_f_a);
  t.use_patterns(TestPattern::build(Op::F, {Sym(Op::F, Var{kVarX})}));
  return {{{{kVarX, a}}, {{kVarX, f_a}}}};
}},
...
// Parent walk with a symbol child whose eclass has multiple matching enodes.
// The pattern enters at H (deepest symbol), walks up to the root Plus, then
// iterates Plus enodes in the sibling eclass. Both enodes must produce matches.
//
...
TEST_F(PatternVM_Matching, ParentWalk_SymbolChildMultipleEnodes) {
  ...
  auto b = leaf(Op::B);
  auto c = leaf(Op::C);
  auto h_a = node(Op::H, a);
  auto plus_bc = node(Op::Plus, b, c);
  auto plus_bb = node(Op::Plus, b, b);
  merge(plus_bc, plus_bb);       // eclass p has 2 Plus enodes
  node(Op::Plus, h_a, plus_bc);  // Plus(H(a), p)

  rebuild_egraph();
  rebuild_index();

  use_patterns(TestPattern::build(Op::Plus, {Sym(Op::H, Var{kVarX}), Sym(Op::Plus, Var{kVarY}, Var{kVarZ})}));

  run_compiled();

  auto f = [&](EClassId id) { return egraph.find(id); };
  verify({
