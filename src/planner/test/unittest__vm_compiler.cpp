...
static auto const kCompilerCases = std::vector<CompilerTestCase>{
    ...
    // --- InlineRule variants: compare Symbol() constraint placement ---
    {"Inline_SeparateSymbol",                                                    // Bind(_,?x,?y), ?z=Ident(?x), ?x=A()
     [] {
       return std::vector{
           TestPattern::build(Op::Bind, {Wildcard{}, Var{kVarX}, Var{kVarY}}),
           TestPattern::build(kVarZ, Op::Ident, {Var{kVarX}}),
           TestPattern::build(kVarX, Op::A)};
     }},
    {"Inline_InBind",                                                            // Bind(_,?x=A(),?y), ?z=Ident(?x)
     [] {
       return std::vector{
           TestPattern::build(Op::Bind, {Wildcard{}, BoundSym(kVarX, Op::A), Var{kVarY}}),
           TestPattern::build(kVarZ, Op::Ident, {Var{kVarX}})};
     }},
    {"Inline_InIdent",                                                           // Bind(_,?x,?y), ?z=Ident(?x=A())
     [] {
       return std::vector{
           TestPattern::build(Op::Bind, {Wildcard{}, Var{kVarX}, Var{kVarY}}),
           TestPattern::build(kVarZ, Op::Ident, {BoundSym(kVarX, Op::A)})};
     }},
...
TEST_P(Compile, Verify) {
  ...
  VerifyBytecode(compiled.code(), compiled.symbols());
}
