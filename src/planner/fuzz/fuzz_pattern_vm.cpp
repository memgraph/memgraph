...
bool g_skip_egglog = false;  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)
}  // anonymous namespace
...
auto run_egglog(std::string_view program) -> EgglogResult {
  ...
  std::string const cmd = fmt::format("egglog {} 2>&1", temp_path.string());
  ...
  if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
    ...
    while (std::getline(iss, line)) {
      ...
      if (pos != std::string::npos) {
        ...
        if (trimmed.starts_with("(MatchResult")) {
          result.match_count++;
        }
      }
    }
...
class FuzzerState {
 public:
  bool execute_operation(uint8_t op, uint8_t const *data, size_t &pos, size_t size) {
    ...
    int const operation_type = kDispatch[op % kDispatch.size()];
  ...
  bool create_leaf_node(uint8_t const *data, size_t &pos, size_t size) {
    ...
    uint8_t const symbol = data[pos++] % 5;  // A-E
  ...
  bool set_pattern(uint8_t const *data, size_t &pos, size_t size) {
    ...
    // value, making the query more restrictive than the MatcherIndex.
    int leaf_counter = 0;
  ...
  bool match_and_verify() {
    ...
    patterns.reserve(current_patterns_ast_.size());
    for (auto const &ast : current_patterns_ast_) {
    ...
    auto parse_var_id = [](std::string_view s) -> uint8_t {
      uint8_t id = 0;
      std::from_chars(s.data() + 2, s.data() + s.size(), id);  // skip "?v"
      return id;
    };

    std::vector<uint8_t> var_ids;
    ...
    bool vm_compilation_succeeded = false;
    {
      ...
      try {
        ...
        VERBOSE_OUT << "Pattern too complex (register overflow), skipping\n";
        return true;
      }
    }
    ...
    std::size_t const vm_count = vm_unique.size();
    ...
    size_t const egglog_count = egglog_result.match_count;
    ...
    // caught here.
    bool const vm_ok = (vm_count == egglog_count);
  ...
  bool apply_rewrite_cycle(uint8_t const *data, size_t &pos, size_t size) {
    ...
    uint8_t const shape = (pos < size) ? data[pos++] % 6 : 0;
    ...
    std::vector<EClassId> match_roots;
    {
      ...
      try {
        ...
        VERBOSE_OUT << "APPLY_REWRITE: pattern too complex (register overflow), skipping\n";
        return true;
      }
    }
    ...
    for (auto root_id : match_roots) {
      ...
      auto root_name = egglog_names_.contains(canonical) ? egglog_names_[canonical] : egglog_names_[root_id];
  ...
  bool verify_absent() {
    ...
    for (size_t i = 0; i < arity; ++i) {
      children.push_back(builder.var(PatternVar{static_cast<uint8_t>(i)}));
    }
    ...
    std::size_t match_count = 0;
    {
      ...
      try {
        ...
        VERBOSE_OUT << "VERIFY_ABSENT: pattern too complex (register overflow), skipping\n";
        return true;
      }
    }
 ...
 private:
  ...
  std::set<uint8_t> current_shared_vars_;
