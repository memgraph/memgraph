...
// Simple test symbols for fuzzing (uses uint8_t for this fuzzer)
...
bool ValidateCanonicalClasses(EGraph<Symbol, Analysis> &egraph) {
  for (auto id : egraph.canonical_eclass_ids()) {
    if (egraph.find(id) != id) {
      std::cerr << "Non-canonical class ID " << id << " in canonical_eclass_ids()\n";
      return false;
    }
  }
  return true;
...
bool ValidateCongruenceClosure(EGraph<Symbol, Analysis> &egraph) {
  ...
  for (auto const &[class_id, eclass] : egraph.canonical_classes()) {
    for (auto enode_id : eclass.nodes()) {
      std::string const sig = CreateCanonicalSignature(egraph.get_enode(enode_id), egraph);
...
class FuzzerState {
 public:
  bool execute_operation(uint8_t op, const uint8_t *data, size_t &pos, size_t size) {
    ...
    int const operation_type = op % 5;
    static constexpr std::array op_names = {
        "CREATE_LEAF", "CREATE_COMPOUND", "MERGE_CLASSES", "REBUILD", "CREATE_CONGRUENT"};
  ...
  bool create_leaf_node(const uint8_t *data, size_t &pos, size_t size) {
    ...
    uint64_t const disambiguator = pos < size ? data[pos++] : 0;
  ...
  bool create_compound_node(const uint8_t *data, size_t &pos, size_t size) {
    ...
    uint8_t const symbol = (data[pos++] % 5) + 10;       // F-Mul
    uint8_t const num_children = (data[pos++] % 3) + 1;  // 1-3 children

    if (pos + num_children > size) return true;

    std::vector<EClassId> children;
    for (uint8_t i = 0; i < num_children; ++i) {
      uint8_t const child_idx = data[pos++] % created_ids.size();
  ...
  bool merge_classes(const uint8_t *data, size_t &pos, size_t size) {
    ...
    uint8_t const idx1 = data[pos++] % created_ids.size();
    uint8_t const idx2 = data[pos++] % created_ids.size();
  ...
  bool create_congruent_pattern(const uint8_t *data, size_t &pos, size_t size) {
    ...
    uint8_t const symbol_a = data[pos++] % 5;                         // A-E
    uint8_t const symbol_b = (symbol_a + 1 + (data[pos++] % 4)) % 5;  // Different from symbol_a
    ...
    if (pos < size) {
      uint8_t const compound_symbol = 10 + (data[pos++] % 5);  // F-Mul
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  size_t const classes_before = state.egraph.num_classes();
  state.egraph.rebuild(state.ctx);
