...
// Variables for eclass-level hoisting benchmarks
constexpr PatternVar kHoistR{30};  // ?r = F(?x) root binding
constexpr PatternVar kHoistY{31};  // Mul(?r, ?y) sibling binding
...
// ComplexGraph: Realistic e-graph structure for benchmarking.
...
// EclassHoist: Benchmark for eclass-level hoisting optimization.
//
// Creates eclasses with multiple F enodes (via merges) and Mul parents.
...
//
// Parameters:
//   num_eclasses:     number of distinct eclasses containing F enodes
//   enodes_per_class: F enodes per eclass (controls merge fan-in / redundancy)
//   parents:          Mul parents per F eclass (controls parent traversal cost)
//
...
inline void BuildEclassHoistGraph(TestEGraph &g, int64_t num_eclasses, int64_t enodes_per_class, int64_t parents) {
  ...
  for (int64_t ec = 0; ec < num_eclasses; ++ec) {
    // Create the first F enode: F(leaf_0)
    auto leaf0 = g.emplace(Op::Const, static_cast<uint64_t>(ec * 1000)).eclass_id;
    auto first_f = g.emplace(Op::F, {leaf0}).eclass_id;

    // Merge additional F enodes into the same eclass.
    // Each F(leaf_i) is a distinct enode but they all end up in one eclass.
    ...
    // Create distinct Mul parents: Mul(f_eclass, unique_leaf).
    // Binary op with a unique second child avoids deduplication.
...
// Eclass-level hoisting benchmark patterns: ?r=F(?x) + Mul(?r, ?y)
// With hoisting, Mul(?r, ?y) parent traversal runs once per eclass.
// Without hoisting, it runs once per F enode (redundant).
// Uses ?y (not wildcard) so the sibling child gets LoadChild + BindSlot,
// making each parent traversal do real work.
...
// ============================================================================
// Extern Template Declarations (bench types)
// ============================================================================
// Suppress implicit instantiation in each TU. Explicit instantiation
// definitions live in bench_instantiations.cpp.
