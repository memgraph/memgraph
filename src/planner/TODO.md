...
## ProcessCosts: Fragile Caching Side Effect

In `extractor.hpp` `ProcessCosts`, the inner loop over an enode's children intentionally
does NOT break when a child returns nullopt (cycle). This is because the recursive calls
have a side effect of populating `enode_selection` for reachable eclasses — other extraction
paths depend on those cached results.

This is fragile: the loop looks like it's just deciding whether to skip this enode, so adding
a `break` on nullopt is a natural "optimisation" that silently breaks extraction for cyclic
graphs.

**Fix**: Split into two phases — first recursively populate `enode_selection` for all children
of all enodes in the eclass, then do a second pass to select the cheapest enode using only
cached lookups. The second pass can safely `break` on missing costs.

**Location**: `extractor.hpp` `ProcessCosts`

## Single-Phase Extraction

Explore whether cost extraction can be done in a single BFS/topo pass instead of the
current three-phase approach (ProcessCosts → CollectDependencies → TopologicalSort).

Sketch: start from root, BFS to discover eclasses. Track per-enode counters (number of
unprocessed children). When all children of an enode are processed, compute its cost and
check if it's the best for its eclass. When all enodes in an eclass are processed, propagate
to parent enodes. Needs cycle handling.

**Location**: `extractor.hpp`
