...
class RuleSet {
 public:
  using RulePtr = std::shared_ptr<RewriteRule<Symbol, Analysis, Graph> const>;

  class Builder {
   public:
    auto add_rule(RewriteRule<Symbol, Analysis, Graph> rule) -> Builder & {
      rules_.push_back(std::make_shared<RewriteRule<Symbol, Analysis, Graph> const>(std::move(rule)));
      return *this;
    }
  ...
  static auto Build(Rules &&...rules) -> RuleSet {
    ...
    (rule_vec.push_back(std::make_shared<RewriteRule<Symbol, Analysis, Graph> const>(std::forward<Rules>(rules))), ...);
