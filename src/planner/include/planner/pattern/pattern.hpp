...
/// DSL types for fluent pattern construction
namespace dsl {
  ...
  class Builder {
   public:
    ...
    auto sym(Symbol symbol, std::optional<PatternVar> binding = std::nullopt) -> PatternNodeId {
      auto id = PatternNodeId{static_cast<uint32_t>(nodes_.size())};
      nodes_.emplace_back(SymbolWithChildren<Symbol>{symbol, {}});
      if (binding) {
    ...
    auto sym(Symbol symbol, utils::small_vector<PatternNodeId> children,
      ...
      auto id = PatternNodeId{static_cast<uint32_t>(nodes_.size())};
      nodes_.emplace_back(SymbolWithChildren<Symbol>{symbol, std::move(children)});
      if (binding) {
        bindings_[id] = *binding;
      }
      return id;
     ...
     */
    auto sym(Symbol symbol, std::initializer_list<dsl::ChildSpec<Symbol>> children,
             std::optional<PatternVar> binding = std::nullopt) -> PatternNodeId {
      utils::small_vector<PatternNodeId> child_ids;
      for (auto const &spec : children) {
        child_ids.emplace_back(build_child(spec));
      }
   ...
   private:
     ...
     */
    auto build_child(dsl::ChildSpec<Symbol> const &spec) -> PatternNodeId {
      return std::visit(
          [this](auto const &s) -> PatternNodeId {
            ...
            if constexpr (std::is_same_v<T, PatternNodeId>) {
              ...
              return wildcard();
            } else if constexpr (std::is_same_v<T, dsl::Var>) {
              return var(s.pvar);
            } else if constexpr (std::is_same_v<T, std::shared_ptr<dsl::SymNode<Symbol>>>) {
              // Recursively build nested symbol
              utils::small_vector<PatternNodeId> child_ids;
              for (auto const &child : s->children) {
                child_ids.emplace_back(build_child(child));
              }
