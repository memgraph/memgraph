...
class VMExecutor {
 ...
 private:
  ...
  [[gnu::always_inline]] void exec_mark_seen(Instruction instr) {
    state_.mark_seen(instr.arg);
    if constexpr (DevMode) {
      collector_.on_mark_seen(state_.pc, instr.arg, state_.slots[instr.arg]);
    }
  }
...
auto VMExecutor<Symbol, Analysis, DevMode>::exec_iter_parents(Instruction instr) -> bool {
  ...
  state_.start_parent_iter(instr.dst, parents);
...
auto VMExecutor<Symbol, Analysis, DevMode>::exec_check_symbol(Instruction instr) -> bool {
  ...
  if (enode.symbol() != symbols_[instr.arg]) {
    if constexpr (DevMode) {
      collector_.on_check_symbol_miss(state_.pc, state_.enode_regs[instr.src], instr.arg);
    }
    return false;
  }
  if constexpr (DevMode) {
    collector_.on_check_symbol_hit(state_.pc, state_.enode_regs[instr.src], instr.arg);
  }
  return true;
}
...
auto VMExecutor<Symbol, Analysis, DevMode>::exec_check_arity(Instruction instr) -> bool {
  ...
  if (enode.arity() != instr.arg) {
    if constexpr (DevMode) {
      collector_.on_check_arity_fail(state_.pc, state_.enode_regs[instr.src], instr.arg, enode.arity());
    }
    return false;
  }
  if constexpr (DevMode) {
    collector_.on_check_arity_pass(state_.pc, state_.enode_regs[instr.src], instr.arg);
  }
  return true;
}
...
auto VMExecutor<Symbol, Analysis, DevMode>::exec_bind_slot(Instruction instr) -> bool {
  ...
  if constexpr (DevMode) {
    if (is_new) {
      collector_.on_bind(state_.pc, instr.arg, eclass);
    } else {
      collector_.on_bind_duplicate(state_.pc, instr.arg, eclass);
    }
  }
  return is_new;
}
...
auto VMExecutor<Symbol, Analysis, DevMode>::exec_check_slot(Instruction instr) -> bool {
  ...
  if (state_.slots[instr.arg] != state_.eclass_regs[instr.src]) {
    if constexpr (DevMode) {
      collector_.on_check_slot_miss(state_.pc, instr.arg, state_.slots[instr.arg], state_.eclass_regs[instr.src]);
    }
    return false;
  }
  if constexpr (DevMode) {
    collector_.on_check_slot_hit(state_.pc, instr.arg, state_.slots[instr.arg]);
  }
  return true;
}
...
auto VMExecutor<Symbol, Analysis, DevMode>::exec_check_eclass_eq(Instruction instr) -> bool {
  ...
  if (state_.eclass_regs[instr.dst] != state_.eclass_regs[instr.src]) {
    if constexpr (DevMode) {
      collector_.on_check_eclass_eq_fail(state_.pc, state_.eclass_regs[instr.dst], state_.eclass_regs[instr.src]);
    }
    return false;
  }
  if constexpr (DevMode) {
    collector_.on_check_eclass_eq_pass(state_.pc, state_.eclass_regs[instr.dst]);
  }
  return true;
}
...
void VMExecutor<Symbol, Analysis, DevMode>::exec_yield(Instruction instr, MatchArena &arena,
  ...
  if constexpr (DevMode) {
    collector_.on_mark_seen(state_.pc, instr.arg, state_.slots[instr.arg]);
    collector_.on_yield(state_.pc, state_.slots);
  }
