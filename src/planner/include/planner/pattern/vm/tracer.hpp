...
struct RecordingTracer final : VMTracer {
  ...
  void on_bind(std::size_t slot, EClassId value) override {
    events.push_back({Event::Type::Bind, 0, std::format("slot={} value={}", slot, value.value_of())});
  }
  ...
  void on_yield(std::span<EClassId const> slots) override {
    ...
    for (std::size_t i = 0; i < slots.size(); ++i) {
      if (i > 0) result += ", ";
      std::format_to(std::back_inserter(result), "{}", slots[i].value_of());
    }
    result += ']';
    events.push_back({Event::Type::Yield, 0, std::move(result)});
  }

  void on_halt(std::size_t total_instructions) override {
    events.push_back({Event::Type::Halt, 0, std::format("total={}", total_instructions)});
  }
...
struct VMCollector {
  ...
  void on_check_eclass_eq_fail(std::size_t pc) {
    if (tracer) tracer->on_check_fail(pc, "eclass mismatch");
  }
...
auto disassemble(std::span<Instruction const> code, std::span<Symbol const> symbols) -> std::string {
  ...
  for (std::size_t i = 0; i < code.size(); ++i) {
    ...
    switch (instr.op) {
      case VMOp::LoadChild:
        std::format_to(out, " r{}, r{}, {}", instr.dst, instr.src, instr.arg);
        break;

      case VMOp::GetENodeEClass:
        std::format_to(out, " r{}, r{}", instr.dst, instr.src);
        break;

      case VMOp::IterENodes:
      case VMOp::IterParents:
        std::format_to(out, " r{}, r{}, @{}", instr.dst, instr.src, instr.target);
        break;
