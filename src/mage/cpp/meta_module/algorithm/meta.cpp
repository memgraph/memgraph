...
void OutputFromMetadata(Metadata &data, const mgp::RecordFactory &record_factory, bool online) {
  ...
  const int64_t label_count = LockAndExecuteMaybeReturn(
      metadata.labels_mutex, online, [&]() { return static_cast<int64_t>(data.labels.size()); });
  ...
  auto relationship_types_map = LockAndExecuteMaybeReturn(
      metadata.relationship_types_mutex, online, [&]() { return create_map(data.relationship_types); });
