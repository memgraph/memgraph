...
utfchar32_t next16(word_iterator &it) {
  ...
  if (utf8::internal::is_lead_surrogate(cp)) return (cp << 10) + *it++ + utf8::internal::SURROGATE_OFFSET;
  return cp;
}
...
void advance(octet_iterator &it, distance_type n) {
  ...
  if (n < zero) {
    // backward
    for (distance_type i = n; i < zero; ++i) utf8::unchecked::prior(it);
  } else {
    // forward
    for (distance_type i = zero; i < n; ++i) utf8::unchecked::next(it);
  }
}
