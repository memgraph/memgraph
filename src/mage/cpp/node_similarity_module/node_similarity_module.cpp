...
void JaccardPairwise(mgp_list *args, mgp_graph * /*memgraph_graph*/, mgp_result *result, mgp_memory *memory) {
  ...
  try {
    insert_results(node_similarity_algs::CalculateSimilarityPairwise(
                       arguments[0].ValueList(), arguments[1].ValueList(), node_similarity_util::Similarity::jaccard),
                   record_factory);
...
void OverlapPairwise(mgp_list *args, mgp_graph * /*memgraph_graph*/, mgp_result *result, mgp_memory *memory) {
  ...
  try {
    insert_results(node_similarity_algs::CalculateSimilarityPairwise(
                       arguments[0].ValueList(), arguments[1].ValueList(), node_similarity_util::Similarity::overlap),
                   record_factory);
  } catch (const mgp::ValueException &e) {
    record_factory.SetErrorMessage(e.what());
  }
...
void CosinePairwise(mgp_list *args, mgp_graph * /*memgraph_graph*/, mgp_result *result, mgp_memory *memory) {
  ...
  try {
    const std::string prop_str(arguments[0].ValueString());
    insert_results(
        node_similarity_algs::CalculateSimilarityPairwise(
            arguments[1].ValueList(), arguments[2].ValueList(), node_similarity_util::Similarity::cosine, prop_str),
        record_factory);
  } catch (const mgp::ValueException &e) {
    record_factory.SetErrorMessage(e.what());
  }
...
extern "C" int mgp_init_module(struct mgp_module *module, struct mgp_memory *memory) {
  try {
    ...
    const std::vector<mgp::Parameter> cosine_params_pairwise{
        mgp::Parameter(prop_vector, mgp::Type::String),
        mgp::Parameter(src_nodes, {mgp::Type::List, mgp::Type::Node}),
        mgp::Parameter(dst_nodes, {mgp::Type::List, mgp::Type::Node})};
    ...
    mgp::AddProcedure(Cosine,
                      ...
                      memory);
    mgp::AddProcedure(
        CosinePairwise, cosinePairwise, mgp::ProcedureType::Read, cosine_params_pairwise, returns, module, memory);
