    ...
    if (!algorithm.Initialized()) {
      ...
      if (created_nodes.empty() && deleted_nodes.empty()) {  // Edge update
        ...
        for (const auto created_edge : created_edges) {
          ...
          node_bc_scores = algorithm.EdgeUpdate(
              *prior_graph, *graph, online_bc::Operation::CREATE_EDGE, created_edge, normalize, threads);
        ...
        for (const auto deleted_edge : deleted_edges) {
          ...
          graph = mg_generate::BuildGraph(graph_nodes_ids, prior_edges_ids, mg_graph::GraphType::kUndirectedGraph);
          node_bc_scores = algorithm.EdgeUpdate(
              *prior_graph, *graph, online_bc::Operation::DELETE_EDGE, deleted_edge, normalize, threads);
          prior_graph = std::move(graph);
        }
        ...
        algorithm.NodeEdgeUpdate(
            *graph, online_bc::Operation::CREATE_ATTACH_NODE, created_nodes[0], created_edges[0], normalize);
        ...
        algorithm.NodeEdgeUpdate(
            *graph, online_bc::Operation::DETACH_DELETE_NODE, deleted_nodes[0], deleted_edges[0], normalize);
  ...
  // Add .update()
  {
    try {
      ...
      mgp::proc_add_opt_arg(
          update_proc, kArgumentCreatedEdges, mgp::type_list(mgp::type_relationship()), default_edges);
      mgp::proc_add_opt_arg(update_proc, kArgumentDeletedVertices, mgp::type_list(mgp::type_node()), default_vertices);
      mgp::proc_add_opt_arg(
          update_proc, kArgumentDeletedEdges, mgp::type_list(mgp::type_relationship()), default_edges);
