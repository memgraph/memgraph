...
std::vector<std::uint64_t> OnlineBC::MergeBFSOrders(
  ...
  std::ranges::copy_if(initial_order,
                       std::back_inserter(initial_order_no_overlap),
  ...
  std::ranges::merge(
      initial_order_no_overlap, partial_order, std::back_inserter(merged_order), longest_distances_first);
...
void OnlineBC::iCentralIteration(const mg_graph::GraphView<> &graph, const Operation operation,
  ...
  const auto [n_shortest_paths_with_edge,
              distances_with_edge,
              predecessors_with_edge,
              partial_reverse_bfs_order_with_edge] = PartialBFS(graph,
                                                                updated_edge,
                                                                affected_bcc_nodes,
                                                                s_id,
                                                                n_shortest_paths_no_edge,
                                                                distances_no_edge,
                                                                predecessors_no_edge);
  const auto reverse_bfs_order_with_edge = MergeBFSOrders(
      reverse_bfs_order_no_edge, distances_no_edge, partial_reverse_bfs_order_with_edge, distances_with_edge);
...
#pragma omp parallel for default(none) shared(array_size,                           \
                                                  affected_bcc_nodes_array,         \
                                                  distances_first,                  \
                                                  distances_second,                 \
                                                  graph_without_updated_edge,       \
                                                  operation,                        \
                                                  affected_bcc_nodes,               \
                                                  affected_bcc_articulation_points, \
                                                  updated_edge,                     \
                                                  peripheral_subgraph_orders,       \
                                                  this_ptr)
  for (std::uint64_t i = 0; i < array_size; i++) {
    ...
    if (distances_first.at(node_id) != distances_second.at(node_id)) {
      this_ptr->iCentralIteration(graph_without_updated_edge,
                                  operation,
                                  node_id,
                                  affected_bcc_nodes,
                                  affected_bcc_articulation_points,
