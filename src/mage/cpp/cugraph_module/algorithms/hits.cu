...
void InsertHitsRecord(mgp_graph *graph, mgp_result *result, mgp_memory *memory, const std::uint64_t node_id, double hub,
                      double authority) {
...
void HitsProc(mgp_list *args, mgp_graph *graph, mgp_result *result, mgp_memory *memory) {
  try {
    ...
    auto [hub_diff, iterations] = cugraph::hits<vertex_t, edge_t, result_t, false>(handle,
                                                                                   cu_graph_view,
                                                                                   hubs.data(),
                                                                                   authorities.data(),
    ...
    for (vertex_t node_id = 0; node_id < static_cast<vertex_t>(n_vertices); ++node_id) {
      ...
      InsertHitsRecord(
          graph, result, memory, mg_graph->GetMemgraphNodeId(original_id), h_hubs[node_id], h_authorities[node_id]);
    }
