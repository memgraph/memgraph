...
void SpectralClusteringProc(mgp_list *args, mgp_graph *graph, mgp_result *result, mgp_memory *memory) {
  try {
    ...
    auto mg_graph = mg_utility::GetWeightedGraphView(
        graph, result, memory, mg_graph::GraphType::kUndirectedGraph, weight_property, kDefaultWeight);
    ...
    cugraph::ext_raft::spectralModularityMaximization(handle,
                                                      ...
                                                      num_clusters,
                                                      num_eigenvectors,
                                                      static_cast<weight_t>(ev_tolerance),
                                                      ev_maxiter,
                                                      static_cast<weight_t>(kmean_tolerance),
                                                      kmean_maxiter,
    ...
    for (vertex_t node_id = 0; node_id < static_cast<vertex_t>(n_vertices); ++node_id) {
      InsertSpectralClusteringResult(
          graph, result, memory, mg_graph->GetMemgraphNodeId(node_id), h_clustering[node_id]);
    }
...
extern "C" int mgp_init_module(struct mgp_module *module, struct mgp_memory *memory) {
  ...
  try {
    auto *spectral_proc = mgp::module_add_read_procedure(module, kProcedureSpectralClustering, SpectralClusteringProc);
