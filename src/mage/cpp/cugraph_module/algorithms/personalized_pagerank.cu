...
void PersonalizedPagerankProc(mgp_list *args, mgp_graph *graph, mgp_result *result, mgp_memory *memory) {
  try {
    ...
    raft::update_device(
        d_pers_vertices.data(), h_personalization_vertices.data(), h_personalization_vertices.size(), stream);
    ...
    auto personalization = std::make_optional(
        std::make_tuple(raft::device_span<vertex_t const>(d_pers_vertices.data(), d_pers_vertices.size()),
