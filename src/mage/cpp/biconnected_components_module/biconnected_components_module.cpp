  ...
  try {
    ...
    for (std::uint64_t bcc_id = 0; bcc_id < bccs.size(); bcc_id++) {
      for (const auto &edge : bccs[bcc_id]) {
        InsertBiconnectedComponentRecord(memgraph_graph,
                                         result,
                                         memory,
                                         static_cast<int>(bcc_id),
                                         static_cast<int>(edge.id),
                                         static_cast<int>(graph->GetMemgraphNodeId(edge.from)),
                                         static_cast<int>(graph->GetMemgraphNodeId(edge.to)));
      }
    }
