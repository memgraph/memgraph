#!/bin/bash
# Build toolchain v9 inside a fresh ubuntu:20.04 container and drop the
...
IMAGE=mg-toolchain-v9-builder
CONTAINER=mg-toolchain-v9-build
...
if [[ "$(arch)" == "aarch64" ]]; then
  docker exec -w /workspace/memgraph "$CONTAINER" ./environment/toolchain/v9/build.sh --for-arm
else
  docker exec -w /workspace/memgraph "$CONTAINER" ./environment/toolchain/v9/build.sh
fi

echo "==> Copying output back to $OUTPUT_DIR"
mkdir -p "$OUTPUT_DIR"
docker cp \
    "$CONTAINER:/workspace/memgraph/environment/toolchain/v9/output/." \
    "$OUTPUT_DIR/"
