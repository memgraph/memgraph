...
if [[ "$SKIP_CHECK" == false ]]; then
    check_operating_system "ubuntu-26.04"
    check_architecture "x86_64"
else
    echo "Skipping checks for ubuntu-26.04"
fi
...
TOOLCHAIN_BUILD_DEPS=(
    ...
    libreadline-dev libsqlite3-dev lzma tk-dev uuid-dev # for building python
...
check() {
    ...
    if [ ${#standard_packages[@]} -gt 0 ]; then
        missing=$(python3 "$DIR/check-packages.py" "check" "ubuntu-26.04" "${standard_packages[@]}")
    fi
...
install() {
    ...
    if [ ${#standard_packages[@]} -gt 0 ]; then
        if ! python3 "$DIR/check-packages.py" "install" "ubuntu-26.04" "${standard_packages[@]}"; then
