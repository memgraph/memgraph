...
if [[ "$SKIP_CHECK" == false ]]; then
    check_operating_system "fedora-43"
    check_architecture "x86_64"
else
    echo "Skipping checks for fedora-43"
fi
...
check() {
    ...
    if [ ${#standard_packages[@]} -gt 0 ]; then
        missing=$(python3 "$DIR/check-packages.py" "check" "fedora-43" "${standard_packages[@]}")
    fi
...
install() {
    ...
    if [ ${#standard_packages[@]} -gt 0 ]; then
        if ! python3 "$DIR/check-packages.py" "install" "fedora-43" "${standard_packages[@]}"; then
