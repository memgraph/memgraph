...
if [ $include_toolchain_deps == 1 ]; then
    apt install -y bison automake libpcre3-dev libedit-dev libxml2-dev \
        libffi-dev libreadline-dev libcurl4-openssl-dev texinfo python3-dev \
        liblzma-dev libbabeltrace-ctf-dev libbabeltrace-dev libipt-dev \
        libexpat1-dev zlib1g-dev make build-essential g++ gcc
fi
