...
# `build/` on the host is usually created as a side effect of the memgraph
# compile, but MAGE now skips that step (it downloads the prebuilt deb
# instead — see reusable_package_mage.yaml). Ensure the dir exists so the
# docker cp lines below don't fail when this is the first thing to write
# into build/.
mkdir -p build
