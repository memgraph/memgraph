...
`RecoverData` call tree, so any corruption type produces a broken tenant rather
than a crash:
...
- [ ] With the flag on, each listed corruption type results in a broken tenant and a successful boot.
...
- [ ] **Unit (byte-flip):** copy a real WAL file; for each byte offset, flip the byte, attempt recovery, and assert corruption is detected — with the flag on it yields a broken/empty storage (no crash); pattern per the helper in the spec's Testing Decisions.
- [ ] **Unit:** a missing prefix WAL file (no snapshot) and a WAL sequence-number gap each produce a broken tenant with the flag on, and remain fatal with the flag off.
