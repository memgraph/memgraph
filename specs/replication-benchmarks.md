...
| 16 | Log level | Originally `WARNING`, not the `TRACE` the e2e HA tests use, because logging on the commit path competes with what is being measured. **Amended during implementation**: switched to `TRACE` on every instance while the suite is being brought up, deliberately trading measurement accuracy for the ability to see what the cluster is doing. Numbers taken this way diagnose the harness; they are not publishable, and the level goes back to `WARNING` before any result is trusted |
...
- **TRACE logging is on, so current numbers are not measurements.** Decision 16 was reversed while the
  suite is brought up. Every commit is logged on five instances, which costs throughput and latency on
  exactly the path being measured and writes a large volume into `ha_logs/` — the pokec import alone is
  on the order of a million commits. Both are acceptable for diagnosis and unacceptable for a result.
