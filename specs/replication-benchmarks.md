...
| 10 | Workloads | `pokec/medium`, targeting **5 distinct write shapes plus 1 read as a control**: `create/pattern`, `create/vertex_big`, `arango/single_vertex_write`, `arango/single_edge_write`, `basic/single_vertex_property_update_update`, and `arango/single_vertex_read` as the control. 6 queries, no new workload code. `arango/unwind_range_vertex_write` was dropped too, which leaves every remaining write a single-entity commit. **Amended after the first cluster run**: `create/vertex` and `create/edge` were dropped and the `create` group is now named query by query. They duplicate `arango/single_vertex_write` and `arango/single_edge_write` — differing only by a label, a property and a `RETURN`, and by `WITH`/`RETURN` respectively — and measured within 12% and 2% of them, so each was paying a cluster restart to re-measure the same shape. Each write is one autocommit transaction, so each pays exactly one SYNC round trip to every replica — the unit of cost being measured. `unwind_range_vertex_write` is the deliberate exception at 100 nodes per commit, the only point where the delta batch is large enough to show bandwidth cost rather than round-trip cost. **Amended during implementation**: this was originally `create/*` + `update/*` for writes with all of `match/*` as control. Enumerating the queries showed that pokec contains exactly 11 writes, that the published dashboard series come from the `arango`, `create` and `match` groups, and that group-level filtering therefore both missed arango's three writes and dragged in 25 reads. Reads are held to two on purpose: they are only a harness sanity check, and every query costs its own cluster restart |
...
- **Coordinator chatter is inside the measurement.** Coordinators health-check every data
  ...
  is not excluded from the throughput number either. The control read exists to detect this: reads on
  main should show a near-zero delta against standalone, and if it does not, the perturbation is the
  harness or the coordinators rather than replication. It is one query now rather than two, so the
  control is a single point of comparison and a surprising result there is worth re-running before
  acting on it.
...
- **The commit-size axis has one sample, so it is not measured at all.** Every remaining write is a
  single-entity commit. The first cluster run is why this is worth stating: every single-entity write
  landed within a few percent of 1700 QPS regardless of what it created — two nodes and an edge cost
  the same as one empty node, two indexed lookups cost nothing measurable — because the SYNC commit
  round trip dominates. The one multi-entity query, `arango/unwind_range_vertex_write`, moved 4344
  nodes a second against `vertex_big`'s 1229, and it has since been dropped from the set. So the
  suite now characterises the per-commit cost well and says nothing about how batching amortises it,
  which on that evidence is the more interesting axis. A workload with tunable commit size is the way
  to get it back; see Future work.
...
- **A purpose-built workload with tunable commit size**, to sweep rather than sample the axis
  between RTT-bound and delta-volume-bound replication cost. Promoted from a nice-to-have by the first
  cluster run: query shape turned out not to differentiate at all, while the single batched query
  moved three and a half times the entities per second. With that query dropped, nothing in the suite
  covers this.
