...
void distanceOneConfResolution(graph *G, long v, int *vtxColor, double *randValues, long *QtmpTail, long *Qtmp,
                               ColorVector &freq, int type);
void distanceOneChecked(graph *G, long nv, int *colors);
void buildColorsIndex(int *colors, const int numColors, const long nv, ColorVector &colorPtr, ColorVector &colorIndex,
                      ColorVector &binSizes);
...
void computeBinSizesWeighted(ColorVector &binSizes, const ColorVector &colors, const GraphElem nv, const ColorElem
numColors, const Graph &g); void outPut(const ColorVector &colors, std::string output, const ColorVector& freq, const
ColorElem ncolors);

void buildColorsIndex(const ColorVector& colors, const ColorElem numColors, const ColorElem nv, ColorVector& colorPtr,
ColorVector& colorIndex, ColorVector& binSizes);

//bool findConflicts(const Graph &g, const ColorElem targetColor, const ColorVector &colors, const GraphElem vertex);

void distanceOneConfResolution(const Graph &g, const GraphElem &sv,  ColorVector &colors, ColorVector &freq, const
RandVec& randVec,  ColorQueue& qtmp, GraphElem& qtmpPos, int type);

void distanceOneConfResolutionWeighted(const Graph &g, const GraphElem &sv,  ColorVector &colors, ColorVector &freq,
const RandVec& randVec,  ColorQueue& qtmp, GraphElem& qtmpPos, int type);
...
ColorElem TrueSerialvBaseRedistribution(const Graph &g, ColorVector &baseColors, std::string input, ColorElem ncolors,
int type); ColorElem wBaseRedistribution(const Graph &g, ColorVector &baseColors, std::string input, ColorElem ncolors,
int type);
