...
#pragma omp parallel
  ...
  long NT = NV - NS;
  bool isSym = true;
  if (NT > 0) isSym = false;  // A bipartite graph
  long NE = G->numEdges;
  ...
  time1 = omp_get_wtime();
  long *degree = (long *)malloc(NV * sizeof(long));
  assert(degree != 0);
#pragma omp parallel for
  ...
  long *visited = (long *)malloc(NV * sizeof(long));
  assert(visited != 0);
  long *R = (long *)malloc(NV * sizeof(long));
  assert(R != 0);
...
#pragma omp parallel for
  ...
  assert(Q != 0);
  long *Qtmp = (long *)malloc(NV * sizeof(long));
  assert(Qtmp != 0);
  long *Qswap;
...
#pragma omp parallel for
  ...
  if (isSym) {  // A symmetric matrix
    ...
    long rowCounter = 0;
    long colCounter = NS;
    long *Rprime = (long *)malloc(NV * sizeof(long));
    assert(Rprime != 0);
    for (long i = 0; i < NV; i++) {
      Rprime[i] = -1;
    }
    ...
    assert(rowCounter == NS);
    assert(colCounter == NV);  // Sanity check
...
#pragma omp parallel for
  ...
  long *R = (long *)malloc(NV * sizeof(long));
  assert(R != 0);
  long *oneLevel = (long *)malloc(NV * sizeof(long));
  assert(oneLevel != 0);
  // Initialize the Vectors:
#pragma omp parallel for
  for (long i = 0; i < NV; i++) {
