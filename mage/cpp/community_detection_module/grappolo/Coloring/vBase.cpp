...
int vBaseRedistribution(graph *G, int *vtxColor, int ncolors, int type) {
  ...
  int *baseColors = (int *)malloc(NVer * sizeof(int));
  assert(baseColors != 0);
#pragma omp parallel for
  for (long i = 0; i < NVer; i++) baseColors[i] = vtxColor[i];
...
#pragma omp parallel for reduction(max : realMaxDegree)
  for (long i = 0; i < NVer; i++) {
    ...
    adj2 = verPtr[i + 1];
    de = adj2 - adj1;
    if (de > realMaxDegree) realMaxDegree = de;
  }
...
#pragma omp parallel for
  for (size_t ci = 0U; ci < ncolors; ci++)
    if (freq[ci] > avg) overSize[ci] = true;
...
#pragma omp parallel for
    for (long Qi = 0; Qi < QTail; Qi++) {
      ...
      if (overSize[baseColors[v]] == false) continue;
      if ((vtxColor[v] != -1) && (freq[vtxColor[v]] <= avg)) continue;
      ...
      if (type == 0) {  // First Fit
        for (myColor = 0; myColor <= ncolors; myColor++) {
          if ((mark[myColor] == false) && (freq[myColor] < avg) && (overSize[myColor] != true)) break;
        }
      ...
      if (vtxColor[v] == -1 && (myColor == -1 || myColor == ncolors)) myColor = baseColors[v];
