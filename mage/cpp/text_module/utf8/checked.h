...
inline void append(utfchar32_t cp, std::string &s) { append(cp, std::back_inserter(s)); }
...
utfchar32_t next16(word_iterator &it, word_iterator end) {
  ...
  if (err_code == internal::NOT_ENOUGH_ROOM) throw not_enough_room();
  return cp;
}
...
void advance(octet_iterator &it, distance_type n, octet_iterator end) {
  ...
  if (n < zero) {
    // backward
    for (distance_type i = n; i < zero; ++i) utf8::prior(it, end);
  } else {
    // forward
    for (distance_type i = zero; i < n; ++i) utf8::next(it, end);
  }
}
