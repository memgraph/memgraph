...
def init_migrate_postgresql(
    ...
    if postgres_dict[thread_id][Constants.CURSOR] is None:
        ...
        postgres_dict[thread_id][Constants.COLUMN_NAMES] = [column.name for column in cursor.description]
...
def init_migrate_arrow_flight(
    ...
    flight_dict[thread_id][Constants.CURSOR] = iter(_fetch_flight_data(client, flight_info, options))
...
def init_migrate_duckdb(query: str, setup_queries: mgp.Nullable[List[str]] = None):
    ...
    duckdb_dict[thread_id][Constants.COLUMN_NAMES] = [desc[0] for desc in cursor.description]
...
def memgraph(
    ...
    return [mgp.Record(row=row) for row in (next(cursor, None) for _ in range(Constants.BATCH_SIZE)) if row is not None]
