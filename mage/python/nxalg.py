...
def node_connectivity(
    ...
    return mgp.Record(connectivity=nx.node_connectivity(MemgraphMultiDiGraph(ctx=ctx), source, target))
...
def degree_assortativity_coefficient(
    ...
    return mgp.Record(
        assortativity=nx.degree_assortativity_coefficient(MemgraphMultiDiGraph(ctx=ctx), x, y, weight, nodes)
    )
...
def node_boundary(
    ...
    return mgp.Record(boundary=list(nx.node_boundary(MemgraphMultiDiGraph(ctx=ctx), nbunch1, nbunch2)))


# networkx.algorithms.bridges.bridges
@mgp.read_proc
def bridges(ctx: mgp.ProcCtx, root: mgp.Nullable[mgp.Vertex] = None) -> mgp.Record(bridges=mgp.List[mgp.Edge]):
    g = MemgraphMultiGraph(ctx=ctx)
    return mgp.Record(bridges=[next(iter(g[u][v])) for u, v in nx.bridges(MemgraphGraph(ctx=ctx), root=root)])
...
def chain_decomposition(
    ...
    g = MemgraphMultiGraph(ctx=ctx)
    return mgp.Record(
        chains=[[next(iter(g[u][v])) for u, v in d] for d in nx.chain_decomposition(MemgraphGraph(ctx=ctx), root=root)]
    )
...
def clustering(
    ...
    return [
        ...
        for n, c in nx.clustering(MemgraphDiGraph(ctx=ctx), nodes=nodes, weight=weight).items()
    ]
...
def greedy_color(
    ...
    return [
        ...
        for n, c in nx.greedy_color(MemgraphMultiDiGraph(ctx=ctx), strategy, interchange).items()
    ]
...
def k_clique_communities(
    ...
    return mgp.Record(
        communities=[list(s) for s in nx.community.k_clique_communities(MemgraphMultiGraph(ctx=ctx), k, cliques)]
    )
...
def k_components(
    ...
    return [mgp.Record(k=k, components=[list(s) for s in comps]) for k, comps in kcomps.items()]
...
@mgp.read_proc
def k_edge_components(ctx: mgp.ProcCtx, k: int) -> mgp.Record(components=mgp.List[mgp.List[mgp.Vertex]]):
    return mgp.Record(components=[list(s) for s in nx.k_edge_components(nx.DiGraph(MemgraphDiGraph(ctx=ctx)), k)])


# networkx.algorithms.core.core_number
@mgp.read_proc
def core_number(ctx: mgp.ProcCtx) -> mgp.Record(node=mgp.Vertex, core=mgp.Number):
    return [mgp.Record(node=n, core=c) for n, c in nx.core_number(MemgraphDiGraph(ctx=ctx)).items()]


# networkx.algorithms.covering.is_edge_cover
@mgp.read_proc
def is_edge_cover(ctx: mgp.ProcCtx, cover: mgp.List[mgp.Edge]) -> mgp.Record(is_edge_cover=bool):
    cover = set([(e.from_vertex, e.to_vertex) for e in cover])
    return mgp.Record(is_edge_cover=nx.is_edge_cover(MemgraphMultiGraph(ctx=ctx), cover))
...
def find_cycle(
    ...
    try:
        return mgp.Record(cycle=[e for _, _, e in nx.find_cycle(MemgraphMultiDiGraph(ctx=ctx), source, orientation)])
...
def simple_cycles(
    ...
    return mgp.Record(cycles=list(nx.simple_cycles(nx.MultiDiGraph(MemgraphMultiDiGraph(ctx=ctx)).copy())))


# networkx.algorithms.cuts.node_expansion
@mgp.read_proc
def node_expansion(ctx: mgp.ProcCtx, s: mgp.List[mgp.Vertex]) -> mgp.Record(node_expansion=mgp.Number):
    return mgp.Record(node_expansion=nx.node_expansion(MemgraphMultiDiGraph(ctx=ctx), set(s)))
...
@mgp.read_proc
def ancestors(ctx: mgp.ProcCtx, source: mgp.Vertex) -> mgp.Record(ancestors=mgp.List[mgp.Vertex]):
    return mgp.Record(ancestors=list(nx.ancestors(MemgraphMultiDiGraph(ctx=ctx), source)))


# networkx.algorithms.dag.descendants
@mgp.read_proc
def descendants(ctx: mgp.ProcCtx, source: mgp.Vertex) -> mgp.Record(descendants=mgp.List[mgp.Vertex]):
    return mgp.Record(descendants=list(nx.descendants(MemgraphMultiDiGraph(ctx=ctx), source)))
...
def is_distance_regular(ctx: mgp.ProcCtx) -> mgp.Record(is_distance_regular=bool):
    return mgp.Record(is_distance_regular=nx.is_distance_regular(MemgraphMultiGraph(ctx=ctx)))
...
def is_strongly_regular(ctx: mgp.ProcCtx) -> mgp.Record(is_strongly_regular=bool):
    return mgp.Record(is_strongly_regular=nx.is_strongly_regular(MemgraphMultiGraph(ctx=ctx)))
...
def immediate_dominators(
    ...
    return [
        ...
        for n, d in nx.immediate_dominators(MemgraphMultiDiGraph(ctx=ctx), start).items()
    ]
...
def dominating_set(
    ...
    return mgp.Record(dominating_set=list(nx.dominating_set(MemgraphMultiDiGraph(ctx=ctx), start)))
...
def global_efficiency(ctx: mgp.ProcCtx) -> mgp.Record(global_efficiency=float):
    return mgp.Record(global_efficiency=nx.global_efficiency(MemgraphMultiGraph(ctx=ctx)))
...
def has_eulerian_path(ctx: mgp.ProcCtx) -> mgp.Record(has_eulerian_path=bool):
    return mgp.Record(has_eulerian_path=nx.has_eulerian_path(MemgraphMultiDiGraph(ctx=ctx)))


# networkx.algorithms.hierarchy.flow_hierarchy
@mgp.read_proc
def flow_hierarchy(ctx: mgp.ProcCtx, weight: mgp.Nullable[str] = None) -> mgp.Record(flow_hierarchy=float):
    return mgp.Record(flow_hierarchy=nx.flow_hierarchy(MemgraphMultiDiGraph(ctx=ctx), weight=weight))
...
def jaccard_coefficient(
    ...
    return [mgp.Record(u=u, v=v, coef=c) for u, v, c in nx.jaccard_coefficient(MemgraphGraph(ctx=ctx), ebunch)]
...
def lowest_common_ancestor(
    ...
    return mgp.Record(ancestor=nx.lowest_common_ancestor(MemgraphDiGraph(ctx=ctx), node1, node2))
...
def maximal_matching(ctx: mgp.ProcCtx) -> mgp.Record(edges=mgp.List[mgp.Edge]):
    g = MemgraphMultiDiGraph(ctx=ctx)
    return mgp.Record(edges=list(next(iter(g[u][v])) for u, v in nx.maximal_matching(g)))
...
def shortest_path(
    ...
    return [mgp.Record(source=s, target=t, path=p) for s, d in sp.items() for t, p in d.items()]
...
def shortest_path_length(
    ...
    return [mgp.Record(source=s, target=t, length=l) for s, d in sp.items() for t, l in d.items()]
...
@mgp.read_proc
def has_path(ctx: mgp.ProcCtx, source: mgp.Vertex, target: mgp.Vertex) -> mgp.Record(has_path=bool):
    return mgp.Record(has_path=nx.has_path(MemgraphMultiDiGraph(ctx=ctx), source, target))
...
@mgp.read_proc
def is_simple_path(ctx: mgp.ProcCtx, nodes: mgp.List[mgp.Vertex]) -> mgp.Record(is_simple_path=bool):
    return mgp.Record(is_simple_path=nx.is_simple_path(MemgraphMultiDiGraph(ctx=ctx), nodes))
...
def all_simple_paths(
    ...
    return mgp.Record(paths=list(nx.all_simple_paths(MemgraphMultiDiGraph(ctx=ctx), source, target, cutoff=cutoff)))


# networkx.algorithms.tournament.is_tournament
@mgp.read_proc
def is_tournament(ctx: mgp.ProcCtx) -> mgp.Record(is_tournament=bool):
    return mgp.Record(is_tournament=nx.tournament.is_tournament(MemgraphDiGraph(ctx=ctx)))
...
def bfs_predecessors(
    ...
    return [
        ...
        for n, p in nx.bfs_predecessors(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit)
    ]
...
def bfs_successors(
    ...
    return [
        ...
        for n, s in nx.bfs_successors(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit)
    ]
...
def dfs_tree(
    ...
    return mgp.Record(tree=list(nx.dfs_tree(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit)))
...
def dfs_predecessors(
    ...
    return [
        mgp.Record(node=n, predecessor=p)
        for n, p in nx.dfs_predecessors(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit).items()
    ]
...
def dfs_successors(
    ...
    return [
        mgp.Record(node=n, successors=s)
        for n, s in nx.dfs_successors(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit).items()
    ]
...
def dfs_preorder_nodes(
    ...
    return mgp.Record(nodes=list(nx.dfs_preorder_nodes(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit)))
...
def dfs_postorder_nodes(
    ...
    return mgp.Record(
        nodes=list(nx.dfs_postorder_nodes(MemgraphMultiDiGraph(ctx=ctx), source, depth_limit=depth_limit))
    )
...
def edge_bfs(
    ...
    return mgp.Record(
        edges=list(e for _, _, e in nx.edge_bfs(MemgraphMultiDiGraph(ctx=ctx), source=source, orientation=orientation))
    )
...
def edge_dfs(
    ...
    return mgp.Record(
        edges=list(e for _, _, e in nx.edge_dfs(MemgraphMultiDiGraph(ctx=ctx), source=source, orientation=orientation))
    )
...
def minimum_spanning_tree(
    ...
    gres = nx.minimum_spanning_tree(MemgraphMultiGraph(ctx=ctx), weight, algorithm, ignore_nan)
    return mgp.Record(nodes=list(gres.nodes()), edges=[e for _, _, e in gres.edges(keys=True)])


# networkx.algorithms.triads.triadic_census
@mgp.read_proc
def triadic_census(ctx: mgp.ProcCtx) -> mgp.Record(triad=str, count=int):
    return [mgp.Record(triad=t, count=c) for t, c in nx.triadic_census(MemgraphDiGraph(ctx=ctx)).items()]
...
def voronoi_cells(
    ...
    return [
        ...
        for c1, c2 in nx.voronoi_cells(MemgraphMultiDiGraph(ctx=ctx), center_nodes, weight=weight).items()
    ]


# networkx.algorithms.wiener.wiener_index
@mgp.read_proc
def wiener_index(ctx: mgp.ProcCtx, weight: mgp.Nullable[str] = None) -> mgp.Record(wiener_index=mgp.Number):
    return mgp.Record(wiener_index=nx.wiener_index(MemgraphMultiDiGraph(ctx=ctx), weight=weight))
