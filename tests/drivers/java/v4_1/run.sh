...
if [ ! -f $DRIVER ]; then
    ...
    wget_retry https://repo1.maven.org/maven2/org/neo4j/driver/neo4j-java-driver/4.1.1/neo4j-java-driver-4.1.1.jar -O $DRIVER || exit 1
fi

if [ ! -f $REACTIVE_STREAM_DEP ]; then
    wget_retry https://repo1.maven.org/maven2/org/reactivestreams/reactive-streams/1.0.3/reactive-streams-1.0.3.jar -O $REACTIVE_STREAM_DEP || exit 1
fi
