Feature: Vector edge search related features
    ...
    Scenario: Create AND vector edge index is rejected (an edge has only one type)
        Given an empty graph
        When executing query:
            """
    ...
    Scenario: Wildcard vector edge index search returns edges of all types
        Given an empty graph
        And having executed
            """
            CREATE VECTOR EDGE INDEX wildcard_edge ON (embedding) WITH CONFIG {"dimension": 2, "capacity": 10}
        ...
        And having executed
            """
            CREATE (a)-[:E1 {embedding: [1.0, 0.0]}]->(b)
            CREATE (a)-[:E2 {embedding: [0.0, 1.0]}]->(b)
            CREATE (a)-[:E3 {embedding: [2.0, 2.0]}]->(b);
            """
        When executing query:
            """
            SHOW INDEX INFO
        ...
        Then the result should be:
            | index type                  | label | property    | count |
            | 'edge-type+property_vector' | '*'   | 'embedding' | 3     |
        When executing query:
            """
            CALL vector_search.search_edges("wildcard_edge", 10, [1.0, 0.0]) YIELD * RETURN count(*) AS cnt;
            """
        Then the result should be:
            | cnt |
            | 3   |

    Scenario: OR vector edge index search returns only matching edge types
        Given an empty graph
        And having executed
            """
            CREATE VECTOR EDGE INDEX or_edge ON :E1|E2(embedding) WITH CONFIG {"dimension": 2, "capacity": 10}
        ...
        Then the result should be:
            | index type                  | label      | property    | count |
            | 'edge-type+property_vector' | ':E1\|E2'  | 'embedding' | 2     |
        When executing query:
            """
            CALL vector_search.search_edges("or_edge", 10, [1.0, 0.0]) YIELD * RETURN count(*) AS cnt;
            """
        Then the result should be:
            | cnt |
            | 2   |
