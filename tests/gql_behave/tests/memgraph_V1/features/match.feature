Feature: Match
    ...
    Scenario: Using OR expression with two label indices
        Given an empty graph
        And with new index :LabelA
        And with new index :LabelB
        And having executed:
            """
            CREATE (:LabelA {x: 1, name: 'A1'})
            CREATE (:LabelA {x: 999, name: 'A999'})
    ...
    Scenario: Using OR expression with two label property indices
        Given an empty graph
        And with new index :LabelA(x)
        And with new index :LabelB(x)
        And having executed:
            """
            CREATE (:LabelA {x: 1, name: 'A1'})
