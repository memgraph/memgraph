Feature: Order by
    ...
    Scenario: Test maps are ordered by their size, then their keys, then their values
        Given an empty graph
        When executing query:
            """
    ...
    Scenario: Test a map is ordered by every key before any value
        Given an empty graph
        When executing query:
            """
            UNWIND [[{a: 1, c: 9}, 'ac'], [{a: 2, b: 1}, 'ab']] AS p
            WITH p[0] AS v, p[1] AS tag
            ORDER BY v
            RETURN tag
            """
