Feature: Expressions
    ...
    Scenario: Test UNION folds the values equivalence holds alike
        Given an empty graph
        And having executed:
            """
            CREATE ({p: 1}), ({p: 1}), ({p: 0.0 / 0.0}), ({p: 0.0 / 0.0}), ({p: null}), ({p: null})
            """
        When executing query:
            """
            MATCH (n) RETURN n.p AS v
            UNION
            MATCH (n) RETURN n.p AS v
            """
        Then the result should be:
    ...
    Scenario: Test UNION ALL keeps the values UNION would fold
        Given an empty graph
        And having executed:
            """
            CREATE ({p: 1}), ({p: 1}), ({p: null}), ({p: null})
            """
        When executing query:
            """
    ...
    Scenario: Test CASE compares its term by equality
        Given an empty graph
        When executing query:
            """
            RETURN CASE null WHEN null THEN 'matched' ELSE 'no' END AS nothing,
                   CASE 0.0 / 0.0 WHEN 0.0 / 0.0 THEN 'matched' ELSE 'no' END AS nan,
                   CASE 1 WHEN 1 THEN 'matched' ELSE 'no' END AS number
            """
        Then the result should be:
            | nothing | nan  | number    |
            | 'no'    | 'no' | 'matched' |

    Scenario: Test any all and none read a predicate that a null leaves undecided
        Given an empty graph
        When executing query:
            """
            RETURN any(x IN [1, null] WHERE x = 1) AS some,
                   all(x IN [1, null] WHERE x = 1) AS every,
                   none(x IN [1, null] WHERE x = 1) AS neither,
                   single(x IN [1, 2, null] WHERE x = 1) AS exactly_one
            """
        Then the result should be:
            | some | every | neither | exactly_one |
            | true | null  | false   | null        |

    Scenario: Test a list comprehension keeps the elements its predicate settles
        Given an empty graph
        When executing query:
            """
            RETURN [x IN [1, null, 2] WHERE x > 1 | x] AS kept,
                   [x IN [1, 0.0 / 0.0, 2] WHERE x > 1 | x] AS without_nan
            """
        Then the result should be:
            | kept | without_nan |
            | [2]  | [2]         |

    Scenario: Test MERGE refuses a null it would have to match by
        Given an empty graph
        When executing query:
            """
            MERGE (n:M {p: null}) RETURN n
            """
        Then an error should be raised

    Scenario: Test MERGE does not match a stored NaN, which equals nothing
        Given an empty graph
        And having executed:
            """
            CREATE (:M {p: 0.0 / 0.0})
        ...
        When executing query:
            ...
            MATCH (m:M) RETURN count(*) AS c
            """
        Then the result should be:
            | c |
            | 2 |

    Scenario: Test MERGE matches a stored value equality settles
        Given an empty graph
        And having executed:
            """
            CREATE (:M {p: 1})
    ...
    Scenario: Test DISTINCT folds the values equivalence holds alike
        Given an empty graph
        And having executed:
            """
            CREATE ({p: 1}), ({p: 1.0}), ({p: 0.0 / 0.0}), ({p: 0.0 / 0.0}), ({p: null}), ({p: null}), ({p: 'a'})
            """
        When executing query:
            """
            MATCH (n) RETURN DISTINCT n.p AS v ORDER BY v
            """
    ...
    Scenario: Test DISTINCT keeps two containers a null does not tell apart
        Given an empty graph
        When executing query:
            """
            UNWIND [[1, null], [1, null], [1, 2]] AS l
