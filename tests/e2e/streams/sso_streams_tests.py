...
def multi_role_connection(request):
    ...
    with GraphDatabase.driver(MG_URI, auth=("", "")) as client:
        with client.session() as session:
            ...
            session.run("GRANT CREATE ON LABELS * TO user;").consume()
            session.run("GRANT READ ON LABELS * TO user;").consume()
            session.run("GRANT UPDATE ON LABELS * TO user;").consume()
            session.run("GRANT DELETE ON LABELS * TO user;").consume()
            session.run("GRANT CREATE ON EDGE_TYPES * TO user;").consume()
            session.run("GRANT READ ON EDGE_TYPES * TO user;").consume()
            session.run("GRANT UPDATE ON EDGE_TYPES * TO user;").consume()
            session.run("GRANT DELETE ON EDGE_TYPES * TO user;").consume()
