...
def test_check_stream_then_start_stream(pulsar_client, pulsar_topics, connection):
    assert len(pulsar_topics) > 0

    def stream_creator(stream_name, batch_size):
        query = f"CREATE PULSAR STREAM {stream_name} TOPICS {pulsar_topics[0]} TRANSFORM pulsar_transform.simple BATCH_INTERVAL 5000 BATCH_SIZE {batch_size} "
        return query
