...
def test_lba_procedures_show_privileges_first_user():
    expected_assertions_josip = [
        ...
        (
            "CREATE, READ, UPDATE(SET PROPERTY), DELETE ON ALL EDGE_TYPES",
            "GRANT",
            "GLOBAL EDGE_TYPE PERMISSION GRANTED TO USER",
        ),
        ("READ, SET PROPERTY {*} ON ALL LABELS", "GRANT", "GLOBAL PROPERTY PERMISSION GRANTED TO USER"),
        ("READ, SET PROPERTY {*} ON ALL EDGE_TYPES", "GRANT", "GLOBAL PROPERTY PERMISSION GRANTED TO USER"),
        ("READ ON NODES CONTAINING LABELS :Label1 MATCHING ANY", "GRANT", "LABEL PERMISSION GRANTED TO USER"),
        (
            "CREATE, READ, UPDATE(SET LABEL, REMOVE LABEL, SET PROPERTY, CREATE EDGE, DELETE EDGE), DELETE ON NODES CONTAINING LABELS :Label2 MATCHING ANY",
            "DENY",
            "LABEL PERMISSION DENIED TO USER",
        ),
        (
            "UPDATE(SET LABEL, REMOVE LABEL, SET PROPERTY, CREATE EDGE, DELETE EDGE) ON NODES CONTAINING LABELS :Label3 MATCHING ANY",
            "GRANT",
        ...
        (
            "UPDATE(SET LABEL, REMOVE LABEL, SET PROPERTY, CREATE EDGE, DELETE EDGE) ON NODES CONTAINING LABELS :Label6 MATCHING ANY",
            "GRANT",
            "LABEL PERMISSION GRANTED TO USER",
        ),
        (
            "CREATE, READ, UPDATE(SET LABEL, REMOVE LABEL, SET PROPERTY, CREATE EDGE, DELETE EDGE), DELETE ON NODES CONTAINING LABELS :Label7 MATCHING ANY",
            "DENY",
            "LABEL PERMISSION DENIED TO USER",
        ),
    ]
...
def test_lba_procedures_show_privileges_fourth_user():
    expected_assertions_bruno = [
        ("AUTH", "GRANT", "GRANTED TO USER"),
        (
            "UPDATE(SET LABEL, REMOVE LABEL, SET PROPERTY, CREATE EDGE, DELETE EDGE) ON ALL LABELS",
            "GRANT",
            "GLOBAL LABEL PERMISSION GRANTED TO USER",
        ),
    ]
...
def test_show_privileges_multiple_labels_matching_any():
    ...
    execute_and_fetch_all(
        auth_cursor, "GRANT SET PROPERTY ON EDGES OF TYPE :KNOWS, SET PROPERTY ON EDGES OF TYPE :MANAGES TO test_user;"
    )
    ...
    assert "UPDATE(SET PROPERTY) ON EDGES OF TYPE :KNOWS" in privilege_strings
    assert "UPDATE(SET PROPERTY) ON EDGES OF TYPE :MANAGES" in privilege_strings
