...
def test_lba_procedures_show_privileges_first_user():
    expected_assertions_josip = [
        (
            "CREATE, READ, SET LABEL, REMOVE LABEL, SET PROPERTY, DELETE, DELETE EDGE, CREATE EDGE ON ALL LABELS",
            "GRANT",
        ...
        (
            "CREATE, READ, SET LABEL, REMOVE LABEL, SET PROPERTY, DELETE, DELETE EDGE, CREATE EDGE ON NODES CONTAINING LABELS :Label2 MATCHING ANY",
            "DENY",
            "LABEL PERMISSION DENIED TO USER",
        ),
        (
            "SET LABEL, REMOVE LABEL, SET PROPERTY, DELETE EDGE, CREATE EDGE ON NODES CONTAINING LABELS :Label3 MATCHING ANY",
            "GRANT",
        ...
        (
            "SET LABEL, REMOVE LABEL, SET PROPERTY, DELETE EDGE, CREATE EDGE ON NODES CONTAINING LABELS :Label6 MATCHING ANY",
            "GRANT",
            "LABEL PERMISSION GRANTED TO USER",
        ),
        (
            "CREATE, READ, SET LABEL, REMOVE LABEL, SET PROPERTY, DELETE, DELETE EDGE, CREATE EDGE ON NODES CONTAINING LABELS :Label7 MATCHING ANY",
            "DENY",
            "LABEL PERMISSION DENIED TO USER",
        ),
    ]
...
def test_lba_procedures_show_privileges_fourth_user():
    expected_assertions_bruno = [
        ("AUTH", "GRANT", "GRANTED TO USER"),
        (
            "SET LABEL, REMOVE LABEL, SET PROPERTY, DELETE EDGE, CREATE EDGE ON ALL LABELS",
            "GRANT",
            "GLOBAL LABEL PERMISSION GRANTED TO USER",
        ),
    ]
...
def test_show_privileges_multiple_labels_matching_any():
    ...
    assert "SET PROPERTY ON EDGES OF TYPE :KNOWS" in privilege_strings
    assert "SET PROPERTY ON EDGES OF TYPE :MANAGES" in privilege_strings
