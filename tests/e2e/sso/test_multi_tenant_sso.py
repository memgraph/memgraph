...
def multi_tenant_setup(request):
    ...
    with GraphDatabase.driver(MG_URI, auth=("", "")) as client:
        with client.session() as session:
            ...
            session.run("GRANT CREATE ON EDGES CONTAINING TYPES :WORKS_FOR, :OWNS, :REVIEWS TO user;").consume()
            session.run("GRANT READ ON EDGES CONTAINING TYPES :WORKS_FOR, :OWNS, :REVIEWS TO user;").consume()
            session.run("GRANT UPDATE ON EDGES CONTAINING TYPES :WORKS_FOR, :OWNS, :REVIEWS TO user;").consume()
            session.run("GRANT DELETE ON EDGES CONTAINING TYPES :WORKS_FOR, :OWNS, :REVIEWS TO user;").consume()
