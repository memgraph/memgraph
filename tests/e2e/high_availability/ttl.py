...
def test_ttl_high_availability_failover(test_name):
    ...
    execute_and_fetch_all(
        instance1_cursor,
        "UNWIND RANGE(26,50) AS d CREATE ()-[:E2{ttl:timestamp() + timestamp(duration({second:d}))}]->();",
    )
    ...
    execute_and_ignore_dead_replica(
        instance1_cursor, "UNWIND RANGE(51,100) AS d CREATE (:TTL{ttl:timestamp() + timestamp(duration({second:d}))});"
    )
    execute_and_ignore_dead_replica(
        instance1_cursor,
        "UNWIND RANGE(51,75) AS d CREATE ()-[:E1{ttl:timestamp() + timestamp(duration({second:d}))}]->();",
    )
    execute_and_ignore_dead_replica(
        instance1_cursor,
        "UNWIND RANGE(76,100) AS d CREATE ()-[:E2{ttl:timestamp() + timestamp(duration({second:d}))}]->();",
    )
    ...
    # Verify TTL continues working on the main instance
    v_checker_main = VertexChecker(instance2_cursor)
...
def test_ttl_role_transition_ha(test_name):
    ...
    mg_sleep_and_assert(50, partial(get_vertex_count, instance1_cursor))
    mg_sleep_and_assert(50, partial(get_vertex_count, instance2_cursor))
    ...
    mg_assert_until(True, v_checker_old_main.is_same_vertices, max_duration=3)
    mg_assert_until(True, v_checker_old_main.is_same_edges, max_duration=3)
    ...
    # Verify TTL starts working again on the original main
    v_checker_restored_main = VertexChecker(instance1_cursor)
    mg_sleep_and_assert(True, v_checker_restored_main.is_less_vertices, max_duration=5)
    mg_sleep_and_assert(True, v_checker_restored_main.is_less_edges, max_duration=5)
