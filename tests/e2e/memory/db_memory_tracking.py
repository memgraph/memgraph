...
def take(result, count):
    """Pull at most *count* records from a neo4j driver result, leaving the rest unread.

    Iterating the result is the one way to do this that both the 4.x and 5.x drivers
    support, so the test does not depend on which of the two the environment resolved.

    The counter has to be zip's first argument: zip draws from its arguments in order and
    stops at the first exhausted one, so putting the result first would draw a record that
    is then discarded, and the caller would be told fewer records were pulled than were.
    """
    return [record for _, record in zip(range(count), result)]
