...
static void procedure(struct mgp_list *args, struct mgp_graph *graph, struct mgp_result *result,
  ...
  const enum mgp_error_code new_record_err = mgp_result_new_record(result, &record);
  if (new_record_err != MGP_ERROR_NO_ERROR) return set_error(result);

  struct mgp_value *result_msg = NULL;
  const enum mgp_error_code make_string_err =
      mgp_value_make_string("mgp_init_module allocation works", memory, &result_msg);
  if (make_string_err != MGP_ERROR_NO_ERROR) return set_error(result);

  const enum mgp_error_code result_inserted = mgp_result_record_insert(record, "result", result_msg);
...
int mgp_init_module(struct mgp_module *module, struct mgp_memory *memory) {
  ...
  const enum mgp_error_code alloc_err = mgp_global_alloc(one_gb, &gVal);
  if (alloc_err != MGP_ERROR_NO_ERROR) return 1;

  struct mgp_proc *proc = NULL;
  const enum mgp_error_code proc_err = mgp_module_add_read_procedure(module, "procedure", procedure, &proc);
  if (proc_err != MGP_ERROR_NO_ERROR) return 1;

  const struct mgp_type *string_type = NULL;
  const enum mgp_error_code string_type_err = mgp_type_string(&string_type);
