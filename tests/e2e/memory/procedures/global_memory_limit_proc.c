...
static void error(const struct mgp_list *args, const struct mgp_graph *graph, struct mgp_result *result,
  ...
  if (!gVal) {
    const enum mgp_error_code err = mgp_global_alloc(one_gb, &gVal);
  ...
  const enum mgp_error_code make_string_err = mgp_value_make_string("ERROR", memory, &error_value);
  if (make_string_err != MGP_ERROR_NO_ERROR) return set_error(result);
  const enum mgp_error_code result_inserted = mgp_result_record_insert(record, "error_result", error_value);
...
static void success(struct mgp_list *args, struct mgp_graph *graph, struct mgp_result *result,
  ...
  if (!gVal) {
    const enum mgp_error_code err = mgp_global_alloc(bytes, &gVal);
    if (err == MGP_ERROR_UNABLE_TO_ALLOCATE) return set_out_of_memory_error(result);
    if (err != MGP_ERROR_NO_ERROR) return set_error(result);
  }
  ...
  const enum mgp_error_code make_string_err = mgp_value_make_string("sucess", memory, &success_value);
  if (make_string_err != MGP_ERROR_NO_ERROR) return set_error(result);
  const enum mgp_error_code result_inserted = mgp_result_record_insert(record, "success_result", success_value);
...
int mgp_init_module(struct mgp_module *module, struct mgp_memory *memory) {
  ...
  const enum mgp_error_code error_proc_err = mgp_module_add_read_procedure(module, "error", error, &error_proc);
  ...
  const enum mgp_error_code succ_proc_err = mgp_module_add_read_procedure(module, "success", success, &succ_proc);
