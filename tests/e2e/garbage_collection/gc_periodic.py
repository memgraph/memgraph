...
def test_gc_periodic(connection):
    ...
    It does so by checking that the allocated memory is lowered by at least 1/3 of the memory allocated by creating nodes.
    If we choose a number around 1/2 or higher the test will become flaky because the memory only gets fully cleared after a few minutes
    ...
    assert memory_after_gc < memory_pre_creation + (memory_after_creation - memory_pre_creation) / 3 * 2
