...
Creates 100 nodes, then 12 workers each create edges between random pairs
and call nxalg.ancestors / date.format on each iteration.
"""
import os
import random
import sys
...
NUM_REPLICA_DATE_FORMAT_WORKERS = 4
NUM_ITERATIONS = 100
...
def replica_date_format_worker(worker_id: int) -> dict:
    query = """
CALL date.format(74976, "h", "%Y/%m/%d %H:%M:%S %Z", "Mexico/BajaNorte")
YIELD formatted
RETURN formatted;"""
    ...
    for iteration in range(NUM_ITERATIONS):
        ...
        if (iteration + 1) % 10 == 0:
            print(f"  [replica_date-{worker_id}] {iteration + 1}/{NUM_ITERATIONS}")
    elapsed = time.time() - t0
    print(f"  [replica_date-{worker_id}] Completed {NUM_ITERATIONS} iterations in {elapsed:.1f}s")
    return {"worker_id": worker_id, "type": "replica_date_format", "elapsed": elapsed}


def dispatch_worker(fn_type: str, worker_id: int) -> dict:
    if fn_type == "ancestors":
        return ancestors_worker(worker_id)
    if fn_type == "replica_date_format":
        return replica_date_format_worker(worker_id)
    return date_format_worker(worker_id)


def run_all_workers() -> None:
    total_workers = NUM_ANCESTORS_WORKERS + NUM_DATE_FORMAT_WORKERS + NUM_REPLICA_DATE_FORMAT_WORKERS
    print(
        f"\nRunning {total_workers} workers "
        f"({NUM_ANCESTORS_WORKERS} ancestors + {NUM_DATE_FORMAT_WORKERS} date_format + "
        f"{NUM_REPLICA_DATE_FORMAT_WORKERS} replica_date_format)..."
    )
    print(f"Each worker: {NUM_ITERATIONS} iterations")

    tasks = (
        [("ancestors", i) for i in range(NUM_ANCESTORS_WORKERS)]
        + [("date_format", i) for i in range(NUM_DATE_FORMAT_WORKERS)]
        + [("replica_date_format", i) for i in range(NUM_REPLICA_DATE_FORMAT_WORKERS)]
    )
...
def main():
    ...
    print(f"Replica date format workers: {NUM_REPLICA_DATE_FORMAT_WORKERS}")
        ...
        try:
            ...
            with driver.session() as session:
                if name in COORDINATOR_INSTANCES:
                    ...
                    print(f"  {name}: OK ({len(records)} instances)")
                else:
