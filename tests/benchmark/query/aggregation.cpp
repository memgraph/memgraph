...
// by running the binary once per setting of that flag:
//
//   for f in false true; do
//     ./aggregation --some-server-flag=$f --benchmark_min_time=0.8s \
...
// One property, read five times a row: the shape with the most to gain from any change that stops a
// property being looked up again within the same row.
...
// One property, read once. Nothing to batch and nothing to repeat, so a change that only removes
// repeated lookups must leave this alone - it is the control that says a change is not simply
// moving everything.
//
// It has to be *one* property and not two distinct ones. A change that fetches a row's properties
// in one pass would still turn two reads into one, so two distinct properties do get faster;
// `TwoDistinctProperties` below is that case.
void SinglePropertyRead(benchmark::State &state) {
  RunQuery(state, "MATCH (:Hub)-[:HAS]->(i:Item) RETURN min(i.value) AS lo");
}

// Two properties, each read once. No repeat, but still two lookups, so a change that fetches a
// row's properties in one pass replaces two reads with one.
