...
# HA port set — used to evict any leftover processes on teardown.
...
def _build_result_json(
    ...
    """Assemble the bench-graph schema uploadable by tools/bench-graph-client/main.py.

    ``results`` maps (label, target) → {"combined_qps", "read_qps", "write_qps"}.  Each cell
    produces three bench/default/scan entries: rust_<label>_<target>_combined, _read, _write.
    """
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--write-pct", type=int, default=20, metavar="P", help="Write percentage for mix mode (default 20)."
    )
...
def main() -> None:
    ...
    # Client-pin command prefix: numactl in numa mode (node available + numactl
    # present), taskset otherwise.
