...
fn main() {
    ...
    if args.len() < 5 {
        eprintln!(
            "usage: {} <nthreads> <duration_secs> <mode:point|heavy|split> <target:direct|routing> [n_writers] [write_batch]",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    let n_writers: usize = {
        let raw = args
            .get(5)
            .map(|s| s.parse::<usize>().expect("n_writers must be a positive integer"))
            .unwrap_or(2);
    ...
    let mode = match mode_str {
        ...
        "heavy" => Mode::Heavy,
        "split" if nthreads < 2 => {
            eprintln!("split mode needs at least 2 threads (1 writer + 1 reader)");
            std::process::exit(1);
        }
        "split" => Mode::Split { write_batch },
        other => {
            eprintln!("unknown mode '{}'; expected point, heavy, or split", other);
            std::process::exit(1);
        }
    };
