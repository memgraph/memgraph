...
def _verify_event_counters_fields(event_counters: dict) -> bool:
    actual_keys = set(event_counters.keys())
    missing = EXPECTED_EVENT_COUNTERS - actual_keys
    unexpected = actual_keys - EXPECTED_EVENT_COUNTERS
    assert not missing, f"Missing event counters: {sorted(missing)}"
    assert not unexpected, f"Unexpected event counters: {sorted(unexpected)}"
...
def _verify_storage_fields(storage_data: dict) -> bool:
    ...
    storage_fields = [
        ...
        ("num_descriptions", int),  # Number of server-side descriptions
    ]
    ...
    # Verify descriptions
    assert "num_descriptions" in storage_data, f"Missing 'num_descriptions' field in storage data {storage_data}"
    assert isinstance(
        storage_data["num_descriptions"], int
    ), f"Invalid type for 'num_descriptions': expected int, got {type(storage_data['num_descriptions'])}"
