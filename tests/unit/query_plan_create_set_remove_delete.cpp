...
TEST(QueryPlan, NodeFilterSet) {
  ...
  auto node_filter = std::make_shared<Filter>(expand.op_, nullptr, filter_expr);
  // SET n.prop = n.prop + 1
...
TEST(QueryPlan, FilterRemove) {
  ...
  auto filter = std::make_shared<Filter>(expand.op_, nullptr, LESS(filter_prop, LITERAL(43)));
  // REMOVE n.prop
