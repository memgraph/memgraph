...
inline constexpr int kRapidCreationCount = 100;
...
TEST(AsyncTimer, RapidCreationAndDestruction) {
  // Test rapid creation and destruction of timers
  for (int i = 0; i < kRapidCreationCount; ++i) {
    auto duration = std::chrono::milliseconds(20 + i * 5);
    AsyncTimer timer{ToSeconds(duration)};
    // Just let it go out of scope
  }

  // Verify system still works after rapid operations
  AsyncTimer final_timer{ToSeconds(kDefaultTimerDuration)};
  EXPECT_TRUE(VerifyTimerExpiration(final_timer));
}
