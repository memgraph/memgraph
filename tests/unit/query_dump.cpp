...

  {
    ...
    ASSERT_FALSE(unique_acc
                     ->CreateIndex(this->db->storage()->NameToLabel("Label1"),
                                   {this->db->storage()->NameToProperty("prop")}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
    auto unique_acc = this->db->UniqueAccess();
    ASSERT_FALSE(unique_acc
                     ->CreateIndex(this->db->storage()->NameToLabel("Label 2"),
                                   {this->db->storage()->NameToProperty("prop `")}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
        ...
        unique_acc
            ->CreateIndex(this->db->storage()->NameToLabel("Label1"),
                          {this->db->storage()->NameToProperty("prop_a"), this->db->storage()->NameToProperty("prop_b"),
                           this->db->storage()->NameToProperty("prop_c")}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
...
TYPED_TEST(DumpTest, EdgeIndicesKeys) {
  ...
  {
    auto unique_acc = this->db->UniqueAccess();
    !ASSERT_FALSE(unique_acc->CreateIndex(this->db->storage()->NameToEdgeType("EdgeType")).has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }

  {
    auto unique_acc = this->db->UniqueAccess();
    ASSERT_FALSE(unique_acc
                     ->CreateIndex(this->db->storage()->NameToEdgeType("EdgeType"),
                                   this->db->storage()->NameToProperty("prop")) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  ...
  {
    auto unique_acc = this->db->UniqueAccess();
    !ASSERT_FALSE(unique_acc->CreateGlobalEdgeIndex(this->db->storage()->NameToProperty("prop")).has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
...
TYPED_TEST(DumpTest, PointIndices) {
...

  {
    auto unique_acc = this->db->UniqueAccess();
    ASSERT_FALSE(unique_acc
                     ->CreatePointIndex(this->db->storage()->NameToLabel("Label1"),
                                        this->db->storage()->NameToProperty("prop")) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
...
TYPED_TEST(DumpTest, VectorIndices) {
...

  {
    const auto spec = memgraph::storage::VectorIndexSpec{test_index1.data(),
                                                         ...
                                                         scalar_kind};
    auto unique_acc = this->db->UniqueAccess();
    !ASSERT_FALSE(unique_acc->CreateVectorIndex(spec).has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
...
TYPED_TEST(DumpTest, VectorEdgeIndices) {
...

  {
    const auto spec = memgraph::storage::VectorEdgeIndexSpec{test_index1.data(),
                                                             ...
                                                             scalar_kind};
    auto unique_acc = this->db->UniqueAccess();
    !ASSERT_FALSE(unique_acc->CreateVectorEdgeIndex(spec).has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
...

  {
    ...
    ASSERT_FALSE(unique_acc
                     ->CreateIndex(this->db->storage()->NameToLabel("Person"),
                                   {this->db->storage()->NameToProperty("id")}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
    auto unique_acc = this->db->UniqueAccess();
    ASSERT_FALSE(unique_acc
                     ->CreateIndex(this->db->storage()->NameToLabel("Person"),
                                   {this->db->storage()->NameToProperty("unexisting_property")}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
    auto unique_acc = this->db->UniqueAccess();
    ASSERT_FALSE(
        unique_acc
            ->CreateTextIndex(memgraph::storage::TextIndexSpec{
                "text_index_without_properties", this->db->storage()->NameToLabel("Person"), {}}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
    auto unique_acc = this->db->UniqueAccess();
    ASSERT_FALSE(unique_acc
                     ->CreateTextIndex(memgraph::storage::TextIndexSpec{
                         "text_index_with_properties",
                         this->db->storage()->NameToLabel("Person"),
                         {this->db->storage()->NameToProperty("name")}}) !.has_value());
    !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  ...
  if constexpr (std::is_same_v<StorageTypes, memgraph::storage::InMemoryStorage>) {
    {
      ...
      ASSERT_FALSE(unique_acc
                       ->CreateTextEdgeIndex(
                           memgraph::storage::TextEdgeIndexSpec{"text_edge_index_without_properties",
    ...
    }
    {
      ...
      ASSERT_FALSE(unique_acc
                       ->CreateTextEdgeIndex(memgraph::storage::TextEdgeIndexSpec{
                           ...
                           this->db->storage()->NameToEdgeType("RELATES_TO"),
                           {this->db->storage()->NameToProperty("title"),
                            this->db->storage()->NameToProperty("content")}}) !.has_value());
      !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
    }
  }
...
TYPED_TEST(DumpTest, MultiplePartialPulls) {
  {
    ...
    }
    {
      ...
      ASSERT_FALSE(unique_acc
                       ->CreateIndex(this->db->storage()->NameToLabel("PERSON"),
                                     {this->db->storage()->NameToProperty("surname")}) !.has_value());
      !ASSERT_FALSE(unique_acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
    }
