...
auto Execute(storage::Storage *db, const std::string &query) {
  ...
  query::InterpreterContext context(db, data_directory, "non existing bootstrap servers");
...
class StatefulInterpreter {
 public:
  explicit StatefulInterpreter(storage::Storage *db)
      : db_(db), context_(db_, data_directory_, "non existing bootstrap servers"), interpreter_(&context_) {}
