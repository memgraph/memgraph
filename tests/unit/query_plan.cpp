...
TYPED_TEST(TestPlanner, CorrelatedPatternComprehensionInWithWhereAfterForeach) {
  ...
  // own copy tested only `external_symbols`, which held nothing here - the comprehension's sole free reference is its
  // pattern's start node, which the set this replaced subtracted as one of the pattern's own - so it drained on entry
  // to the FOREACH and was planned uncorrelated inside the body.
  FakeDbAccessor dba;
  auto prop = dba.Property("prop");
  auto *pattern_comp = PATTERN_COMPREHENSION(
      nullptr, PATTERN(NODE("n"), EDGE("anon1", EdgeAtom::Direction::BOTH), NODE("m")), nullptr, LITERAL(1));
...
TYPED_TEST(TestPlanner, PatternComprehensionOverYieldedSymbolInPatternPropertyStaysAboveCallProcedure) {
  ...
  // Same defect as the test above, reached from the comprehension's own pattern instead of its WHERE. The set the
  // splice decision reads was built by walking the comprehension's filter and result expression, so a correlation
  // living in a node property map, an edge property map or a variable-length bound was invisible: the RollUpApply went
  // below the CallProcedure and read an unwritten `field`, silently matching nothing. `external_symbols` now comes
  // from the symbol generator, which sees every position alike.
  FakeDbAccessor dba;
...
TYPED_TEST(TestPlanner, PatternComprehensionOverYieldedSymbolInNestedComprehensionStaysAboveCallProcedure) {
  ...
  // matching and the whole outer+nested pair was spliced below the CallProcedure. The outer frame records the
  // reference wherever inside the comprehension it occurs, nesting included.
  FakeDbAccessor dba;
  auto prop = dba.Property("prop");
