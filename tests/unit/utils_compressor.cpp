...
TEST(ZlibCompressorTest, CompressDecompressTest) {
  ...
  memgraph::utils::CompressedBuffer compressed = compressor->Compress(input_data.data(), input_size);
  EXPECT_GT(compressed.data.size(), 0);
  EXPECT_EQ(compressed.original_size, input_size);

  memgraph::utils::CompressedBuffer decompressed =
      compressor->Decompress(compressed.data.data(), compressed.data.size());
  EXPECT_EQ(decompressed.original_size, input_size);
  EXPECT_EQ(std::string_view(reinterpret_cast<char *>(decompressed.data.data()), decompressed.original_size),
...
TEST(ZlibCompressorTest, CompressEmptyDataTest) {
  ...
  memgraph::utils::CompressedBuffer compressed = compressor->Compress(input_data.data(), input_data.size());
  EXPECT_EQ(compressed.data.size(), 0);
  EXPECT_EQ(compressed.original_size, 0);

  memgraph::utils::CompressedBuffer decompressed =
      compressor->Decompress(compressed.data.data(), compressed.data.size());
  EXPECT_EQ(decompressed.data.size(), 0);
...
TEST(ZlibCompressorTest, IsCompressedTest) {
  ...
  memgraph::utils::CompressedBuffer compressed = compressor->Compress(input_data.data(), input_size);
  EXPECT_TRUE(compressor->IsCompressed(compressed.data.data(), compressed.data.size()));
