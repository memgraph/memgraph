...
TYPED_TEST(MatchReturnFixture, ScanAllWithAuthChecker) {
...

  {
    ...
    user.fine_grained_access_handler().label_permissions().Deny({labelName}, memgraph::auth::kAllPermissions);
...
TYPED_TEST(ExpandFixture, ExpandWithEdgeFiltering) {
  ...
  user.fine_grained_access_handler().edge_type_permissions().Deny({"edge_type_test"}, memgraph::auth::kAllPermissions);
  user.fine_grained_access_handler().label_permissions().GrantGlobal(memgraph::auth::kAllPermissions);
...
TYPED_TEST(QueryPlanExpandVariable, FineGrainedOneVariableExpansion) {
  ...
  // Layer 1 labels denied, All edge types granted
  {
    ...
    user.fine_grained_access_handler().label_permissions().Deny({"1"}, memgraph::auth::kAllPermissions);
  ...
  // Layer 2 labels denied, All edge types granted
  {
    ...
    user.fine_grained_access_handler().label_permissions().Deny({"2"}, memgraph::auth::kAllPermissions);
...
TYPED_TEST(QueryPlanExpandWeightedShortestPath, FineGrainedFiltering) {
  ...
  {
    memgraph::auth::User user{"test"};
    user.fine_grained_access_handler().label_permissions().Deny({"l0"}, memgraph::auth::kAllPermissions);
    user.fine_grained_access_handler().edge_type_permissions().GrantGlobal(memgraph::auth::FineGrainedPermission::READ);
  ...
  // Denied vertex label 2
  {
    ...
    user.fine_grained_access_handler().label_permissions().Deny({"l2"}, memgraph::auth::kAllPermissions);
