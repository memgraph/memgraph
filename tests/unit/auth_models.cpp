...
#ifdef MG_ENTERPRISE
TEST(AuthModule, UserMultiTenantRoleManagement) {
  memgraph::license::global_license_checker.EnableTesting(memgraph::license::LicenseType::ENTERPRISE);
  ASSERT_TRUE(memgraph::license::global_license_checker.IsEnterpriseValidFast());
  ...
  // Test SetMultiTenantRole
  ASSERT_NO_THROW(user.AddMultiTenantRole(role, "db1"));
  ASSERT_NO_THROW(user.AddMultiTenantRole(role, "db2"));
  ...
  ASSERT_EQ(roles_db3.size(), 0);  // Not set for db3

  // Test setting the same role again (should not change anything)
  ASSERT_NO_THROW(user.AddMultiTenantRole(role, "db1"));

  // Test ClearMultiTenantRoles
  user.ClearMultiTenantRoles("db1");

  roles_db1 = user.GetMultiTenantRoles("db1");
  ASSERT_EQ(roles_db1.size(), 0);  // Cleared

  roles_db2 = user.GetMultiTenantRoles("db2");
  ASSERT_EQ(roles_db2.size(), 1);  // Still set

  // Test clearing non-existent database
  ASSERT_NO_THROW(user.ClearMultiTenantRoles("non_existent_db"));

  // Test setting role without database access
  memgraph::auth::Role role_no_access("role_no_access");
  ASSERT_THROW(user.AddMultiTenantRole(role_no_access, "db1"), memgraph::auth::AuthException);

  // Test setting role that's already set globally
  ASSERT_NO_THROW(user.SetRole(role));
  ASSERT_THROW(user.AddMultiTenantRole(role, "db1"), memgraph::auth::AuthException);

  user.ClearRole();
  ASSERT_NO_THROW(user.AddMultiTenantRole(role, "db1"));
  ASSERT_THROW(user.AddRole(role), memgraph::auth::AuthException);
  ASSERT_EQ(user.GetMultiTenantRoles("db1").size(), 1);
  ASSERT_EQ(user.GetMultiTenantRoles("db1").begin()->rolename(), "test_role");
}

TEST(AuthModule, UserMultiTenantRoleWithMultipleRoles) {
  memgraph::license::global_license_checker.EnableTesting(memgraph::license::LicenseType::ENTERPRISE);
  ASSERT_TRUE(memgraph::license::global_license_checker.IsEnterpriseValidFast());

  // Create multiple roles with different database access
  ...
  memgraph::auth::Role role3("role3");
  role3.db_access().Grant("db1");
  role3.db_access().Grant("db3");

  // Create a user
  memgraph::auth::User user("test_user");

  // Set multiple roles for different databases
  user.AddMultiTenantRole(role1, "db1");
  user.AddMultiTenantRole(role2, "db2");
  user.AddMultiTenantRole(role3, "db3");
  ...
  auto roles_db2 = user.GetMultiTenantRoles("db2");
  ASSERT_EQ(roles_db2.size(), 1);
  ASSERT_EQ(roles_db2.begin()->rolename(), "role2");

  auto roles_db3 = user.GetMultiTenantRoles("db3");
  ASSERT_EQ(roles_db3.size(), 1);
  ASSERT_EQ(roles_db3.begin()->rolename(), "role3");
  ...
  roles_db2 = user.GetMultiTenantRoles("db2");
  ASSERT_EQ(roles_db2.size(), 0);

  roles_db3 = user.GetMultiTenantRoles("db3");
  ASSERT_EQ(roles_db3.size(), 1);  // Still set
}

TEST(AuthModule, UserMultiTenantRoleClearRole) {
  memgraph::license::global_license_checker.EnableTesting(memgraph::license::LicenseType::ENTERPRISE);
  ASSERT_TRUE(memgraph::license::global_license_checker.IsEnterpriseValidFast());
  ...
  ASSERT_EQ(user.GetMultiTenantRoles("db1").size(), 1);
  ASSERT_EQ(user.GetMultiTenantRoles("db2").size(), 1);

  // Clear all roles (should clear multi-tenant roles too)
  user.ClearRole();

  // Verify all roles are cleared
  ASSERT_EQ(user.GetMultiTenantRoles("db1").size(), 0);
  ASSERT_EQ(user.GetMultiTenantRoles("db2").size(), 0);
  ASSERT_TRUE(user.roles().empty());
}

TEST(AuthModule, UserMultiTenantRoleEdgeCases) {
  memgraph::license::global_license_checker.EnableTesting(memgraph::license::LicenseType::ENTERPRISE);
  ASSERT_TRUE(memgraph::license::global_license_checker.IsEnterpriseValidFast());
...
TEST(AuthModule, UserMultiTenantRoleWithGlobalRoles) {
  memgraph::license::global_license_checker.EnableTesting(memgraph::license::LicenseType::ENTERPRISE);
  ASSERT_TRUE(memgraph::license::global_license_checker.IsEnterpriseValidFast());

  // Create roles
  memgraph::auth::Role global_role("global_role");
  global_role.db_access().Grant("db1");
  global_role.db_access().Grant("db2");

  memgraph::auth::Role mt_role("mt_role");
  ...
  ASSERT_THROW(user.AddMultiTenantRole(global_role, "db1"), memgraph::auth::AuthException);
  ...
  ASSERT_EQ(user.roles().size(), 2);  // global_role + mt_role
  ASSERT_EQ(user.GetMultiTenantRoles("db1").size(), 1);
  ASSERT_EQ(user.GetMultiTenantRoles("db1").begin()->rolename(), "mt_role");

  // Clear global roles
  user.ClearRole();

  // Now should be able to set global_role as multi-tenant role
  ASSERT_NO_THROW(user.AddMultiTenantRole(global_role, "db1"));
  ASSERT_EQ(user.GetMultiTenantRoles("db1").size(), 1);
  ASSERT_EQ(user.GetMultiTenantRoles("db1").begin()->rolename(), "global_role");
}
#endif
