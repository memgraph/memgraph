...
// exercising the light-edge WAL replay arms (CreateLightEdge / DestroyLightEdge /
// edge SET_PROPERTY out-edges scan).
...
// replay arms: CreateLightEdge (EDGE_CREATE), the out-edges scan for
// EDGE_SET_PROPERTY on light edges, and DestroyLightEdge (EDGE_DELETE).
TEST_F(CreateSnapshotTest, LightEdgeWalReplayRoundTrip) {
  ...
  DurabilityTestGids gids;
  {
    ...
    gids = WriteDurabilityTestGraph(mem_storage);
    // Create then delete an extra edge to exercise the DELETE (DestroyLightEdge) arm.
    {
