...
class StorageV3 : public ::testing::Test {
 protected:
  ...
  VertexAccessor CreateVertexAndValidate(LabelId primary_label, const std::vector<LabelId> &labels,
...
TEST_F(StorageV3, Commit) {
  ...
  {
    auto acc = store.Access();
    auto vertex = CreateVertexAndValidate(label, {}, {{property, PropertyValue(0)}});
    gid = vertex.Gid();
