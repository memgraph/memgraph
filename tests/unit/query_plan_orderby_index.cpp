...
// Basic elimination - ORDER BY n.prop with :L(prop) index and range filter
...
// DESC ordering is not eliminated -- index only provides ASC order
...
// Non-property expression in ORDER BY -- computed expressions can't match index columns
...
// ORDER BY property differs from index property -- no elimination
...
// ORDER BY has more columns than the single-property index -- can't fully satisfy
...
// Expand preserves order - ORDER BY n.prop with Expand between OrderBy and ScanAll
...
// ORDER BY on expanded symbol m, not scan symbol n -- index order is on n.prop, not m.prop.
...
// Cartesian product between OrderBy and ScanAll breaks ordering guarantee
...
// Aggregate (hash grouping) between OrderBy and ScanAll destroys ordering
...
// Equality on first column + range on second, ORDER BY second -- pinned skip allows elimination
...
// Equality-only filter -- all rows have the same value, ORDER BY is trivially satisfied
...
// Two separate MATCHes produce a Cartesian -- ORDER BY n.prop can't be eliminated
...
// WITH renames n to m -- rename is tracked through Produce, elimination still applies
...
// Label-only index (no property index) should not trigger elimination
...
// WITH passes n through without renaming -- elimination still applies (complement of Test 18)
...
// ORDER BY n.b, n.a with index (a, b) -- reversed column order doesn't match
...
// Composite index (a, b), range on a, ORDER BY b -- should NOT eliminate.
...
// ORDER BY has more columns than composite index -- ORDER BY n.a, n.b, n.c with index (a, b).
...
// RETURN n AS m ORDER BY n.prop -- ORDER BY uses input scope name (dual-scope semantics)
...
// RETURN n AS m ORDER BY m.prop -- ORDER BY uses output scope name (dual-scope semantics)
...
// IN on first column, ORDER BY second -- IN is multi-valued, b not globally sorted
...
// IN on first column, ORDER BY first -- Unwind iterates in list order, not index order
...
// IN on first column, ORDER BY first and second -- a is sorted but b is only sorted within each a-group
...
// User-written UNWIND between OrderBy and ScanAll -- Unwind is order-preserving
...
// WITH n.prop AS a ORDER BY a -- bare Identifier resolved through Produce.
...
// RETURN n.prop AS a ORDER BY a -- alias resolved through RETURN Produce above OrderBy.
...
// WITH n.a AS x ORDER BY x -- different alias name, still resolved.
...
// WITH n AS m WITH m.a AS a ORDER BY a -- rename chain before projection.
...
// Mixed ORDER BY -- a is alias for n.a, n.b is PropertyLookup. Index on (a, b).
...
// Nested PropertyLookup on alias -- ORDER BY m.key where m is a map alias, not a node.
...
// PropertyLookup alias renamed through multiple WITHs -- ORDER BY t where t ← prop ← n.p.
...
TYPED_TEST(OrderByIndexTest, MixedOrderDirectionsNotEliminated) {
  // MATCH (n:L) WHERE n.a > 5 ORDER BY n.a ASC, n.b DESC RETURN n
  FakeDbAccessor dba;
  const auto *const label_name = "L";
  const auto label = dba.Label(label_name);
  const auto prop_a = PROPERTY_PAIR(dba, "a");
...
TYPED_TEST(OrderByIndexTest, BothIndicesExistCorrectOneSelected) {
  // MATCH (n:L) WHERE n.prop > 5 ORDER BY n.prop [ASC|DESC] RETURN n
  FakeDbAccessor dba;
  const auto *const label_name = "L";
  const auto label = dba.Label(label_name);
  const auto property = PROPERTY_PAIR(dba, "prop");
...
// Both ASC and DESC indices exist, mixed ORDER BY (ASC, DESC) → NOT eliminated
TYPED_TEST(OrderByIndexTest, BothIndicesMixedOrderNotEliminated) {
  // MATCH (n:L) WHERE n.a > 5 ORDER BY n.a ASC, n.b DESC RETURN n
  FakeDbAccessor dba;
  const auto *const label_name = "L";
  const auto label = dba.Label(label_name);
  ...
  dba.SetIndexCount(label, 1);
  std::vector<ms::PropertyPath> composite_props{ms::PropertyPath{prop_a.second}, ms::PropertyPath{prop_b.second}};
  dba.SetIndexCount(label, std::span{composite_props}, 1, ms::IndexOrder::ASC);
  dba.SetIndexCount(label, std::span{composite_props}, 1, ms::IndexOrder::DESC);
  ...
  EXPECT_TRUE(PlanContainsOp(planner.plan(), OrderBy::kType))
      << "Mixed ASC/DESC ORDER BY should NOT be eliminated even with both ASC and DESC indices";
}
...
TYPED_TEST(OrderByIndexTest, WithNewQueryPartInnerScanNotEliminated) {
  // MATCH (n:X) WHERE n.id > 0 WITH n AS m MATCH (n:Y) WHERE n.id > 0 RETURN m, n ORDER BY n.id
  FakeDbAccessor dba;
  const auto *const label_x = "X";
  const auto *const label_y = "Y";
...
TYPED_TEST(OrderByIndexTest, EdgeTypePropertyRangeEliminated) {
  // MATCH ()-[e:KNOWS]->() WHERE e.since > 2020 RETURN e.since AS es ORDER BY e.since
  FakeDbAccessor dba;
...
TYPED_TEST(OrderByIndexTest, EdgeTypePropertyRangeDifferentPropNotEliminated) {
  // MATCH ()-[e:KNOWS]->() WHERE e.since > 2020 RETURN e ORDER BY e.name
  FakeDbAccessor dba;
  const auto *const edge_type_name = "KNOWS";
  const auto edge_type = dba.EdgeType(edge_type_name);
  const auto since_prop = PROPERTY_PAIR(dba, "since");
...
TYPED_TEST(OrderByIndexTest, GlobalEdgePropertyRangeEliminated) {
  // MATCH ()-[e]->() WHERE e.since > 2020 RETURN e.since AS es ORDER BY e.since
  FakeDbAccessor dba;
  const auto since_prop = PROPERTY_PAIR(dba, "since");
...
TYPED_TEST(OrderByIndexTest, GlobalEdgePropertyRangeDifferentPropNotEliminated) {
  // MATCH ()-[e]->() WHERE e.since > 2020 RETURN e ORDER BY e.name
  FakeDbAccessor dba;
  const auto since_prop = PROPERTY_PAIR(dba, "since");
  const auto name_prop = PROPERTY_PAIR(dba, "name");
