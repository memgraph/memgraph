...
TEST_F(QueryPlanMatchFilterTest, NodeFilterLabelsAndProperties) {
  ...
  std::get<0>(n.node_->properties_)[storage.GetPropertyIx(property1.first)] = LITERAL(42);

  // node filtering
  auto *filter_expr = AND(storage.Create<LabelsTest>(n.node_->identifier_, n.node_->labels_),
                          EQ(PROPERTY_LOOKUP(n.node_->identifier_, property1), LITERAL(42)));
  ...
  ASSERT_TRUE(v4.AddLabel(label1).HasValue());
  EXPECT_EQ(1, PullAll(*produce, &context));
  dba.AdvanceCommand();
...
TEST_F(QueryPlanMatchFilterTest, CartesianEmptySet) {
  ...
  auto m = MakeScanAllNew(storage, symbol_table, "m");
  auto return_n = NEXPR("n", IDENT("n")->MapTo(n.sym_))->MapTo(symbol_table.CreateSymbol("named_expression_1", true));
  auto return_m = NEXPR("m", IDENT("m")->MapTo(m.sym_))->MapTo(symbol_table.CreateSymbol("named_expression_2", true));
...
TEST_F(QueryPlanMatchFilterTest, CartesianThreeWay) {
  ...
  auto n = MakeScanAllNew(storage, symbol_table, "n");
  auto m = MakeScanAllNew(storage, symbol_table, "m");
  auto l = MakeScanAllNew(storage, symbol_table, "l");
  auto *return_n = NEXPR("n", IDENT("n")->MapTo(n.sym_))->MapTo(symbol_table.CreateSymbol("named_expression_1", true));
  auto *return_m = NEXPR("m", IDENT("m")->MapTo(m.sym_))->MapTo(symbol_table.CreateSymbol("named_expression_2", true));
...
TEST_F(QueryPlanMatchFilterTest, ExpandBothCycleEdgeCase) {
...
TEST_F(QueryPlanMatchFilterTest, Filter) {
  ...
  auto n = MakeScanAll(storage, symbol_table, "n");
  auto e = PROPERTY_LOOKUP(IDENT("n")->MapTo(n.sym_), property1);
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabel) {
  ...
  AstStorage storage;
  SymbolTable symbol_table;
  auto scan_all_by_label = MakeScanAllByLabel(storage, symbol_table, "n", label1);
  // RETURN n
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelProperty) {
  ...
  auto run_scan_all = [&](const TypedValue &lower, Bound::Type lower_type, const TypedValue &upper,
    ...
    SymbolTable symbol_table;
    auto scan_all =
        MakeScanAllByLabelPropertyRange(storage, symbol_table, "n", label1, prop, "prop",
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelPropertyEqualityNoError) {
  ...
  AstStorage storage;
  SymbolTable symbol_table;
  auto scan_all = MakeScanAllByLabelPropertyValue(storage, symbol_table, "n", label1, prop, "prop", LITERAL(42));
  // RETURN n
  auto output = NEXPR("n", IDENT("n")->MapTo(scan_all.sym_))->MapTo(symbol_table.CreateSymbol("n", true));
  auto produce = MakeProduce(scan_all.op_, output);
  auto context = MakeContext(storage, symbol_table, &dba);
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelPropertyValueError) {
  auto label1 = db.NameToLabel("label1");
  auto prop = db.NameToProperty("prop");
  {
    auto storage_dba = db.Access();
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelPropertyRangeError) {
  ...
  }
  {
    ...
    auto scan_index = MakeScanAllByLabelPropertyRange(storage, symbol_table, "n", label1, prop, "prop", std::nullopt,
  ...
  }
  {
    ...
    auto scan_index = MakeScanAllByLabelPropertyRange(storage, symbol_table, "n", label1, prop, "prop",
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelPropertyEqualNull) {
  ...
  SymbolTable symbol_table;
  auto scan_all =
      MakeScanAllByLabelPropertyValue(storage, symbol_table, "n", label1, prop, "prop", LITERAL(TypedValue()));
  // RETURN n
  auto output = NEXPR("n", IDENT("n")->MapTo(scan_all.sym_))->MapTo(symbol_table.CreateSymbol("n", true));
  auto produce = MakeProduce(scan_all.op_, output);
  auto context = MakeContext(storage, symbol_table, &dba);
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelPropertyRangeNull) {
  // Add 2 vertices with the same label, but one has a property value while
  ...
  AstStorage storage;
  SymbolTable symbol_table;
  auto scan_all = MakeScanAllByLabelPropertyRange(storage, symbol_table, "n", label1, prop, "prop",
...
TEST_F(QueryPlanMatchFilterTest, ScanAllByLabelPropertyNoValueInIndexContinuation) {
  auto label1 = db.NameToLabel("label1");
  auto prop = db.NameToProperty("prop");
  {
    auto storage_dba = db.Access();
  ...
  // MATCH (n :label1 {prop: x})
  auto scan_all = MakeScanAllByLabelPropertyValue(storage, symbol_table, "n", label1, prop, "prop", x_expr, unwind);

  auto context = MakeContext(storage, symbol_table, &dba);
  EXPECT_EQ(PullAll(*scan_all.op_, &context), 1);
}

TEST_F(QueryPlanMatchFilterTest, ScanAllEqualsScanAllByLabelProperty) {
  auto label1 = db.NameToLabel("label1");
  auto prop = db.NameToProperty("prop");
  ...
  auto count_with_index = [this, &label1, &prop](int prop_value, int prop_count) {
    AstStorage storage;
    SymbolTable symbol_table;
    auto storage_dba = db.Access();
    memgraph::query::DbAccessor dba(&storage_dba);
