...
TEST(StorageV3Gc, Sanity) {
  Storage storage{Config{.gc = {.type = Config::Gc::Type::NONE, .interval = std::chrono::milliseconds(100)}}};
  ...
  std::vector<PrimaryKey> vertices;
  {
    auto acc = storage.Access();
    // Create some vertices, but delete some of them immediately.
    for (int i = 0; i < 1000; ++i) {
      auto vertex = CreateVertexAndValidate(acc, label, {}, {{property, PropertyValue{i}}});
      auto pk = vertex.PrimaryKey(View::NEW);
      ASSERT_TRUE(pk.HasValue());
      vertices.push_back(std::move(pk.GetValue()));
    }
  ...
  // Verify existing vertices and add labels to some of them.
  {
    ...
    for (uint64_t i = 0; i < 1000; ++i) {
      ...
      if (vertex.has_value()) {
        EXPECT_FALSE(vertex->AddLabelAndValidate(LabelId::FromUint(3 * i)).HasError());
        EXPECT_FALSE(vertex->AddLabelAndValidate(LabelId::FromUint(3 * i + 1)).HasError());
        EXPECT_FALSE(vertex->AddLabelAndValidate(LabelId::FromUint(3 * i + 2)).HasError());
      }
    }
    ...
    for (uint64_t i = 0; i < 1000; ++i) {
      ...
      if (vertex.has_value()) {
        auto labels_old = vertex->Labels(View::OLD);
        EXPECT_TRUE(labels_old.HasValue());
        EXPECT_TRUE(labels_old->empty());

        auto labels_new = vertex->Labels(View::NEW);
        EXPECT_TRUE(labels_new.HasValue());
        EXPECT_THAT(labels_new.GetValue(), UnorderedElementsAre(LabelId::FromUint(3 * i), LabelId::FromUint(3 * i + 1),
                                                                LabelId::FromUint(3 * i + 2)));
      }
    }
  ...
  // Add and remove some edges.
  {
    ...
    for (uint64_t i = 0; i < 1000; ++i) {
      ...
      if (from_vertex.has_value() && to_vertex.has_value()) {
        EXPECT_FALSE(acc.CreateEdge(&from_vertex.value(), &to_vertex.value(), EdgeTypeId::FromUint(i)).HasError());
      }
    }
...
// NOLINTNEXTLINE(hicpp-special-member-functions)
// TODO(jbajic) Fix whe indices are enabled
...
//     for (auto vertex : acc2.Vertices(View::OLD)) {
...
//     std::set<Gid> gids;
//     for (auto vertex : acc1.Vertices(acc1.NameToLabel("label"), View::OLD)) {
