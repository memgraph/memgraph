...
// M4: a fork-existing vertex the branch COW'd (BranchContext::CowVertex recreates it at its own gid
// via CreateVertexEx, producing a bare WalVertexCreate record) but never otherwise modified must
// STILL run the D3 unchanged-since-fork check when pass 2 applies that lone create -- the COW echo
// itself is evidence the branch touched this vertex, so it is not a genuine no-op from the branch's
// perspective. Main independently deletes the SAME vertex after the fork point; the merge must
// reject with kModifyConflict instead of silently no-op'ing the echo and letting main's delete win
// unopposed.
TEST_F(VersioningMergeTest, CowEchoWithNoModifyRecordStillDetectsSinceForkMainDelete) {
  const auto prop_p = storage_->NameToProperty("p");
  ...
  // Main deletes vx AFTER the fork point.
  {
    auto acc = storage_->Access(ms::WRITE);
    auto vx = acc->FindVertex(vx_gid, ms::View::OLD);
    ASSERT_TRUE(vx.has_value());
    auto ret = acc->DeleteVertex(&*vx);
    ASSERT_TRUE(ret.has_value());
    ASSERT_TRUE(acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }

  // The branch's own change-log: a LONE WalVertexCreate for vx_gid (the COW echo) with NO
  // accompanying WalVertexSetProperty/AddLabel/RemoveLabel/Delete record for the same gid anywhere
  // in the log -- this is the exact shape M4 identified as bypassing D3 (pass 2's WalVertexCreate
  // handler used to no-op purely on "not branch-local", never invoking the conflict check).
  std::vector<WalDeltaData> changelog{WalDeltaData{ms::durability::WalVertexCreate{vx_gid}}};

  auto result = mv::MergeBranch(*mem_storage_, fork_ts, changelog, mapper, memgraph::tests::MakeMainCommitArgs());
  ASSERT_FALSE(result.has_value());
  EXPECT_EQ(result.error().kind, mv::MergeErrorKind::kModifyConflict)
      << "a bare COW-echo WalVertexCreate for a fork-existing vertex must still trip D3 when main "
         "deleted it since fork -- got: "
      << result.error().message;
  ASSERT_EQ(result.error().conflicting_gids.size(), 1u);
  EXPECT_EQ(result.error().conflicting_gids[0], vx_gid);

  // Main must be byte-unchanged by the rejected merge (still deleted, not resurrected).
  auto acc = storage_->Access(ms::READ);
  EXPECT_FALSE(acc->FindVertex(vx_gid, ms::View::OLD).has_value());

  auto historical = mem_storage_->HistoricalAccess(fork_ts);
  EXPECT_TRUE(historical.has_value()) << "R37: a rejected merge must not release the branch's fork pin";
}

// D3 correctness (edge analog of CowEchoWithNoModifyRecordStillDetectsSinceForkMainDelete): a
// fork-existing edge whose ONLY branch changelog record is a COW echo (WalEdgeCreate with no
// following WalEdgeSetProperty or WalEdgeDelete) must still detect that main modified the edge
// after the fork point.  Before the fix, the WalEdgeCreate COW-echo handler returned {} immediately
// without calling CheckEdgeUnchangedSinceFork, so a since-fork main-side property change was
// silently ignored and would have been overwritten on merge.
TEST_F(VersioningMergeTest, CowEchoEdgeWithNoModifyRecordStillDetectsSinceForkMainChange) {
  const auto prop_p = storage_->NameToProperty("p");
  ...
  ms::Gid v1_gid, v2_gid, e_gid;
  {
    ...
    e_gid = e->Gid();
    ASSERT_TRUE(e->SetProperty(prop_p, ms::PropertyValue(1)).has_value());
    ASSERT_TRUE(acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  ...
  // Main modifies the edge's property AFTER the fork point.
  {
    auto acc = storage_->Access(ms::WRITE);
    auto e = acc->FindEdge(e_gid, ms::View::OLD);
    ASSERT_TRUE(e.has_value());
    ASSERT_TRUE(e->SetProperty(prop_p, ms::PropertyValue(99)).has_value());
    ASSERT_TRUE(acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }

  // The branch's own change-log: a LONE WalEdgeCreate for e_gid (the COW echo produced by
  // CowEdge when the from-vertex is mutated) with NO following WalEdgeSetProperty or WalEdgeDelete
  // for the same gid -- this is the exact shape that bypassed CheckEdgeUnchangedSinceFork before
  // the fix (pass 2's WalEdgeCreate handler returned {} purely on "not branch-local", never
  // invoking the conflict check).
  std::vector<WalDeltaData> changelog{WalDeltaData{ms::durability::WalEdgeCreate{e_gid, "R", v1_gid, v2_gid}}};

  auto result = mv::MergeBranch(*mem_storage_, fork_ts, changelog, mapper, memgraph::tests::MakeMainCommitArgs());
  ASSERT_FALSE(result.has_value());
  EXPECT_EQ(result.error().kind, mv::MergeErrorKind::kModifyConflict)
      << "a bare COW-echo WalEdgeCreate for a fork-existing edge must still trip D3 when main "
         "modified it since fork -- got: "
      << result.error().message;
  ASSERT_EQ(result.error().conflicting_gids.size(), 1u);
  EXPECT_EQ(result.error().conflicting_gids[0], e_gid);

  // Main must be byte-unchanged by the rejected merge: the edge must still carry p=99, not
  // reverted to the branch's fork-point value of 1.
  {
    auto acc = storage_->Access(ms::READ);
    auto e = acc->FindEdge(e_gid, ms::View::OLD);
    ASSERT_TRUE(e.has_value()) << "the rejected merge must not delete main's edge";
    auto props = e->Properties(ms::View::OLD);
    ASSERT_TRUE(props.has_value());
    auto it = props->find(prop_p);
    ASSERT_NE(it, props->end()) << "edge property p must survive the rejected merge";
    EXPECT_EQ(it->second, ms::PropertyValue(99))
        << "main's post-fork property value (99) must not be overwritten by the rejected merge";
  }
...
TEST_F(VersioningMergeTest, DeleteConflictsWhenMainAttachedAnEdgeSinceFork) {
  ...
  EXPECT_EQ(result.error().kind, mv::MergeErrorKind::kModifyConflict)
      ...
      << result.error().message;
  ASSERT_EQ(result.error().conflicting_gids.size(), 1u);
  EXPECT_EQ(result.error().conflicting_gids[0], v_target_gid);
