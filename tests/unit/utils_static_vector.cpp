...
constexpr auto kByteLimit = sizeof(T) * N + sizeof(uint32_t);

static_assert(std::is_trivially_destructible_v<static_vector<int, kByteLimit<int>>>);
static_assert(!std::is_trivially_destructible_v<static_vector<std::string, kByteLimit<std::string>>>);

static_assert(std::contiguous_iterator<static_vector<int, kByteLimit<int>>::iterator>);

TEST(StaticVectorTest, DefaultConstruction) {
  constexpr auto N = 3;
  using sut_t = static_vector<int, kByteLimit<int, N>>;
...
TEST(StaticVector, EmplaceBack) {
  auto vec = static_vector<int, kByteLimit<int, 5>>{};
  vec.emplace_back(42);
