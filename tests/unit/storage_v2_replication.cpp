...
TEST_F(ReplicationTest, BasicSynchronousReplicationTest) {
  ...
  std::optional<Gid> vertex_gid;
  {
    ...
    ASSERT_TRUE(v.SetProperty(main_db.storage()->NameToProperty(vertex_property), PropertyValue(vertex_property_value))
                    .HasValue());
...

  {
    ...
    ASSERT_THAT(*properties, UnorderedElementsAre(std::make_pair(replica_db.storage()->NameToProperty(vertex_property),
  ...
  std::optional<Gid> edge_gid;
  {
    ...
    ASSERT_TRUE(edge.SetProperty(main_db.storage()->NameToProperty(edge_property), PropertyValue(edge_property_value))
                    .HasValue());
...

  {
    ...
    ASSERT_TRUE(properties.HasValue());
    ASSERT_EQ(properties->size(), 1);
    ASSERT_THAT(*properties, UnorderedElementsAre(std::make_pair(replica_db.storage()->NameToProperty(edge_property),
  ...
  }
  {
    auto unique_acc = main_db.UniqueAccess();
    ASSERT_FALSE(
        unique_acc->CreateIndex(main_db.storage()->NameToLabel(label), main_db.storage()->NameToProperty(property))
            .HasError());
    ASSERT_FALSE(unique_acc->Commit().HasError());
  }
  {
    auto unique_acc = main_db.UniqueAccess();
    unique_acc->SetIndexStats(main_db.storage()->NameToLabel(label), main_db.storage()->NameToProperty(property),
                              lp_stats);
    ASSERT_FALSE(unique_acc->Commit().HasError());
  }
  {
...

  {
    ...
    const auto &l_stats_rep = replica_db.Access()->GetIndexStats(replica_db.storage()->NameToLabel(label));
    ASSERT_TRUE(l_stats_rep);
    ASSERT_EQ(l_stats_rep->count, l_stats.count);
    ASSERT_EQ(l_stats_rep->avg_degree, l_stats.avg_degree);
    const auto &lp_stats_rep = replica_db.Access()->GetIndexStats(replica_db.storage()->NameToLabel(label),
                                                                  replica_db.storage()->NameToProperty(property));
  ...
  }
  {
    auto unique_acc = main_db.UniqueAccess();
    ASSERT_FALSE(
        unique_acc->DropIndex(main_db.storage()->NameToLabel(label), main_db.storage()->NameToProperty(property))
            .HasError());
    ASSERT_FALSE(unique_acc->Commit().HasError());
  }
  {
  ...
  }
  {
    ...
    ASSERT_EQ(unique_acc->DropUniqueConstraint(
                  main_db.storage()->NameToLabel(label),
                  {main_db.storage()->NameToProperty(property), main_db.storage()->NameToProperty(property_extra)}),
