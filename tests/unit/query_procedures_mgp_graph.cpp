...
TYPED_TEST(MgpGraphTest, CreateVertex) {
  ...
  auto read_uncommitted_accessor = this->storage->Access(memgraph::storage::IsolationLevel::READ_UNCOMMITTED);
  ...
  EXPECT_TRUE(read_uncommitted_accessor->FindVertex(memgraph::storage::Gid::FromInt(vertex_id.as_int),
...
TYPED_TEST(MgpGraphTest, VertexSetProperty) {
  ...
  }
  {
    ...
    const auto maybe_prop = vertex_acc->GetProperty(read_uncommitted_accessor->NameToProperty(property_to_set),
                                                    memgraph::storage::View::NEW);
    ASSERT_TRUE(maybe_prop.HasValue());
...
TYPED_TEST(MgpGraphTest, EdgeSetProperty) {
  ...
  MgpEdgePtr edge;
  ASSERT_NO_FATAL_FAILURE(this->GetFirstOutEdge(graph, from_vertex_id, edge));
  const auto edge_acc = get_edge(read_uncommitted_accessor.get());
