...
TYPED_TEST(InfoTest, InfoCheck) {
...

  {
    ...
    ASSERT_FALSE(!acc->CreateIndex(lbl).has_value());
    ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
    auto acc = this->CreateIndexAccessor();
    ASSERT_FALSE(!acc->CreateIndex(lbl, {prop}).has_value());
    ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  {
    auto acc = this->CreateIndexAccessor();
    ASSERT_FALSE(!acc->CreateIndex(lbl, {prop2}).has_value());
    ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  if constexpr (!is_using_disk_storage) {
    {
      auto acc = this->CreateIndexAccessor();
      ASSERT_FALSE(!acc->CreateIndex(lbl, {prop, prop2}).has_value());
      ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
    }
    {
      auto acc = this->CreateIndexAccessor();
      ASSERT_FALSE(!acc->CreateIndex(lbl, {prop2, prop}).has_value());
      ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
    }
  }
  {
    auto acc = this->DropIndexAccessor();
    ASSERT_FALSE(!acc->DropIndex(lbl, {prop}).has_value());
    ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
  if constexpr (!is_using_disk_storage) {
    auto acc = this->DropIndexAccessor();
    ASSERT_FALSE(!acc->DropIndex(lbl, {prop, prop2}).has_value());
    ASSERT_FALSE(!acc->PrepareForCommitPhase(memgraph::tests::MakeMainCommitArgs()).has_value());
  }
