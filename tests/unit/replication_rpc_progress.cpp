...
TEST_F(ReplicationRpcProgressTest, PrepareCommitNoTimeout) {
  ...
  ReplicaStream stream{&main_storage, std::move(stream_handler.value())};
...
TEST_F(ReplicationRpcProgressTest, PrepareCommitTimeout) {
  ...
  ASSERT_EQ(res.error(), memgraph::utils::RpcError::GENERIC_RPC_ERROR);
}
...
TEST_F(ReplicationRpcProgressTest, TestTTT) {
...

  {
    ...
    ASSERT_TRUE(wal_files_stream.value().SendAndWaitProgress().has_value());
  }
}
