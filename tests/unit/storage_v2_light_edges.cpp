...
// PR7b-ii regression: with the collection-time routing fix, a DELETE+commit
// (without any subsequent GC/FreeMemory) leaves the deleted light Edge* sitting
// in deleted_edges_ — they are NOT pushed to the graveyard until a GC cycle
// swaps deleted_edges_ and snaps the graveyard watermark. ClearLightEdges()
// (run from ~InMemoryStorage) must drain deleted_edges_ via DestroyLightEdge or
// those pool-allocated Edge* leak. Under ASan this test fails on the leak if the
// drain is missing; under RelWithDebInfo it at least exercises the teardown path.
TEST(LightEdgesCleanup, DeletedButNotGCedEdgesFreedAtTeardown) {
  {
    ...
    // DELETE + commit all edges. With the PR7b-ii fix these go to deleted_edges_,
    // NOT the graveyard.
    {
      auto acc = store->Access(WRITE);
      auto vf = acc->FindVertex(gid_from, View::NEW);
...
// PR7b-ii regression, Clear() path: same scenario as above but the deleted
...
TEST(LightEdgesGraveyard, PostCommitIndexIterableBlocksDrain) {
  ...
  // collection-time light arm snaps guard_epoch = CurrentEpoch() > e_R.
  // Drain must NOT happen yet because IsSafeToFree(guard_epoch) is false (e_R is live).
  store->FreeMemory();
...
//
// This is the teeth behind PostCommitIndexIterableBlocksDrain: it not only
// asserts the graveyard size stays at 1 while the iterable is alive, but
// actually ITERATES the post-commit index iterable AFTER a FreeMemory drain and
...
// PR7b-ii: a light edge created AND deleted in the same committed transaction,
// never GC'd. When this is the only transaction in flight, FastDiscardOfDeltas
// fires at commit time (no older/newer transactions), which routes the deleted
// Edge* into deleted_edges_; ClearLightEdges then drains it. This test verifies
// the basic DropGraph + ClearLightEdges path for the single-transaction case.
// The concurrent-transaction variant (FastDiscard blocked) is covered by
// DropGraphFreesDeltaChainOnlyEdgesWithConcurrentTxn below.
// NOTE: DropGraph does NOT call Clear() — it calls ClearLightEdges directly
// plus the HarvestDeltaChainOnlyLightEdges call added in PR7b-ii (F2 fix).
...
// PR7b-ii F2 fix: DropGraph must free delta-chain-only light edges even when
// FastDiscardOfDeltas was blocked by a concurrent older transaction.
//
...
// Without the HarvestDeltaChainOnlyLightEdges() call added to DropGraph (F2),
...
TEST(LightEdgesCleanup, DropGraphFreesDeltaChainOnlyEdgesWithConcurrentTxn) {
  ...
  // holds B's delta chain (the only owner of the deleted Edge*).
  acc_a->Abort();
  acc_a.reset();

  // Precondition: the edge must NOT have reached the graveyard — GC must not
  // have run. If this assertion fires the test setup is wrong.
  ASSERT_EQ(mem_storage->LightEdgeGraveyardSizeForTest(), 0)
      << "Graveyard must be empty — GC must not have run before DropGraph";

  // DropGraph on the analytical accessor. Without the F2 fix
