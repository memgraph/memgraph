...
test_mgconsole() {
    ...
    # ($MEMGRAPH_EXEC is `docker exec` or `kubectl exec` depending on the deployment)
    ...
    path="$($MEMGRAPH_EXEC bash -c 'realpath "$(command -v mgconsole)"')"
    ...
    version="$($MEMGRAPH_EXEC mgconsole --version)"
