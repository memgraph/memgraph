...
test_user_profiles() {
  ...
  run_query_admin "CREATE PROFILE user_profile LIMIT SESSIONS 2, TRANSACTIONS_MEMORY 100KB;"
  run_query_admin "SET PROFILE FOR tester TO user_profile;"
  run_query_admin "SHOW PROFILE FOR tester;"

  echo "SUBFEATURE: Test memory limit enforcement"
  # Test query that should work within 100KB limit
  run_query_tester "CREATE (n:TestNode {id: 1, data: 'small'});"
  # This one should fail. Check for failure and continue only if it fails.
  if ! run_query_tester "UNWIND range(1, 10000) AS i CREATE (n:MemoryTest {id: i, data: 'Data string ' + i}) RETURN count(n);"; then
    echo "Memory limit enforcement by profile test failed as expected."
  fi

  echo "SUBFEATURE: Cleanup profile"
  run_query_admin "DROP PROFILE user_profile;"
  run_query_admin "SHOW PROFILES;"
  run_query_admin "MATCH (n:TestNode) DETACH DELETE n;"
  run_query_admin "MATCH (n:MemoryTest) DETACH DELETE n;"
