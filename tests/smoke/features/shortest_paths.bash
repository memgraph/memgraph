...
test_shortest_paths() {
  ...
  run_query "MATCH (n1:Node {id: 'A'}), (n2:Node {id: 'E'}) WITH n1, n2 MATCH p=(n1)-[*KShortest | 3]->(n2) RETURN p;"
  echo "SUBFEATURE: KShortest paths - with path length bounds"
  run_query "MATCH (n1:Node {id: 'A'}), (n2:Node {id: 'E'}) WITH n1, n2 MATCH p=(n1)-[*KShortest 2..4]->(n2) RETURN p;"
  echo "SUBFEATURE: Test KShortest paths with the bounds and limit"
  run_query "MATCH (n1:Node {id: 'A'}), (n2:Node {id: 'E'}) WITH n1, n2 MATCH p=(n1)-[*KShortest 2..4 | 5]->(n2) RETURN p;"

  echo "SUBFEATURE: AllShortest paths - find all shortest paths from A to E (with weight lambda)"
  run_query "MATCH (n1:Node {id: 'A'}), (n2:Node {id: 'E'}) WITH n1, n2 MATCH p=(n1)-[*AllShortest (r, n | r.weight)]->(n2) RETURN p;"
  echo "SUBFEATURE: AllShortest paths - with the upper bound and total_weight"
  run_query "MATCH (n1:Node {id: 'A'}), (n2:Node {id: 'E'}) WITH n1, n2 MATCH p=(n1)-[*AllShortest ..4 (r, n | r.weight) total_weight]->(n2) RETURN p, total_weight;"
  echo "SUBFEATURE: AllShortest paths - using weight lambda + total_weight + the filter"
  run_query "MATCH p=(n1:Node {id: 'A'})-[*AllShortest ..4 (r, n | r.weight) total_weight (r, n, p, w | r.weight > 0 AND length(p) > 0)]->(n2:Node {id: 'E'}) RETURN p;"
