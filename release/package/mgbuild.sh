...
test_memgraph() {
  ...
  case "$1" in
    ...
    mgbench)
      ...
      docker exec -u mg $build_container bash -c "$EXPORT_LICENSE && $EXPORT_ORG_NAME && cd $MGBUILD_ROOT_DIR/tests/mgbench && ./benchmark.py --installation-type native --num-workers-for-benchmark 24 --export-results $EXPORT_RESULTS_FILE $DATASET/$DATASET_SIZE/*/*"
    ;;
    mgbench-supernode)
      shift 1
      local EXPORT_RESULTS_FILE='benchmark_result.json'
