...
test_memgraph() {
  ...
  case "$1" in
    ...
    ;;
    query_modules_e2e)
      docker exec -u mg $build_container bash -c "$EXPORT_LICENSE && $EXPORT_ORG_NAME && $ACTIVATE_CARGO && pip install -r $MGBUILD_ROOT_DIR/tests/query_modules/requirements.txt && cd $MGBUILD_ROOT_DIR/tests && $ACTIVATE_VENV && source ve3/bin/activate_e2e cd $MGBUILD_ROOT_DIR/tests/query_modules "'&& python3 -m pytest .'
    ;;
    *)
