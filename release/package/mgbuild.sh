...
test_memgraph() {
  ...
  case "$1" in
    ...
    stress-plain)
      ...
      echo "-------------------------"
      docker exec -u mg $build_container bash -c "cd $MGBUILD_ROOT_DIR/tests/stress && export PATH=\$PATH:\$HOME/.local/bin && echo $PATH && source ve3/bin/activate && uv pip list"
      echo "-------------------------"
