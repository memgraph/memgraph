...
print("Starting get_version.py", flush=True, file=sys.stderr)
...
def get_output(*cmd, multiple=False):
    print(f"Running command: {' '.join(cmd)}", flush=True, file=sys.stderr)
...
print("Checking if we can connect to GitHub...", flush=True, file=sys.stderr)
can_connect = can_connect_to_github()
print(f"Can connect to GitHub: {can_connect}", flush=True, file=sys.stderr)
